...
| **"No effect because it didn't train"** | The per-candidate budget is set from a **measured learning-curve pilot** (critic-loss knee ~100k; held-out performance flat within seed noise across the 25k–350k ladder; the extended 30-point same-protocol curve then located the knee at 400,000 steps (R77), which is the confirmatory campaign budget B\*), applied **identically across arms** (matched compute), with a learning-curve diagnostic disclosed. |
