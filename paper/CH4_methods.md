...
designer beats numerical search over the identical reward family — its confirmatory node (N4) is the pre-registered intersection–union test over the best of the optimiser portfolio (§4.5), developed below.
...
**H4 dominates the optimiser portfolio.** Exactly as H1, H4 is a snoop-free intersection–union test (node N4): the designer's reward *dominates* the portfolio {random search, GP-EI, CMA-ES, TPE} — beating every member one-sided at $\alpha$, which is precisely "beats the best" (the best is the pointwise maximum) while selecting no comparator to snoop. Because the intersection–union conjunction has size $\le\alpha$ however many optimisers it spans [`berger1982iut`], enlarging the portfolio only makes the test *more* conservative — it adds no family-wise multiplicity and cannot inflate significance. Two features make the contest both hard and decisive. First, the designer authors free-form code — a strict superset of the six-term family the optimisers search — so under the identical thirty-evaluation budget it must navigate a vastly larger, harder space; its only advantage is its semantic prior, which makes beating optimisers that enjoy a small, efficiently searchable, hand-crafted space a *strong* result rather than a foregone one. Whether any edge is expressivity or search quality — does the designer author rewards outside the family's span? — is not asserted but *decomposed* by the mechanism audit (Chapter 6). Second, the verdict is informative either way: beyond superiority, the same comparison reads non-inferiority to the best optimiser at the pre-registered SESOI, giving a three-way conclusion — the designer *exceeds*, *matches* (within the SESOI), or is *beaten by* best-in-class numerical optimisation, each a publishable outcome. To our knowledge no prior reward-design study pre-registers such a head-to-head against best-in-class derivative-free optimisation of a matched reward family — its neighbours benchmark against human, sparse, or prior-model rewards [`ma2024eureka`; `hazra2025revolve`; `xie2024text2reward`] — a gap this node is built to fill.
