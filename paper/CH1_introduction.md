...
![**Figure 1.2 — The three outcomes, and the reading each was given before the data were seen.** The root is H2, decided on two co-primary outcomes at once. Left, the whole interval falls inside the pre-registered SESOI; middle, the lower confidence limit clears 0; right, the minimum detectable effect exceeds SESOI, so neither verdict may be claimed. **What to conclude:** no outcome can be reinterpreted after the fact, including the one that favours this study least.
