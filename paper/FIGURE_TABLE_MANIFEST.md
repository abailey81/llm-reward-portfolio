...
| F5 | rliable headline interval | Per-seed IQM intervals (stratified bootstrap) for the H2 contrasts across the 9 arms — the headline visual. | `[CAMPAIGN]` archive (rliable). | 6 (§6.2) | **[CAMPAIGN]** |
...
| T4 | Secondary hypotheses | H1 (descriptive, both caveats), H3 (TOST-bounded equivalence), H4 (LLM vs the best of the optimiser portfolio {random_search, bayes_opt, cma_es, tpe} at matched compute; node N4). | `[CAMPAIGN]` archive. | 6 (§6.4) | **[CAMPAIGN]** |
