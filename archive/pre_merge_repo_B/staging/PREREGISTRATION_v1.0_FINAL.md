...
- Rank stability is operationalised as the fraction of (known-good, known-degenerate) pairs whose fitness
  orders good above degenerate, averaged over seeds; ties on the grid are broken by (i) the smallest
  across-seed standard deviation of that accuracy, then (ii) the **smallest** λ (parsimony). The full
  per-λ table is reported in the freezing ADR (implementation: `src/calibrate_lambda.py`).
...
### 4a. Parameterised reward family (the H4 search space)

The random-search and BayesOpt arms draw from one pre-registered family — the six-term linear reward

> r_t = w_return·net_return + w_log·log1p(net_return) − w_turnover·turnover − w_drawdown·drawdown_t
> − w_cvar·max(0, −CVaR̂_α(rolling window)) − w_vol·σ̂(rolling window)

with weight ranges, the CVaR-level choice set, and the rolling-window choice set fixed in
`config/eureka_loop.yaml: reward_family` (uniform sampling; implementation `src/reward_family.py`;
both arms share the same constructor `params_to_reward`). The family's vertices recover the
hand-designed canon of §4's baseline set, making H4 a like-for-like comparison of **search procedures**
at matched budget rather than of unrelated reward spaces.
...
| Freeze commit hash | recorded in `DECISIONS.md` ADR-005 (single source; unfillable inside the freeze commit itself) |
