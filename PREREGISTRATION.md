...
return−CVaR, differential Sharpe, differential downside deviation ratio (R97), mean–variance utility,
return−drawdown, return−downside, return−turnover, log-growth) are reported as a secondary "did the LLM
beat hand-written rewards?" panel.
...
> the FROZEN confirmatory-adjacent H1 family remains the four
> (`h1_baselines`, unchanged — H1 multiplicity untouched); the full ten-name canon
> (`config/eureka_loop.yaml: baseline_rewards` == `REWARD_CANON`) is the **report-only secondary
> panel, executed at the tier-30 floor seed set (seeds 0–29) post-headline at rock-bottom queue
> priority** via the documented `--baselines` override (runbook §9(h)); if deadline compute forces
> truncation, the executed subset is disclosed in §6.7's slot rather than silently narrowed. No
> arm, no fed input, no H2 quantity changes; the identification principle is untouched.
...
> "0.25 CVaR tail margin" was an invented number and is RETRACTED (the tail bounded-effect is the R86
> bootstrap CI). `tests/test_sesoi_derivation.py` re-derives the band from the registered inputs. Mirrored
