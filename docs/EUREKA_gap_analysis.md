...
| LLM authors reward *code* | Yes — Python source, numpy-only, AST-gated once then run in-process. Author = **Claude Opus 4.8** (campaign) / Sonnet 4.6 (prototype). | `src/sandbox/executor.py`; `src/llm/loop.py:353,361` |
...
| Reward reflection | **Different object (the contribution).** Reflection feeds the **scalar fitness + an off-critic tail block**, NOT the reward's component trajectories. | `prompts/reflection.txt`; `src/feedback/schema.py::build_block` |
...
|---|---|---|---|
| Reward authored as code by an LLM | ✔ (GPT-4) | ✔ (Claude Opus 4.8) | **shared (lineage)** |
...
- **Verdict: NOT WORTH IT / REJECT.** (i) The reflection prompt is part of the **frozen** arm-isolation design;
  ...
  (ii) Opus 4.8 already authors competent multi-term rewards under the existing contract (prototype ran to
