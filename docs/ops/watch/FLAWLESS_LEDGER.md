...
| **A-f4** | 2026-08-04 pass 5 | **OPEN (deferred, correctly)** | Auditor finding 4: `git commit --only` returns **rc=128** (`fatal: cannot do a partial commit during a merge`) if a human leaves a merge in progress. `publish_status.sh:520` redirects stderr to `/dev/null` and the `&&` chain means **neither push runs**, while the loop prints "no change to publish" -- indistinguishable from benign, and `check_status_page` only measures the page's MTIME so it stays OK. Honest counterweight: the OLD behaviour was worse (a 2-minute loop would have CONCLUDED a human's merge). It is an observability regression, not a safety one. ⛔ **NOT fixed this pass because `publish_status.sh` was VERIFIED LIVE at the moment of the edit** (chain 14992 -> 15572 -> 10720) and editing a running shell script is P250. Fix when idle: capture the commit's stderr and echo a distinct message when it is not "nothing to commit". |
