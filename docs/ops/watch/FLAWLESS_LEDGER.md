...
|---|---|---|---|
| A5 | 08-04 pass-3 auditor | S15's **exit code ignores C6 entirely** (`if holed or over or dupes: return 1`). An archive with ZERO holes but registered arms banking 0 exits **0** and prints "VERDICT: CLEAN". The campaign-capping condition C6 exists to surface has no machine-readable signal. Masked today only because 11 arms do hold holes. | give C6 its own exit path, without making the code permanently non-zero (the always-on-alarm pathology) |
| A6 | 08-04 pass-3 auditor | **`test_h3_singleshot` is folded into the campaign-wide minimum.** R101 defines the result as the common rung over the **11 full-loop models**; h3 is the H3 single-shot CONTROL, not a model leg. Direction: too LOW, it would depress the reported result if it lagged. Non-binding today (h3 reads 568). ⚠ **ESCALATED, not fixed: this changes the definition of the reported scientific result and is a pre-registration question, not an ops patch.** For Tamer and Dr Okhrati. | Tamer's call |
