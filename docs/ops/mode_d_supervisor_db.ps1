# UNFENCED SUPERVISOR COPY - POOL WIDENING d -> db (created 2026-08-06, RUN 24, D30).
#
# WHY A COPY RATHER THAN AN EDIT. The one token this file changes lives in
# scripts/mode_d_supervisor.ps1, which is DRIFT-FENCED: editing it makes `drift` non-zero against
# cycle.py RUNNING_SHA, and re-basing that sha would assert "the running drivers are on this
# commit" while only the canary line actually is. The supervisor has no -Pool parameter, so the
# value cannot be injected. This repo already solved this exact shape once:
# docs/ops/watchdog_fenced.ps1 is "a faithful copy of scripts/mode_d_watchdog.ps1 that also
# carries -ExcludeHosts", created because the fenced original lacked a parameter. Same pattern.
#
# THE ONLY DIFFERENCE FROM THE FENCED ORIGINAL is the $cpuLane pool token: "d" -> "db".
# Diff this file against scripts/mode_d_supervisor.ps1 and exactly one line may differ.
#
# WHY db IS SAFE - IDENTITY, NOT TOLERANCE. The C3 gate enforces per-seed substrate homogeneity on
# `cpu model | omp | threads | cuda`. Probed first-hand 2026-08-06: node-b00a-014 and node-b00a-013
# both report Intel(R) Xeon(R) Gold 6240 CPU @ 2.60GHz, 2 sockets, 36 cores, microcode 0x5003901,
# flags sha 9ede37ab7eb264ea - byte-identical to node-d00a-246 in pool d. Same silicon, same
# reduction order. And the site JSV maps the allow= context onto a wildcard PE, so the value is
# "db" (granted PE smp-[BD]*), never "d,b" - `-pe smp-B` is rejected outright by policyjsv.
#
# THE ONE HOST THAT IS FENCED, AND WHY. node-b00a-008 reports the SAME model and SAME microcode but
# a DIFFERENT flags sha (639b672208417b8c). The C3 gate keys on the model NAME and so would NOT
# see it, and the determinism envelope covers anything that can change floating-point reduction
# order. "Most likely harmless" is not the standard, so it is EXCLUDED until its flag set is
# diffed. Unknown means excluded. That costs 1 host of 16.
#
# !! THE NAME MATTERS. docs/ops/watchdog_fenced.ps1 finds live lines with
#   $_.CommandLine -match 'mode_d_supervisor'
# so this filename DELIBERATELY keeps that substring: the watchdog therefore sees a line running
# under this copy as ALIVE and will not start a second supervisor for it. Renaming this file
# without that substring would produce two supervisors and two drivers for one batch tag, which is
# exactly the P12 "ONE driver per batch name" violation the queue-adoption guard exists to prevent.
#
# FALLBACK IS SAFE BY CONSTRUCTION. If a line running under this copy dies, the watchdog revives it
# from the FENCED script, i.e. back onto pool d. Losing the widening is not a hazard.
#
# ROLLOUT IS GRADUAL BY CONSTRUCTION. run_batch submits only when a batch has NO live job in the
# queue, so a restarted line ADOPTS its in-flight work and submits nothing; already-queued jobs keep
# allow=d and drain normally, and only the NEXT batch boundary picks up the wider pool.
#
# !! REGISTERED DEFERRED FIX: the correct long-term change is a -Pool parameter on
# scripts/mode_d_supervisor.ps1 at the next deploy window, which retires this copy. A copy DRIFTS
# from its original - that is the standing cost of this pattern and the reason it is registered.
