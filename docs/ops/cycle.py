...
RUNNING_SHA = "996f3cbc"  # ⚠ RE-BASED 2026-08-06 (RUN 26) — THE "RELAUNCHED ONTO IT" KIND, and the first in a while that genuinely was. `996f3cbc` separates `pack` (concurrency) from `specs_per_task` (batch size): the DURATION lever. SIX lines were STOPPED AND RESTARTED onto it within four minutes — nemotron(32256) deepseek(36208) glm(41752) qwen3.6(25728) haiku(17596) kimi(24804) — each verified by reading `-SpecsPerTask 24 -HRt 45:0:0` back off its LIVE `CommandLine` via Win32_Process, and each driver confirmed error-free over its last 300 log lines. ⚠ THE `core` LINE (pid 4768) WAS DELIBERATELY NOT RESTARTED: it carries the entire reported result and its C2 h2_pair round is imminent. It therefore still executes the PRE-996f3cbc code — and that is FINE rather than a hidden drift, because the change is BYTE-IDENTICAL with the flag unset (`_chunk_packs` returns [8,8,8,8,8] for 40 specs at pack 8 either way, verified on the real function), so HEAD describes `core`'s behaviour exactly as well as the old sha did. WHY 24 AND NOT 16: 2,000 cores = 250 running jobs, and steady state is `running = rate x duration`; our demonstrated sustained rate is 10.9 jobs/h (99 jobs / 9.12 h), so 250 jobs needs 27.4/h at 8 specs, 13.7/h at 16 (above what we have ever held) and 9.1/h at 24 (BELOW it). `--h-rt 45:0:0` keeps today's exact 61% utilisation margin and stays under `ucbtjji`'s demonstrated 48 h. It also drops the campaign's remaining 22,590 records from 2,824 jobs to 941 — under `max_u_jobs 1000` for the first time — which independently retires the C4 cap breach in ledger R25-2. PREVIOUS VALUE 309565f9, re-based 2026-08-01:… -> b38ad14 (§100.40) -> 26807b8 (§100.43/47) -> 775c942d (§100.49, the leg --pipeline-rungs fix) -> 58b388f2 -> a8ff5e64 (RUN 12: src/viz/figures.py::seed_trajectory, D2/registry row 38) -> dd51ba59 -> d866afd3 (RUN 13: D27, CMA-ES generation batching + the chain_progress unit fix; the CORE line was relaunched onto it, and the change is provably unreachable from any leg, so this sha describes every line's behaviour) -> 309565f9 (RUN 14: a COMMENT-ONLY change to scripts/mode_d_supervisor.ps1 recording the D30 pool-widening measurement and its one-token edit at the insertion point. NO line was relaunched and none needed to be: a diff of that file against d866afd3 with comment lines excluded is EMPTY, PowerShell comments do not execute, and the $cpuLane argument array is byte-identical — the pool token still reads "d". This is the "provably inert" kind of re-base, not the "relaunched onto it" kind, and it is recorded as such so the distinction is never blurred).
