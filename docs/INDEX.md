...
- [CAMPAIGN_DESIGN_AND_EXECUTION_PLAN.md](CAMPAIGN_DESIGN_AND_EXECUTION_PLAN.md) — **the authoritative determination record** (2026-06-29): every parameter (200k steps / 30 seeds / 30 candidates / 7 arms) literature-justified, the 4-class framework, the power analysis, construct-validity verification, the tiered 14-day 24/7 compute plan, the framing strategy, and the freeze-blocker list. Methods-chapter feedstock.
- [DESIGN_DETERMINATION.md](DESIGN_DETERMINATION.md) — the Design Determination Pipeline output (per-parameter status + FREEZE-READY verdict; generated by `scripts/determine_design.py`).
