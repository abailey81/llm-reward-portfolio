...
## D37-D48 — `src/cluster/campaign.py` + `src/cluster/driver.py`, the FULL fenced-module audit (2026-08-04, RUN 20)

**A read-only auditor read 100% of both files** (`driver.py` 673/673, `campaign.py` 2072/2072) plus
`poll.py`, `ledger.py`, `integrity.py`, `test_leg.py`, `run_campaign*.py`, `mode_d_supervisor.ps1`,
`sentinel.py` and `spec_io.py`. **Both files are DRIFT-FENCED for the campaign's duration, so nothing
here is actionable now — this is the register a deploy window works from.** Ranked.

| id | sev | file:line | defect | can any instrument see it? |
|---|---|---|---|---|
| **D37** | **CRITICAL** | `campaign.py:1793-1795`, `:1905`, `:1980` | **AN ARM THAT FAILS SELECTION VANISHES FROM C2 AND C4 WITH NO LOG, NO MARKER AND NO SUMMARY FIELD.** `winners` is the only channel from C1 to C2/C4 and both consumers filter with `if a in winners`, so a selection failure removes the arm from the H2 pair array and from every C4 rung silently. Reached when `select_winner` returns `None` (an empty search sub-root — including a relaunch whose `--leg`/`--root-suffix` differs) or on `NoEligibleWinnerError` (R115: every candidate over the fallback ceiling). Case (a) **emits nothing at all** in the tiered path, and `campaign_summary.json` never carries the `reason`. | **LOUD variant YES** — no core test records → the C3 gate reads `present=0/30`, `health_ok=False`, exit 3, and the supervisor deliberately does NOT relaunch. **SILENT variant NO** — an arm with complete core records whose selection later fails leaves the gate GREEN, and `sentinel.check_progress` returns only INFO. **A silent cap on the COMMON RUNG.** |
| **D38** | MAJOR | `campaign.py:1161-1170` vs `:1111-1117` | The **DFO batch path** never calls `record_failed_candidate`, and its resume filter never calls `candidate_failed_before` — so a candidate that failed inside a batched generation leaves no `.failed.json` and is re-trained on resume. A retry that succeeds substitutes a real fitness for the `-1e9` the original chain consumed, so **the same registered budget can yield a different H4 winner across a relaunch.** Reopens deep-review #62 for `cma_es`/`tpe`/`bayes_opt`. | **NO.** Only indirectly, and only after a freeze: `_refuse_winner_swap:1357` would raise on the hash change, producing a crash report rather than a diagnosis. |
| **D39** | MAJOR | `campaign.py:237-240` | **The shared throttled puller RETURNS SUCCESS WITHOUT PULLING.** With `min_pull_interval=60 s` and `busy` held for a whole chunked tar-over-ssh, a cached return is indistinguishable from a real pull. Three consequences: `driver.py:445-447` **resets the consecutive-failure counter AND the 12 h outage clock on a non-pull**; the documented "never act on a stale mirror" guarantee is void, so the drain-transition block can requeue already-archived run_ids; and `run_search_arm`'s paid-resume guard (`:815-830`) retries only on EXCEPTIONS, so a resume can **re-author PAID candidates** off a mirror this process has never refreshed. | **NO instrument checks either.** Evidence would be duplicate `llm_calls.jsonl` + spend rows for one `candidate_id`, and rising `rounds` in `driver_status/<batch>.json`. |
| **D40** | MAJOR | `driver.py:414-421`, `:578-585` + `ledger.py:17-18` | **The retry-exhaustion guard is effectively unreachable under the live supervisor.** `_cluster_retries` lives in the in-memory spec dict, so **every relaunch resets it to 0** — and the file documents that as safe on the premise that "a human intervened", which is false: the supervisor relaunches automatically every 600 s. Compounded by `job_ids=[]` after a restart granting 3 further UNBUMPED requeues, and by stale `task_N.json` files mis-keying `_attempted_run_ids`. With `max_wall_secs=None`, a deterministically-failing spec can hold a batch, a generation and a line open **forever with no error**. | Partial — static `done` with rising `rounds`. Nothing attributes it to the retry reset. |
| **D41** | MAJOR | `campaign.py:1938`, `:1971-1975`; `run_campaign_cluster.py:1382` | **The C3 approval is consumed BEFORE the work it authorises is durable**, so a reboot anywhere in C4 sends the relaunch back into the same RED gate with no approval → exit 3 → the supervisor **breaks without relaunching**, and the line is dead until a human notices. Separately `--approve-tier1` writes the **unqualified** `TIER1_APPROVED` filename the gate no longer reads, so the flag is decoration; only a hand-created `TIER1_APPROVED_<line_tag>` works. | Yes, but only as a dead line. |
| **D42** | MODERATE | `campaign.py:1897-1902` | The ARM_CRASH marker's real problems are **not** the one RUN 20 first claimed (see P295). They are: the clear needs a COMPLETE clean C1 pass, which mid-search is DAYS; the clear condition (`_crashed`, only results with an `error` key) is **weaker than the alarm condition**, so a `no_winner` pass clears it while the line is degraded; and the unlink's `except Exception: pass` is **silent**, so a transient Windows lock makes it genuinely permanent with nothing reporting it. The write is also a bare `write_text` (a torn marker is possible; it reads as RED, the safe direction). | Yes — `cycle.py` is the instrument, and its text was corrected by P295. |
| **D43** | MINOR | `driver.py:289-311` | The driver lock is **created empty then filled non-atomically**; a second acquirer reading in that window sees `""`, treats it as torn/stale, unlinks a LIVE owner's lock and takes it — two drivers on one batch. Sub-millisecond window, so low probability; the fix is trivial (write to `<lock>.tmp` then `os.rename`). | No. |
| **D44** | MINOR | `campaign.py:1287`, `:1914`, `:2043-2045` | Three surviving fail-open `.get("ok", True)` aggregation sites, including one where a sweep block that **never ran** counts as OK. The two headline sites were deliberately made fail-closed; these were not. | No. |
| **D45** | MINOR | `campaign.py:369` | `spend_guard` is **inert in the live configuration** — `max_author_calls` is None because the supervisor passes no `--max-author-calls`, so `author_guard` is `lambda: None`. A guard that cannot fire; must not be mistaken for active protection. | n/a |
| **D46** | MINOR | `campaign.py:701-705`; `integrity.py:443,470` | `_ledger_failure` appends and flushes but **never fsyncs**, so a power loss drops the last rows → `--resume` re-authors those (PAID) candidates and the search census under-counts. The tier1 integrity report is a bare `write_text`, so a concurrent reader can see a torn file. ★ The frozen winner itself is SAFE — `io/results.write_run` fsyncs sidecars first and commits `record.json` atomically. | No. |
| **D47** | MINOR | `campaign.py:250-255` | `sync_epilogue_ledgers` **and** `_enforce_kill_switch` share one `except Exception: pass`, so a lost RETREAT verdict is invisible — a safety mechanism whose failure is silent. It also only runs on the thread performing a REAL pull, so it is skipped in every busy window (ties to D39). | No. |
| **D48** | MINOR | `poll.py:339` | `pull_archive` returns the size of the DIFF, not what landed: entries losing the D18 rename race are counted as pulled. No scientific impact (the mirror is re-walked for completion truth) but it is an intention reported as an observation, and it is the number the heartbeat carries. | n/a |

**★ SWEPT AND CLEAN, recorded so nobody re-derives them:** partially-written records are **never**
visible to readers (staging is `.pull_tmp.<pid>`, a dir moves only after its `record.json` verifies,
and every mirror reader excludes staging parts); `seed_pool_blocks` drops no spec; `run_search_arm`'s
collection `continue` drops no candidate (each was counted and ledgered earlier in the same
