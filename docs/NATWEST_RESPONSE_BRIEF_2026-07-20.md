...
| "Opus is expensive/slow" | The measured authoring bill for the whole campaign is ~$6–16 (~180 short calls); GPU training, not the LLM, is >99% of cost. We nevertheless adopted the discipline: **total LLM spend is hard-capped at $30, enforced in code**, with a pre-declared trimming order — the whole ~35-model study runs for thirty dollars. |
