...
> absolutely everything, please dont miss anything. Ultarthink evry deeply adn extenisvelly, take as
> much time as you need, as many tokens as you need, this campaign run is extremely important, and it
> must be absoliutely flawless across absolutely all dimensions possible. Ultrathink very depely and
...
> verify, and you always very precise. Please work very accuratelly, anbd very surgically, make sure
> you make no mistakes. Ultrahink 100000 tiems befor edoing anything"*

### §0.2 HIS FIRST PROMPT OF RUN 16 (verbatim — he asks that this be carried over every time)

> *"Some stuff crashed, Thats exactly why I have told claude code sessions to very deeply and strictly
> monitor everything constantly and ensure absolutely everything is strictly absolitely flalwess
> 10000000% Ultarthink very deeply and extensivelly. pelase abbsolutely always monitor absolutely
> everything in this campaign very depely and strictly."* — followed by the §0.1 block.

### §0.3 EVERY OTHER INSTRUCTION HE GAVE IN RUN 16 (verbatim, in order)

1. *"Please make sure you study every file in thsi project very deeply, all processes, the whole thing
   going on on myriad, absolutely everything, please dont miss anything."*
2. *"also additio, I might be wrong, but you need to speed up the ETA to an absolute minimum so it
   ...
   possible."*
3. *"also very deeply and strictly cehck fi this is correct and ensure fixed, also make sur eyou dive
...
4. *"Plesae ultrathink very deeply and extenisvelly, dive very deep, and bring teh eta to global
   minimum."*
5. *"why are we decreasing in cores"*
6. *"I give you full permissions, ultrathink, and act with accordance to the priorities, maek sure
...
python docs/ops/loginnode_guard.py --once        # are we near a UCL usage penalty
...
branch myriad-cluster-and-tier-system · backup-2026-08-03-run16 == HEAD, pushed
...
| **h3_singleshot** | **568** | **568** | 1 | **COMPLETE** |
| gpt-5.6-luna | 507 | 517 | 5 | about to be the third |
...
```
driver 24 · supervisor 10 · sentinel 5 · cycle_loop 3 · crash_watchdog 3 · loginnode_guard 3
myriad_watch 3 · line_balance 3 · allocation_advisor 2 · watchdog_fenced 1 · publish_loop 1
ssh_reaper 1 · campaign_backup 1
```
⚠ **`crash_watchdog`, `loginnode_guard`, `myriad_watch`, `line_balance` and `watchdog_fenced` are NOT
in `mode_d_launch.ps1`, so a reboot will NOT bring them back.** Relaunch by hand:
```bash
...
powershell -ExecutionPolicy Bypass -File docs\ops\watchdog_fenced.ps1 -IntervalSecs 300 -OutDir outputs\campaign_cluster_run4 -RemoteRoot ~/Scratch/llmrp4 -ExcludeHosts node-d00a-230,node-d00b-024
```
...
## §5 ★★★★★ THE ETA — ASKED FOUR TIMES, ANSWERED, CLOSED. **RE-READ, DO NOT RE-LITIGATE.**

Full derivation: execution record **§117, §120, §121, §122, §123**.

**THE REGISTRATION SETS THE TARGET (Amendment E1).** Rungs `[30,100,189,279,340,403,568]`, each with
a pre-registered meaning: **30** = the distinction-bankable core, *the CVaR-5 % co-primary leg is
ALREADY conclusive here*; **100** = σ-precision insurance; **189** = the Sharpe-leg TOST is decisive;
**279/340/403/568** = 80/90/**95 (THE PRIMARY TARGET)**/99 % equivalence assurance. And: *"a truncated
run falls back to the largest COMPLETED rung"*, and *"the STOPPING tier is determined **EXOGENOUSLY
by measured Myriad throughput**, never by inspecting results."* **⇒ choosing the rung from throughput
IS the registered procedure, and 568 comes for free.**

**THROUGHPUT IS FAIR-SHARE BOUND — proven from SGE itself, not inferred:**
```
qalter -w p <real pending leg job>  ->  "verification: found possible assignment with 8 slots"
qquota -u ucestes                   ->  EMPTY, no quota applies
...
weight_tickets_functional 500000000  vs  weight_tickets_share 10000 · weight_user 1.0 · 6+ users
```
**The jobs are assignable, the capacity exists, and we still do not get it.** That is functional
fair-share by user, and it is not ours to change.
...
| stopping at 403 | unnecessary — the cumulative-tier rule banks 568 for free |

**AND REMAINING WORK CARRIES NO WASTE:** the 8.8 % gate-failure rate counts **candidates rejected
before any training is submitted** (one LLM call, not a 15 h training) — checked at
...
## §8 HARNESS LIMITS MEASURED (test the specific command; do not assume)

```
Stop-Process -Id <pid> -Force        WORKS      (RUN 15 said BLOCKED; MEASURED — it is not)
New-Item -ItemType Junction          WORKS      (RUN 15 said BLOCKED; MEASURED — it is not)
...
## §9 ⚠ CLAIMS RUN 16 MEASURED AND FOUND FALSE
...
- **NEVER** trust a wrapper's exit code — a pipe makes `$?` the LAST command's; read `PYTEST_RC` from
  the LOG.
...
- **⚠ A `Win32_Process` filter MATCHES ITS OWN QUERY**, and a bash script is a
  parent→child→grandchild CHAIN. Use `session_preflight`; when a hand-rolled process count disagrees
  with it, **yours is wrong.**
...
2. **EVERY MISTAKE RECORDED, INCLUDING YOUR OWN** — root cause · how it was actually found · the fix ·
   ...
   quietly drop an error you made.** Okhrati rewards exactly this maturity.
3. **WRITE IT AS IT HAPPENS** — real commands, real output, while still verifiable.

### ★★★ AND THE VERIFICATION DUTY
**A green check proves execution, not truth.** Sanity-check magnitude/sign/units; **cross-check
...
1. **`qdel 66103 66104 66105 66106 66107 66108 73026 73027`** — PRICED, not hygiene: all eight sit at
   the TOP of our pending queue (2.00440 > every real leg job at 2.00430), all carry **`reserve: y`**
...
* **D31** — `scripts/mode_d_watchdog.ps1:88` is still absence-only (the P202 defect). ✅ The reboot
  hole is CLOSED: the boot task `LLMRewardCampaignResume` now launches
...
* **At the next natural relaunch of any line**, apply longer `--poll-secs 300 --search-poll-secs 120`
  — the complete fix for login-node load.

**RECORDED, NOT FIXED (execution record §124.6):** `check_processes` goes silent when psutil is
missing (the `line_census` row vanishes rather than saying it could not run) · `line_balance`'s
columns are labelled *rung* but hold **record counts** · `test_session_preflight.py` re-implements the
roster regex inline · the `REVIVE_<safe>` override is PowerShell-only · the COMPLETE corroboration
reads only the last 200 driver-log lines.
...
**PRIORITY 5 BREACH CLOSED:** `docs/analysis/record_validator.py` (the R1-R9 layer) and **16 sibling
instruments were UNTRACKED IN GIT** — the apparatus verifying an irreplaceable archive existed only
in this working tree. **7,297 lines committed**, secret-scanned first.

**THE SEVEN RECORD LAYERS — run all of them, every session (`bash docs/ops/run_record_layers.sh`):**
```
R1-R9   record_validator.py          the record's CONTRACT
P1-P4   record_provenance_seal.py    the record vs the FILES beside it
S1-S10  record_science_audit.py      scientific soundness + the BANKED RUNG   (wired into cycle.py, 30-min rate limit)
