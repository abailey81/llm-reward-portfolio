...
effectively unavailable** (quota blocked — see §6). Committed scope: **30 candidates × 5 seeds**
(decision AMEND-ORIG-1).
...
## 1. What "training everything" is — run-count accounting (30 × 5)
...
| **Core campaign** — 6 arms × 30 candidates × 5 seeds | **900** | the headline |
| Hand-designed baselines (4 reward canon × 5 seeds, H1) | 20 | |
...
| PPO/TD3 robustness on winners | ~20 | small |
| **→ SAC program subtotal** | **≈ 955 full-run-equivalents** | |
| *Optional* secondary SAC-vs-TQC critic experiment (2 headline arms × 30 × 5) | +300 | only if Phase-0 TQC is green |
| **→ Full program (incl. TQC experiment)** | **≈ 1,255** | |

**Efficiency lever — "seeds-on-winners":** train each candidate at **1 seed during the search**, and re-run
only the per-arm **winners at all 5 seeds**. Keeps the committed 30×5 design and matched-compute, but cuts
the core 900 → **~205 runs**. This is what makes the job cheap/fast and is the recommended protocol.
...
GPU-hours = `runs × m`. Lean = seeds-on-winners (~205 runs); Full = 5 seeds on every candidate (~955 runs).

| | Lean (~205 runs) | Full SAC (~955 runs) | Full + TQC (~1,255) |
|---|---|---|---|
| on a **4090** (m≈11) | **~38 GPU-hr** | ~175 GPU-hr | ~230 GPU-hr |
| on a **4050** (m≈18) | ~62 GPU-hr | ~285 GPU-hr | ~377 GPU-hr |
| on a **T4/P100** (m≈20) | ~68 GPU-hr | ~317 GPU-hr | ~418 GPU-hr |
...
| Option | GPU / free limit | Lean (~205) | Full 30×5 (~955) | Cost | Verdict |
|---|---|---|---|---|---|
| **Vast.ai — rent RTX 4090** | ~$0.29–0.40/hr (marketplace; interruptible ~$0.30) | **~1.5 days / ~$15** (1 GPU); ~2–3 h parallel | ~7 days / ~$68 | pay-as-you-go | ⭐ best value + speed, no quota |
| **RunPod — rent RTX 4090** | $0.34/hr community ($0.59 secure) | ~1.5 days / **~$13** | ~7 days / ~$60 | pay-as-you-go | ⭐ clean UX, same tier |
| Rent **RTX 3090** (Vast) | ~$0.20–0.25/hr | ~2 days / **~$8–11** | ~9 days / ~$45–50 | cheapest paid | ✅ if patient |
| **RTX 4050 laptop** (owned) | free | ~2.5 days | ~12 days | free | ✅ free; ties up laptop (heat/throttle) — best for prototype + dev |
...
| **Stacked free** (Kaggle+Colab+Lightning+laptop) | **~50–60 GPU-hr/wk** | **~1.5–2 weeks, $0** | ~5–6 weeks | free | ✅ zero-cost if you'll babysit |
...
2. **Campaign → rent an RTX 4090 on RunPod or Vast.ai, with seeds-on-winners.**
   **≈ $13–16 and ~1.5 days on one GPU** — or rent ~4–8 in parallel for **~2–3 hours** at the same total cost.
   No quota approval, a 4090 suits SAC, fully reproducible from the frozen config.
3. **Free alternative (if the ~$15 matters more than ~2 weeks):** stack **Kaggle (30 h/wk) + Lightning
   (~80 h/mo) + Colab + the laptop overnight** → the lean program in **~1.5–2 weeks, $0**, with checkpointing.
...
**Primary recommendation: rented RTX 4090 + seeds-on-winners (~$15, ~1.5 days).**
