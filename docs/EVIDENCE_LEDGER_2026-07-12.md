...
| 3 | F saturates ≈2.5 at pack-5 | **B→A pending** | curve F(1)=1.00/F(2)=1.30/F(3)=2.16/F(5)=2.48/F(8)=2.51 — 5 points, smooth shape, but ONE task per point | **p1pack5b queued (774333, different node)**; the curve's internal consistency (monotone, saturating) is itself corroborating |
...
| 8 | B\* = 200k is past the eval knee | **B+ (hand-written) / pending (authored)** | 25k–350k flat, 2 ladders, 3 seeds, ONE hand-written reward | **the running ladders**: laptop 100k–400k ×2 authored winners ×3 seeds (~today) + Myriad p6ext to 1.6M → A |
