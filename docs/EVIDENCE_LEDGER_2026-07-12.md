...
| 2 | Cross-node bitwise determinism | **A (UPGRADED 2026-07-14)** | TWO independent pairs, two seeds: seed 777 (e00a-007 vs e00a-017) AND seed 778 (p4detb t1/t2, completed 2026-07-14) — val_fitness identical to the last digit, all 694 val_returns byte-equal, same env sha, both pairs | — . NOTE: the cluster code sync (now unblocked) is DEFERRED until the B\* curve completes, so every curve row runs the SAME code version (no mixed-code confound in the curve comparison); sync lands between the verdict and the C0 canary |
...
| 16 | A100 (L-pool) pack behavior — F_A100 at pack {5, 8} | **pack-5 MEASURED (B, n=1): F_A100(5) ≈ F_V100(5)** | probe 785630 (node-l00a-002, exit 0): 5×50k steps in **1,810 s** vs the V100's 1,847 s — per-GPU parity; the workload is kernel-launch/CPU-bound at SAC's net sizes, so the A100's FLOP advantage doesn't bind. Pack-5 stays correct for BOTH pools; science-neutrality of the stripes comforted (similar wall ⇒ similar co-tenancy exposure) | **CLOSED 2026-07-14**: probe 785631 (pack-8, same node l00a-002, exit 0): 8×50k in **2,353 s** → net-of-overhead aggregate ≈268 st/s vs pack-5's ≈263 — the A100 saturates at the SAME ~265 st/s ceiling (mirrors the V100's F(8)≈F(5)). **Pack-5 is the setting on BOTH pools; no per-pool parameter needed** |
| 18 | Paired-contrast dispersion at 400k (directional): sd(D_s) grows ~1.9x vs 200k on the curve's authored winners | **C (directional proxy; n=3 pairs, val-DSR units, prototype-era winners)** | curve grid: D_s(400k) = [0.1209, 0.146, 0.4691] (sd 0.194) vs D_s(200k) = [0.08, 0.0696, 0.2477] (sd 0.100) — consistent with the R77 disclosure that dispersion fans out with budget | the E1 rung-100 in-campaign sigma_D re-estimate is the REAL number (designed-in); this row only corroborates the direction of the R77 power caveat |
| 17 | The serialization policy (snx=1) holds big-array tails at ~1 task/~2 h and has PURGED pending tails twice | **B+ (observed live, independent episodes)** | the 771972 crawl + the pm2 hqw tails in today's queue + the rehearsal-array and p6ext purges | the chunked-submission lever (`dc86322`; launch `--chunk-tasks 1`) makes the campaign structurally immune — validated by the C measurement in the first 48 h |
