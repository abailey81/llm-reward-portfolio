...
**Auto-generated 2026-08-11 07:42 UTC -- T+322h33m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+322h33m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **613** = 145 running + **114 ELIGIBLE** + 354 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **100.0%** -- 1152 of 1152 cores (10 min old) |
...
```
generated 2026-08-11 07:43 UTC | elapsed 13.44 d | 15.7 d to the Aug-27 stop
test tier: 28,440 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 114.9 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     444 records     148.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1379 records     114.9 rec/h   usable
    last 24 h    2432 records     101.3 rec/h   usable
    12 h rate is 38% from ONE line (test); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       1,024     140  2026-08-11 16:38  2026-08-11 16:38  yes
      340       3,159     140  2026-08-12 11:12  2026-08-12 11:12  yes
      403       5,364     140  2026-08-13 06:24  2026-08-13 06:24  yes
      568      11,888     140  2026-08-15 15:10  2026-08-15 15:10  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 5% of the rung-568 backlog (585 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test -- baseline_log_growth has 173 HOLE(S) below its frontier 406 -- that is what caps this line**
