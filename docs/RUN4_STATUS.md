...
**Auto-generated 2026-08-12 09:34 UTC -- T+348h25m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+348h25m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 15.9 h ago** |
...
| | |
|---|---|
| cluster jobs | **425** = 97 running + **0 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **776** |
| **cores doing RUNG-RAISING work** | **100.0%** -- 776 of 776 cores (1 min old) |
...
```
generated 2026-08-12 09:35 UTC | elapsed 14.52 d | 14.6 d to the Aug-27 stop
test tier: 30,771 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 66.3 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      84 records      84.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     233 records      77.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     796 records      66.3 rec/h   usable
    last 24 h    2116 records      88.2 rec/h   usable
    12 h rate is 25% from ONE line (test_leg_glm_5_2); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (66 rec/h); latest excludes cells already within
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,005      84  2026-08-13 00:44  2026-08-13 00:44  yes
      403       3,210      84  2026-08-14 09:58  2026-08-14 09:58  yes
      568       9,557      84  2026-08-18 09:39  2026-08-18 09:39  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @776 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           7.0 d           6.5 d   throughput
      279           9.9 d           9.3 d   throughput
      340          11.9 d          11.1 d   throughput
      403          14.0 d          13.0 d   throughput
      568          19.3 d          18.1 d   throughput
...
2026-08-12T09:29:04Z  OK  records=32262 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.4m  drift=0  sci=OK  r115=22B  sweep=670.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
