...
**Auto-generated 2026-08-05 05:59 UTC -- T+176h50m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+176h50m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on qwen3_6-27b, 57 min ago; none live, newest failure 57 min ago** |
...
| | |
|---|---|
| cluster jobs | **963** (210 running, 753 queued) |
...
```
generated 2026-08-05 06:00 UTC | elapsed 7.37 d | 21.7 d to the Aug-27 stop
test tier: 13,865 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     241 records     241.0 rec/h
    last  3 h     268 records      89.3 rec/h
    ...
    !! 49% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_qwen3_5_9b, test_leg_sonnet_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       3,238      66  GATED             GATED             barrier>=30
      189       7,513     241  GATED             GATED             barrier>=30
      279      12,013     241  GATED             GATED             barrier>=30
      340      15,063     241  GATED             GATED             barrier>=30
      403      18,213     241  GATED             GATED             barrier>=30
      568      26,463     241  GATED             GATED             barrier>=30
...
```
2026-08-05T05:28:24Z  OK  records=15215 (+10)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=389.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
