...
**Auto-generated 2026-08-05 06:42 UTC -- T+177h33m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+177h33m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on qwen3_6-27b, 1.7 h ago; none live, newest failure 1.7 h ago** |
...
| | |
|---|---|
| cluster jobs | **944** (206 running, 738 queued) |
...
```
generated 2026-08-05 06:43 UTC | elapsed 7.40 d | 21.7 d to the Aug-27 stop
test tier: 14,007 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     231 records     231.0 rec/h
    last  3 h     403 records     134.3 rec/h
    last 12 h    1653 records     137.8 rec/h
    last 24 h    4039 records     168.3 rec/h
    12 h rate is 39% from ONE line (test_leg_haiku_4_5); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 42% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_qwen3_5_9b, test_leg_sonnet_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       3,238       0  GATED             GATED             barrier>=30
      189       7,371     231  GATED             GATED             barrier>=30
      279      11,871     231  GATED             GATED             barrier>=30
      340      14,921     231  GATED             GATED             barrier>=30
      403      18,071     231  GATED             GATED             barrier>=30
      568      26,321     231  GATED             GATED             barrier>=30
...
2026-08-05T06:34:01Z  OK  records=15514 (+21)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  sweep=229.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T06:43:10Z  OK  records=15542 (+28)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  sweep=518.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
