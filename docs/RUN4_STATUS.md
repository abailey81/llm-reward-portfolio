...
**Auto-generated 2026-08-12 20:19 UTC -- T+359h10m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+359h10m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=2; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 26.7 h ago** |
...
| | |
|---|---|
| cluster jobs | **325** = 25 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **200** |
| **cores doing RUNG-RAISING work** | **42.3%** -- 88 of 208 cores (38 min old) |
...
```
generated 2026-08-12 20:20 UTC | elapsed 14.97 d | 14.2 d to the Aug-27 stop
test tier: 31,530 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 72.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      24 records      24.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h      44 records      14.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     865 records      72.1 rec/h   usable
    last 24 h    1630 records      67.9 rec/h   usable
    12 h rate is 95% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (72 rec/h); latest excludes cells already within
    8 of the ceiling (72 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         430      24  2026-08-13 02:18  2026-08-13 02:18  yes
      403       2,451      24  2026-08-14 06:20  2026-08-14 06:20  yes
      568       8,798      24  2026-08-17 22:23  2026-08-17 22:23  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @200 cores      @830 cores   binding
       30           7.0 d           4.6 d   critical_chain
      100          15.8 d           4.6 d   critical_chain
      189          27.1 d           6.5 d   throughput
      279          38.5 d           9.3 d   throughput
      340          46.2 d          11.1 d   throughput
      403          54.2 d          13.0 d   throughput
      568          75.0 d          18.1 d   throughput
...
> **What is holding it right now: test_leg_deepseek_v4_pro -- scalar_cvar5 has 26 HOLE(S) below its frontier 339 -- that is what caps this line**
...
|---|---|---|---|---|
| nemotron_3_super | **307** | 308 | 5 |  |
| glm_5_2 | **311** | 312 | 5 |  |
| deepseek_v4_pro | **314** | 317 | 5 |  |
| test | **350** | 350 | 9 |  |
| kimi_k3 | **471** | 474 | 5 |  |
...
## Monitoring -- the cycle (**last monitoring cycle was 16 min ago -- the loop has lapsed**)
...
```
2026-08-12T19:57:05Z  OK  records=33063 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=65.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T19:58:33Z  OK  records=33064 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  sweep=57.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T20:00:01Z  OK  records=33064 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.8m  drift=0  sci=OK  r115=22B  sweep=58.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T20:01:27Z  OK  records=33064 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.2m  drift=0  sci=OK  r115=22B  sweep=56.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T20:02:55Z  OK  records=33066 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=57.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T20:04:21Z  OK  records=33066 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  sweep=55.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
