...
**Auto-generated 2026-08-11 19:01 UTC -- T+333h52m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+333h52m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 81 min ago; none live, newest failure 81 min ago** |
...
| **cores computing** | **504** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (88 min old) |
...
```
generated 2026-08-11 19:02 UTC | elapsed 13.91 d | 15.2 d to the Aug-27 stop
test tier: 29,794 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 119.2 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     280 records      93.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1431 records     119.2 rec/h   usable
    last 24 h    2777 records     115.7 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,805      65  GATED             GATED             barrier>=279
      403       4,010      65  GATED             GATED             barrier>=279
      568      10,534      65  GATED             GATED             barrier>=279
