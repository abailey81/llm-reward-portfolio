...
**Auto-generated 2026-08-11 11:21 UTC -- T+326h12m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+326h12m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on kimi-k3, 4 min ago; LIVE, still failing** |
...
| | |
|---|---|
| cluster jobs | **562** = 131 running + **103 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **87.4%** -- 1000 of 1144 cores (109 min old **STALE**) |
...
```
generated 2026-08-11 11:22 UTC | elapsed 13.59 d | 15.5 d to the Aug-27 stop
test tier: 28,834 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 111.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     100 records     100.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    ...
    last 12 h    1340 records     111.7 rec/h   usable
    last 24 h    2359 records      98.3 rec/h   usable
    12 h rate is 68% from ONE line (test); 3 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         630     100  2026-08-11 17:00  2026-08-11 17:00  yes
      340       2,765     100  2026-08-12 12:08  2026-08-12 12:08  yes
      403       4,970     100  2026-08-13 07:52  2026-08-13 07:52  yes
      568      11,494     100  2026-08-15 18:18  2026-08-15 18:18  yes
...
> **What is holding it right now: test -- baseline_log_growth has 153 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **254 seeds each** (read live from the archive on this publish). !! THE
...
2026-08-11T10:50:21Z  OK  records=30332 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  sweep=61.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T10:51:45Z  OK  records=30336 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.4m  drift=0  sci=OK  r115=22B  sweep=53.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
