...
**Auto-generated 2026-08-11 07:31 UTC -- T+322h22m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+322h22m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **616** = 142 running + **120 ELIGIBLE** + 354 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **100.0%** -- 1096 of 1096 cores (118 min old **STALE**) |
...
```
generated 2026-08-11 07:31 UTC | elapsed 13.43 d | 15.7 d to the Aug-27 stop
test tier: 28,420 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 114.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     151 records     151.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     464 records     154.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1376 records     114.7 rec/h   usable
    last 24 h    2426 records     101.1 rec/h   usable
    12 h rate is 36% from ONE line (test); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       1,044     151  2026-08-11 16:38  2026-08-11 16:38  yes
      340       3,179     151  2026-08-12 11:15  2026-08-12 11:15  yes
      403       5,384     151  2026-08-13 06:28  2026-08-13 06:28  yes
      568      11,908     151  2026-08-15 15:22  2026-08-15 15:22  yes
...
> **What is holding it right now: test -- baseline_return_minus_downside has 175 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **232 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T07:04:36Z  OK  records=29905 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  sweep=53.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
