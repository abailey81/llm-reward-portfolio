...
**Auto-generated 2026-08-06 15:14 UTC -- T+210h05m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+210h05m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=156; worst streak 2/240 (0.8% to fatal), ops on core, 41 min ago; none live, newest failure 40 min ago** |
...
test tier: 17,970 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 74.2 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     254 records     254.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30          66      31  GATED             GATED             unstarted:2-unit(s)-absent
      100       2,516      31  GATED             GATED             unstarted:2-unit(s)-absent>=30
      189       5,631      32  GATED             GATED             unstarted:2-unit(s)-absent>=30
      279       9,345     254  GATED             GATED             unstarted:2-unit(s)-absent>=30
      340      12,090     254  GATED             GATED             unstarted:2-unit(s)-absent>=30
      403      14,925     254  GATED             GATED             unstarted:2-unit(s)-absent>=30
      568      22,358     254  GATED             GATED             unstarted:2-unit(s)-absent>=30
