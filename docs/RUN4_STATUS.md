...
**Auto-generated 2026-08-06 16:02 UTC -- T+210h53m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+210h53m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=156; worst streak 2/240 (0.8% to fatal), ops on core, 89 min ago; none live, newest failure 89 min ago** |
...
| **cores computing** | **376** |
| **cores doing RUNG-RAISING work** | **17.0%** -- 72 of 424 cores (23 min old) |
...
test tier: 18,122 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 85.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     232 records     232.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       9,193     232  GATED             GATED             unstarted:2-unit(s)-absent>=30
      340      11,938     232  GATED             GATED             unstarted:2-unit(s)-absent>=30
      403      14,773     232  GATED             GATED             unstarted:2-unit(s)-absent>=30
      568      22,206     232  GATED             GATED             unstarted:2-unit(s)-absent>=30
