...
**Auto-generated 2026-08-11 13:11 UTC -- T+328h03m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+328h03m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 38 min ago; none live, newest failure 37 min ago** |
...
| | |
|---|---|
| cluster jobs | **537** = 124 running + **85 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **992** |
| **cores doing RUNG-RAISING work** | **77.9%** -- 816 of 1048 cores (99 min old **STALE**) |
...
```
generated 2026-08-11 13:12 UTC | elapsed 13.67 d | 15.4 d to the Aug-27 stop
test tier: 29,038 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 115.3 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1384 records     115.3 rec/h   usable
    last 24 h    2400 records     100.0 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         426     108  2026-08-11 16:54  2026-08-11 16:54  yes
      340       2,561     108  2026-08-12 11:25  2026-08-12 11:25  yes
      403       4,766     108  2026-08-13 06:32  2026-08-13 06:32  yes
      568      11,290     108  2026-08-15 15:06  2026-08-15 15:06  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 28% of the rung-568 backlog (3,152 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @992 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           5.5 d           6.5 d   throughput
      279           7.8 d           9.3 d   throughput
      340           9.3 d          11.1 d   throughput
      403          10.9 d          13.0 d   throughput
      568          15.1 d          18.1 d   throughput
...
> **What is holding it right now: test -- baseline_return_minus_turnover has 144 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **263 seeds each** (read live from the archive on this publish). !! THE
...
2026-08-11T13:06:33Z  OK  records=30567 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.3m  drift=0  sci=OK  r115=22B  sweep=49.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T13:08:06Z  OK  records=30571 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  cores=1000  sweep=62.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
