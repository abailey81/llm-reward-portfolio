...
**Auto-generated 2026-08-12 17:58 UTC -- T+356h49m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+356h49m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=2; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 24.3 h ago** |
...
| | |
|---|---|
| cluster jobs | **327** = 27 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **216** |
| **cores doing RUNG-RAISING work** | **46.4%** -- 104 of 224 cores (26 min old) |
...
```
generated 2026-08-12 17:59 UTC | elapsed 14.87 d | 14.3 d to the Aug-27 stop
test tier: 31,499 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 86.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     107 records      35.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1041 records      86.8 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         461      11  2026-08-12 23:18  2026-08-12 23:18  yes
      403       2,482      16  2026-08-13 22:35  2026-08-13 22:35  yes
      568       8,829      16  2026-08-16 23:45  2026-08-16 23:45  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @216 cores      @830 cores   binding
       30           6.5 d           4.6 d   critical_chain
      100          14.7 d           4.6 d   critical_chain
      189          25.1 d           6.5 d   throughput
      279          35.6 d           9.3 d   throughput
      340          42.8 d          11.1 d   throughput
      403          50.1 d          13.0 d   throughput
      568          69.5 d          18.1 d   throughput
