...
**Auto-generated 2026-08-12 02:19 UTC -- T+341h10m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+341h10m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **501** = 103 running + **70 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **824** |
| **cores doing RUNG-RAISING work** | **56.7%** -- 440 of 776 cores (46 min old) |
...
```
generated 2026-08-12 02:20 UTC | elapsed 14.22 d | 14.9 d to the Aug-27 stop
test tier: 30,166 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    12 h rate is 67% from ONE line (test); 3 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,433      51  GATED             GATED             barrier>=279
      403       3,638      51  GATED             GATED             barrier>=279
      568      10,162      51  GATED             GATED             barrier>=279
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @824 cores      @830 cores   binding
      ...
      279           9.3 d           9.3 d   throughput
      340          11.2 d          11.1 d   throughput
      403          13.1 d          13.0 d   throughput
      568          18.2 d          18.1 d   throughput
...
2026-08-12T02:14:49Z  OK  records=31697 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.4m  drift=0  sci=OK  r115=22B  sweep=615.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
