...
**Auto-generated 2026-08-06 04:07 UTC -- T+198h58m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+198h58m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on nemotron-3-super, 4.1 h ago; none live, newest failure 4.1 h ago** |
...
| | |
|---|---|
| cluster jobs | **961** (68 running, 893 queued) |
| **cores computing** | **544** |
...
```
generated 2026-08-06 04:07 UTC | elapsed 8.29 d | 20.8 d to the Aug-27 stop
test tier: 17,097 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h      18 records      18.0 rec/h
    last  3 h      97 records      32.3 rec/h
    last 12 h    1475 records     122.9 rec/h
    ...
    !! 54% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_haiku_4_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       2,606      18  GATED             GATED             barrier>=30
      189       6,170      18  GATED             GATED             barrier>=30
      279      10,218      18  GATED             GATED             barrier>=30
      340      12,963      18  GATED             GATED             barrier>=30
      403      15,798      18  GATED             GATED             barrier>=30
      568      23,231      18  GATED             GATED             barrier>=30
...
2026-08-06T04:07:22Z  OK  records=18639 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.3m  drift=0  sci=OK  r115=22B  sweep=246.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
