...
**Auto-generated 2026-08-06 15:18 UTC -- T+210h09m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+210h09m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=156; worst streak 2/240 (0.8% to fatal), ops on core, 45 min ago; none live, newest failure 44 min ago** |
...
| | |
|---|---|
| cluster jobs | **862** (68 running, 794 queued) |
| **cores computing** | **544** |
| **cores doing RUNG-RAISING work** | **22.6%** -- 184 of 816 cores (154 min old **STALE**) |
...
```
generated 2026-08-06 15:18 UTC | elapsed 8.76 d | 20.4 d to the Aug-27 stop
test tier: 17,992 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last  3 h     374 records     124.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30          64      25  GATED             GATED             unstarted:2-unit(s)-absent
      100       2,514      25  GATED             GATED             unstarted:2-unit(s)-absent>=30
      189       5,629      26  GATED             GATED             unstarted:2-unit(s)-absent>=30
      279       9,323     261  GATED             GATED             unstarted:2-unit(s)-absent>=30
      340      12,068     261  GATED             GATED             unstarted:2-unit(s)-absent>=30
      403      14,903     261  GATED             GATED             unstarted:2-unit(s)-absent>=30
      568      22,336     261  GATED             GATED             unstarted:2-unit(s)-absent>=30
...
```
2026-08-06T15:03:18Z  OK  records=19429 (+29)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=268.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
