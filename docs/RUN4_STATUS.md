...
**Auto-generated 2026-08-11 20:13 UTC -- T+335h04m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+335h04m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 2.6 h ago; none live, newest failure 2.6 h ago** |
...
| | |
|---|---|
| cluster jobs | **431** = 59 running + **44 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **472** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (41 min old) |
...
```
generated 2026-08-11 20:14 UTC | elapsed 13.96 d | 15.2 d to the Aug-27 stop
test tier: 29,889 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 115.3 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      94 records      94.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     241 records      80.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1384 records     115.3 rec/h   usable
    last 24 h    2795 records     116.5 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,710      94  GATED             GATED             barrier>=279
      403       3,915      94  GATED             GATED             barrier>=279
      568      10,439      94  GATED             GATED             barrier>=279
...
currently at **299 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T19:36:04Z  OK  records=31370 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  sweep=53.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T19:37:27Z  OK  records=31373 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.8m  drift=0  sci=OK  r115=22B  sweep=52.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T19:38:49Z  OK  records=31376 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.9m  drift=0  sci=OK  r115=22B  sweep=52.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T19:40:12Z  OK  records=31376 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=52.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T19:49:15Z  OK  records=31388 (+12)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=513.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T20:02:47Z  OK  records=31389 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.2m  drift=0  sci=OK  r115=22B  sweep=781.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
