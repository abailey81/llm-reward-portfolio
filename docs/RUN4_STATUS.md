...
**Auto-generated 2026-08-12 02:53 UTC -- T+341h45m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+341h45m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 9.2 h ago** |
...
| | |
|---|---|
| cluster jobs | **498** = 107 running + **63 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **856** |
| **cores doing RUNG-RAISING work** | **56.7%** -- 440 of 776 cores (81 min old) |
...
```
generated 2026-08-12 02:54 UTC | elapsed 14.24 d | 14.9 d to the Aug-27 stop
test tier: 30,216 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,387      61  GATED             GATED             barrier>=279
      403       3,592      61  GATED             GATED             barrier>=279
      568      10,112      65  GATED             GATED             barrier>=279
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 21% of the rung-568 backlog (2,157 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
