...
test tier: 17,473 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      189       5,794     105  GATED             GATED             barrier>=30
      279       9,842     105  GATED             GATED             barrier>=30
      340      12,587     105  GATED             GATED             barrier>=30
      403      15,422     105  GATED             GATED             barrier>=30
      568      22,855     105  GATED             GATED             barrier>=30
