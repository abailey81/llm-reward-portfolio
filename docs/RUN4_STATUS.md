...
**Auto-generated 2026-08-04 22:56 UTC -- T+169h47m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+169h47m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 27 min ago; none live, newest failure 27 min ago** |
...
| | |
|---|---|
| cluster jobs | **991** (159 running, 830 queued) |
| **cores computing** | **1272** |
...
```
generated 2026-08-04 22:57 UTC | elapsed 7.08 d | 22.0 d to the Aug-27 stop
test tier: 13,164 records over 68 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     146 records     146.0 rec/h
    last  3 h     558 records     186.0 rec/h
    last 12 h    2215 records     184.6 rec/h
    last 24 h    4297 records     179.0 rec/h
    12 h rate is 66% from ONE line (test_leg_sonnet_5); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 66% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_sonnet_5, test_leg_gpt_5_6_luna) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (185 rec/h); latest excludes cells already within
    8 of the ceiling (63 rec/h). Window 12 h.
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30         240       4  GATED             GATED             barrier
      100       3,341      13  GATED             GATED             barrier>=30
      189       7,791      13  GATED             GATED             barrier>=30
      279      12,291      13  GATED             GATED             barrier>=30
      340      15,341      13  GATED             GATED             barrier>=30
      403      18,491      13  GATED             GATED             barrier>=30
      568      27,164     146  GATED             GATED             barrier>=30
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    (+3 registered unit(s) have no directory yet; each owes a FULL rung and is counted in 'remaining' above)
    !! those 3 unit(s) are NOT in the rate's denominator and CANNOT be -- they have produced nothing at all, so no window contains them.
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1272 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           4.6 d           6.5 d   throughput
      279           6.0 d           9.3 d   throughput
      340           7.3 d          11.1 d   throughput
      403           8.5 d          13.0 d   throughput
      568          11.8 d          18.1 d   throughput

    saturation: more than ~3235 cores buy NOTHING at rung 568
    critical-chain floor: 4.64 d total, 0.19 d still to run   (tpe owes 1 of 30 candidates)
...
|---|---|---|---|---|
| test | **0** | 30 | 8 | 3 arm(s) still at zero |
...
```
2026-08-04T22:29:52Z  ATTN  records=14604 (+11)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  cores=1232  sweep=443.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-04T22:38:14Z  ATTN  records=14628 (+24)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  sweep=472.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-04T22:42:13Z  ATTN  records=14649 (+21)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.1m  drift=0  sci=OK  r115=22B  sweep=208.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-04T22:48:19Z  ATTN  records=14670 (+21)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=334.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-04T22:52:28Z  OK  records=14682 (+12)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  cores=1256  sweep=218.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-04T22:56:39Z  OK  records=14698 (+16)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  sweep=220.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
