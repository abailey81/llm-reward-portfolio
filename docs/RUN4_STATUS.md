...
**Auto-generated 2026-08-11 21:30 UTC -- T+336h22m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| cluster jobs | **418** = 67 running + **23 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **536** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (118 min old **STALE**) |
...
```
generated 2026-08-11 21:31 UTC | elapsed 14.02 d | 15.1 d to the Aug-27 stop
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last 24 h    2719 records     113.3 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,633      50  GATED             GATED             barrier>=279
      403       3,838      50  GATED             GATED             barrier>=279
      568      10,362      50  GATED             GATED             barrier>=279
