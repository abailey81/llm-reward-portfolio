...
**Auto-generated 2026-08-12 15:38 UTC -- T+354h29m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+354h29m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **336** = 36 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **288** |
| **cores doing RUNG-RAISING work** | **58.3%** -- 168 of 288 cores (5 min old) |
...
```
generated 2026-08-12 15:38 UTC | elapsed 14.77 d | 14.3 d to the Aug-27 stop
test tier: 31,446 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 97.2 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      99 records      99.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     354 records     118.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1167 records      97.2 rec/h   usable
    last 24 h    2010 records      83.8 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         472       0  2026-08-12 20:30  2026-08-12 20:30  yes
      403       2,535      99  2026-08-13 17:42  2026-08-13 17:42  yes
      568       8,882      99  2026-08-16 10:58  2026-08-16 10:58  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @288 cores      @830 cores   binding
       30           4.9 d           4.6 d   critical_chain
      100          11.0 d           4.6 d   critical_chain
      189          18.8 d           6.5 d   throughput
      279          26.7 d           9.3 d   throughput
      340          32.1 d          11.1 d   throughput
      403          37.6 d          13.0 d   throughput
      568          52.1 d          18.1 d   throughput
...
currently at **345 seeds each** (read live from the archive on this publish). !! THE
