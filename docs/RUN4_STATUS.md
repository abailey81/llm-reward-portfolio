...
**Auto-generated 2026-08-13 01:31 UTC -- T+364h23m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+364h23m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 31.9 h ago** |
...
| | |
|---|---|
| cluster jobs | **319** = 51 running + **202 ELIGIBLE** + 66 held by us + 0 held only by the site |
...
| **cores computing** | **408** |
| **cores doing RUNG-RAISING work** | **43.8%** -- 112 of 256 cores (119 min old **STALE**) |
...
```
generated 2026-08-13 01:32 UTC | elapsed 15.18 d | 13.9 d to the Aug-27 stop
test tier: 31,536 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 27.0 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h     324 records      27.0 rec/h   usable
    last 24 h    1411 records      58.8 rec/h   usable
    12 h rate is 85% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (27 rec/h); latest excludes cells already within
    8 of the ceiling (27 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         424       0  2026-08-13 17:15  2026-08-13 17:15  yes
      403       2,445       0  2026-08-16 20:06  2026-08-16 20:06  yes
      568       8,792       0  2026-08-26 15:10  2026-08-26 15:10  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @408 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           7.8 d           4.6 d   critical_chain
      189          13.3 d           6.5 d   throughput
      279          18.9 d           9.3 d   throughput
      340          22.6 d          11.1 d   throughput
      403          26.5 d          13.0 d   throughput
      568          36.8 d          18.1 d   throughput
...
2026-08-13T01:17:55Z  ATTN  records=33079 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  cores=392  sweep=899.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
