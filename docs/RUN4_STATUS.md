...
**Auto-generated 2026-08-05 05:49 UTC -- T+176h40m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+176h40m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on qwen3_6-27b, 47 min ago; none live, newest failure 47 min ago** |
...
| | |
|---|---|
| cluster jobs | **974** (207 running, 767 queued) |
...
```
generated 2026-08-05 05:50 UTC | elapsed 7.36 d | 21.8 d to the Aug-27 stop
test tier: 13,810 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     193 records     193.0 rec/h
    last  3 h     214 records      71.3 rec/h
    last 12 h    1636 records     136.3 rec/h
    ...
    !! 52% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_qwen3_5_9b, test_leg_sonnet_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       3,238      73  GATED             GATED             barrier>=30
      189       7,568     193  GATED             GATED             barrier>=30
      279      12,068     193  GATED             GATED             barrier>=30
      340      15,118     193  GATED             GATED             barrier>=30
      403      18,268     193  GATED             GATED             barrier>=30
      568      26,518     193  GATED             GATED             barrier>=30
...
```
2026-08-05T05:16:05Z  OK  records=15190 (+9)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.5m  drift=0  sci=OK  r115=22B  sweep=337.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
