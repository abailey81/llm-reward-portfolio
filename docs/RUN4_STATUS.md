...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 88 min ago; none live, newest failure 88 min ago** |
...
| **cores computing** | **504** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (95 min old **STALE**) |
...
```
generated 2026-08-11 19:08 UTC | elapsed 13.92 d | 15.2 d to the Aug-27 stop
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,805      56  GATED             GATED             barrier>=279
      403       4,010      56  GATED             GATED             barrier>=279
      568      10,534      56  GATED             GATED             barrier>=279
