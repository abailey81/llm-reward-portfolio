...
**Auto-generated 2026-08-11 20:29 UTC -- T+335h20m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+335h20m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **427** = 59 running + **40 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **472** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (57 min old) |
...
```
generated 2026-08-11 20:30 UTC | elapsed 13.97 d | 15.1 d to the Aug-27 stop
test tier: 29,910 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,689      88  GATED             GATED             barrier>=279
      403       3,894      88  GATED             GATED             barrier>=279
      568      10,418      88  GATED             GATED             barrier>=279
...
currently at **301 seeds each** (read live from the archive on this publish). !! THE
