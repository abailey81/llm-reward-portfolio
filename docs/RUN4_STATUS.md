...
**Auto-generated 2026-08-12 13:03 UTC -- T+351h54m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+351h54m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 19.4 h ago** |
...
| | |
|---|---|
| cluster jobs | **377** = 77 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **616** |
| **cores doing RUNG-RAISING work** | **100.0%** -- 600 of 600 cores (90 min old **STALE**) |
...
```
generated 2026-08-12 13:04 UTC | elapsed 14.66 d | 14.5 d to the Aug-27 stop
test tier: 31,141 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 86.4 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1037 records      86.4 rec/h   usable
    last 24 h    2120 records      88.3 rec/h   usable
    12 h rate is 50% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    8 of the ceiling (86 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         635     109  2026-08-12 20:24  2026-08-12 20:24  yes
      403       2,840     109  2026-08-13 21:55  2026-08-13 21:55  yes
      568       9,187     109  2026-08-16 23:22  2026-08-16 23:22  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @616 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           5.1 d           4.6 d   critical_chain
      189           8.8 d           6.5 d   throughput
      279          12.5 d           9.3 d   throughput
      340          15.0 d          11.1 d   throughput
      403          17.6 d          13.0 d   throughput
      568          24.4 d          18.1 d   throughput
...
currently at **328 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-12T12:49:43Z  OK  records=32646 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.5m  drift=0  sci=OK  r115=22B  sweep=58.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
