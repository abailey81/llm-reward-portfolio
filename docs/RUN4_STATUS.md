...
**Auto-generated 2026-08-12 01:59 UTC -- T+340h50m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+340h50m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **503** = 99 running + **76 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **792** |
| **cores doing RUNG-RAISING work** | **56.7%** -- 440 of 776 cores (27 min old) |
...
```
generated 2026-08-12 02:00 UTC | elapsed 14.20 d | 14.9 d to the Aug-27 stop
test tier: 30,153 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,446      51  GATED             GATED             barrier>=279
      403       3,651      51  GATED             GATED             barrier>=279
      568      10,175      51  GATED             GATED             barrier>=279
...
```
2026-08-12T01:42:25Z  OK  records=31678 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.6m  drift=0  sci=OK  r115=22B  sweep=58.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
