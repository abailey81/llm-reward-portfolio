...
**Auto-generated 2026-08-05 06:29 UTC -- T+177h20m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+177h20m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| stalest driver log | **4 min (nemotron-3-super)** old (P218: the STALEST of the still-running lines, completed ladders excluded; above ~30 means that line has stopped progressing) |
| records archived | **15504** |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on qwen3_6-27b, 87 min ago; none live, newest failure 87 min ago** |
...
| | |
|---|---|
| cluster jobs | **950** (206 running, 744 queued) |
| **cores computing** | **1648** |
...
```
generated 2026-08-05 06:29 UTC | elapsed 7.39 d | 21.7 d to the Aug-27 stop
test tier: 13,961 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     256 records     256.0 rec/h
    last  3 h     358 records     119.3 rec/h
    last 12 h    1648 records     137.3 rec/h
    last 24 h    4012 records     167.2 rec/h
    12 h rate is 37% from ONE line (test_leg_qwen3_5_9b); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 44% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_qwen3_5_9b, test_leg_sonnet_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       3,238       3  GATED             GATED             barrier>=30
      189       7,417     256  GATED             GATED             barrier>=30
      279      11,917     256  GATED             GATED             barrier>=30
      340      14,967     256  GATED             GATED             barrier>=30
      403      18,117     256  GATED             GATED             barrier>=30
      568      26,367     256  GATED             GATED             barrier>=30
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1648 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           4.6 d           6.5 d   throughput
      279           4.7 d           9.3 d   throughput
      340           5.6 d          11.1 d   throughput
      403           6.6 d          13.0 d   throughput
      568           9.1 d          18.1 d   throughput
...
2026-08-05T06:29:42Z  OK  records=15493 (+18)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.2m  drift=0  sci=OK  r115=22B  sweep=443.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
