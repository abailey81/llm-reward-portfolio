...
**Auto-generated 2026-08-11 20:59 UTC -- T+335h50m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+335h50m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 3.3 h ago; none live, newest failure 3.3 h ago** |
...
| | |
|---|---|
| cluster jobs | **422** = 68 running + **26 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **544** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (86 min old) |
...
```
generated 2026-08-11 20:59 UTC | elapsed 13.99 d | 15.1 d to the Aug-27 stop
test tier: 29,944 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 111.4 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     218 records      72.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1337 records     111.4 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,655      80  GATED             GATED             barrier>=279
      403       3,860      80  GATED             GATED             barrier>=279
      568      10,384      80  GATED             GATED             barrier>=279
...
```
2026-08-11T20:44:54Z  OK  records=31463 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.0m  drift=0  sci=OK  r115=22B  sweep=509.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T20:53:24Z  OK  records=31483 (+20)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  sweep=480.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T20:54:52Z  OK  records=31483 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.3m  drift=0  sci=OK  r115=22B  sweep=57.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
