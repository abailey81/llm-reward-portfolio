...
**Auto-generated 2026-08-11 18:34 UTC -- T+333h25m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+333h25m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on core, 6.0 h ago; none live, newest failure 54 min ago** |
...
| | |
|---|---|
| cluster jobs | **444** = 64 running + **52 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **512** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (61 min old) |
...
```
generated 2026-08-11 18:34 UTC | elapsed 13.89 d | 15.2 d to the Aug-27 stop
test tier: 29,781 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,818     106  GATED             GATED             barrier>=279
      403       4,023     106  GATED             GATED             barrier>=279
      568      10,547     106  GATED             GATED             barrier>=279
...
> **What is holding it right now: test -- bayes_opt has 112 HOLE(S) below its frontier 406 -- that is what caps this line**
