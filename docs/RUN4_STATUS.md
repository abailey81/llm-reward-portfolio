...
**Auto-generated 2026-08-11 21:54 UTC -- T+336h45m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+336h45m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **415** = 78 running + **9 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **624** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (142 min old **STALE**) |
...
```
generated 2026-08-11 21:55 UTC | elapsed 14.03 d | 15.1 d to the Aug-27 stop
test tier: 30,004 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 108.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1304 records     108.7 rec/h   usable
    last 24 h    2687 records     112.0 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,595      61  GATED             GATED             barrier>=279
      403       3,800      61  GATED             GATED             barrier>=279
      568      10,324      61  GATED             GATED             barrier>=279
...
> **What is holding it right now: test_leg_deepseek_v4_pro -- distributional has 31 HOLE(S) below its frontier 338 -- that is what caps this line**
...
currently at **304 seeds each** (read live from the archive on this publish). !! THE
