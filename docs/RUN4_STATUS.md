...
**Auto-generated 2026-08-11 12:14 UTC -- T+327h05m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+327h05m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **30483** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 0.8 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
| LLM calls / spend | 2956 / **$45.5021** |
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on kimi-k3, 57 min ago; none live, newest failure 57 min ago** |
...
| | |
|---|---|
| cluster jobs | **550** = 134 running + **88 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **77.9%** -- 816 of 1048 cores (42 min old) |
...
```
generated 2026-08-11 12:15 UTC | elapsed 13.63 d | 15.5 d to the Aug-27 stop
test tier: 28,940 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 114.6 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     304 records     101.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1375 records     114.6 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (115 rec/h); latest excludes cells already within
    8 of the ceiling (115 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         524     109  2026-08-11 16:49  2026-08-11 16:49  yes
      340       2,659     109  2026-08-12 11:27  2026-08-12 11:27  yes
      403       4,864     109  2026-08-13 06:42  2026-08-13 06:42  yes
      568      11,388     109  2026-08-15 15:38  2026-08-15 15:38  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 20% of the rung-568 backlog (2,266 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test -- baseline_return_minus_turnover has 149 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **258 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T11:40:11Z  ATTN  records=30405 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.4m  drift=0  sci=OK  r115=22B  sweep=58.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T11:41:36Z  ATTN  records=30411 (+6)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.4m  drift=0  sci=OK  r115=22B  sweep=54.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T11:43:07Z  ATTN  records=30411 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  cores=1032  sweep=60.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T11:44:30Z  ATTN  records=30415 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=53.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T11:53:27Z  ATTN  records=30437 (+22)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=506.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T12:06:48Z  OK  records=30438 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.5m  drift=0  sci=OK  r115=22B  sweep=770.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
