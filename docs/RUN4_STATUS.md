...
**Auto-generated 2026-08-11 23:31 UTC -- T+338h22m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+338h22m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 5.8 h ago; none live, newest failure 5.8 h ago** |
...
| | |
|---|---|
| cluster jobs | **505** = 82 running + **95 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **656** |
| **cores doing RUNG-RAISING work** | **36.2%** -- 232 of 640 cores (53 min old) |
...
```
generated 2026-08-11 23:31 UTC | elapsed 14.10 d | 15.0 d to the Aug-27 stop
test tier: 30,050 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 100.0 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     134 records      44.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1200 records     100.0 rec/h   usable
    last 24 h    2543 records     106.0 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         119       1  2026-08-12 00:43  2026-08-12 00:43  yes
      340       1,549      12  2026-08-12 15:01  2026-08-12 15:01  yes
      403       3,754      12  2026-08-13 13:04  2026-08-13 13:04  yes
      568      10,278      12  2026-08-16 06:18  2026-08-16 06:18  yes
...
```
2026-08-11T23:19:00Z  OK  records=31592 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=0.8m  drift=0  sci=OK  r115=22B  sweep=51.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T23:20:22Z  OK  records=31592 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  sweep=52.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
