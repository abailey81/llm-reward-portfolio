...
**Auto-generated 2026-08-06 08:58 UTC -- T+203h49m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+203h49m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| **cores computing** | **992** |
| **cores doing RUNG-RAISING work** | **20.8%** -- 208 of 1000 cores (7 min old) |
...
```
generated 2026-08-06 08:59 UTC | elapsed 8.49 d | 20.6 d to the Aug-27 stop
test tier: 17,399 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    !! 6% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_haiku_4_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      189       5,868      36  GATED             GATED             barrier>=30
      279       9,916      36  GATED             GATED             barrier>=30
      340      12,661      36  GATED             GATED             barrier>=30
      403      15,496      36  GATED             GATED             barrier>=30
      568      22,929      36  GATED             GATED             barrier>=30
...
2026-08-06T08:55:50Z  OK  records=18936 (+7)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.8m  drift=0  sci=OK  r115=22B  sweep=608.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
