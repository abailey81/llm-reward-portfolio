...
test tier: 18,506 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340      11,555      76  GATED             GATED             no-records:test/scalar>=30
      403      14,389      77  GATED             GATED             no-records:test/scalar>=30
      568      21,822      77  GATED             GATED             no-records:test/scalar>=30
