...
**Auto-generated 2026-08-04 23:08 UTC -- T+169h59m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+169h59m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 38 min ago; none live, newest failure 38 min ago** |
...
| | |
|---|---|
| cluster jobs | **990** (166 running, 822 queued) |
| **cores computing** | **1328** |
...
```
generated 2026-08-04 23:08 UTC | elapsed 7.08 d | 22.0 d to the Aug-27 stop
test tier: 13,212 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     175 records     175.0 rec/h
    last  3 h     571 records     190.3 rec/h
    last 12 h    2248 records     187.3 rec/h
    last 24 h    4289 records     178.7 rec/h
    12 h rate is 64% from ONE line (test_leg_sonnet_5); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 64% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_sonnet_5, test_leg_gpt_5_6_luna) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    8 of the ceiling (67 rec/h). Window 12 h.
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30         240       1  GATED             GATED             barrier
      100       3,341       5  GATED             GATED             barrier>=30
      189       7,791       5  GATED             GATED             barrier>=30
      279      12,291       5  GATED             GATED             barrier>=30
      340      15,341       5  GATED             GATED             barrier>=30
      403      18,491       5  GATED             GATED             barrier>=30
      568      27,116     175  GATED             GATED             barrier>=30
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1328 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           4.6 d           6.5 d   throughput
      279           5.8 d           9.3 d   throughput
      340           7.0 d          11.1 d   throughput
      403           8.2 d          13.0 d   throughput
      568          11.3 d          18.1 d   throughput
