...
**Auto-generated 2026-08-04 18:45 UTC -- T+165h36m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+165h36m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **13897** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 7.6 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
| LLM calls / spend | 2956 / **$45.5021** |
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on nemotron-3-super, 2.9 h ago; none live, newest failure 2.9 h ago** |
...
| | |
|---|---|
| cluster jobs | **173** (171 running, 0 queued) |
...
```
generated 2026-08-04 18:45 UTC | elapsed 6.90 d | 22.2 d to the Aug-27 stop
test tier: 12,357 records over 63 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     205 records     205.0 rec/h
    last  3 h     587 records     195.7 rec/h
    last 12 h    2385 records     198.8 rec/h
    last 24 h    4259 records     177.5 rec/h
    12 h rate is 99% from ONE line (test_leg_sonnet_5); 2 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    8 of the ceiling (199 rec/h). Window 12 h.
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30         404       0  GATED             GATED             barrier
      100       3,871      33  GATED             GATED             barrier>=30
      189       8,321      33  GATED             GATED             barrier>=30
      279      12,821      33  GATED             GATED             barrier>=30
      340      15,871      33  GATED             GATED             barrier>=30
      403      19,021      33  GATED             GATED             barrier>=30
      568      27,971     205  GATED             GATED             barrier>=30
    ...
    !! 90% of the rung-568 backlog (25,230 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
```
2026-08-04T18:20:30Z  OK  records=13814 (+18)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  sweep=235.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-04T18:27:54Z  OK  records=13838 (+24)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  sweep=414.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
