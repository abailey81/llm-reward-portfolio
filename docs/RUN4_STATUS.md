...
**Auto-generated 2026-08-12 05:57 UTC -- T+344h48m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+344h48m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 12.3 h ago** |
...
| | |
|---|---|
| cluster jobs | **464** = 114 running + **22 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **912** |
| **cores doing RUNG-RAISING work** | **78.3%** -- 720 of 920 cores (25 min old) |
...
```
generated 2026-08-12 05:58 UTC | elapsed 14.37 d | 14.8 d to the Aug-27 stop
test tier: 30,454 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 61.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h     733 records      61.1 rec/h   usable
    last 24 h    2237 records      93.2 rec/h   usable
    12 h rate is 30% from ONE line (test); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279          22      49  2026-08-12 06:20  2026-08-12 06:20  yes
      340       1,307      49  2026-08-13 03:22  2026-08-13 03:22  yes
      403       3,512      49  2026-08-14 15:28  2026-08-14 15:28  yes
      568       9,874      96  2026-08-18 23:37  2026-08-18 23:37  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @912 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           5.9 d           6.5 d   throughput
      279           8.4 d           9.3 d   throughput
      340          10.1 d          11.1 d   throughput
      403          11.9 d          13.0 d   throughput
      568          16.5 d          18.1 d   throughput
...
|---|---|---|---|---|
| nemotron_3_super | **273** | 276 | 5 |  |
| test | **306** | 307 | 9 |  |
| glm_5_2 | **307** | 309 | 5 |  |
| deepseek_v4_pro | **312** | 314 | 5 |  |
| kimi_k3 | **471** | 474 | 5 |  |
| qwen3_6_27b | **544** | 547 | 5 |  |
...
2026-08-12T05:40:51Z  OK  records=31953 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=56.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
