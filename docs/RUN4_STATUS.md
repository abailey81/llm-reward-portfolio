...
**Auto-generated 2026-08-11 17:43 UTC -- T+332h34m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+332h34m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| **cores computing** | **528** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (10 min old) |
...
```
generated 2026-08-11 17:44 UTC | elapsed 13.86 d | 15.3 d to the Aug-27 stop
test tier: 29,697 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last  3 h     433 records     144.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,902     101  GATED             GATED             barrier>=279
      403       4,107     101  GATED             GATED             barrier>=279
      568      10,631     101  GATED             GATED             barrier>=279
