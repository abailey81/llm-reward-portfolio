...
**Auto-generated 2026-08-06 00:18 UTC -- T+195h09m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+195h09m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on haiku-4_5, 5.2 h ago; none live, newest failure 14 min ago** |
...
| | |
|---|---|
| cluster jobs | **986** (63 running, 923 queued) |
| **cores computing** | **504** |
...
```
generated 2026-08-06 00:18 UTC | elapsed 8.13 d | 21.0 d to the Aug-27 stop
test tier: 16,948 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     232 records     232.0 rec/h
    last  3 h     546 records     182.0 rec/h
    last 12 h    1899 records     158.2 rec/h
    last 24 h    3550 records     147.9 rec/h
    12 h rate is 72% from ONE line (test_leg_haiku_4_5); 4 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 72% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_haiku_4_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       2,746     160  GATED             GATED             barrier>=30
      189       6,318     231  GATED             GATED             barrier>=30
      279      10,367     232  GATED             GATED             barrier>=30
      340      13,112     232  GATED             GATED             barrier>=30
      403      15,947     232  GATED             GATED             barrier>=30
      568      23,380     232  GATED             GATED             barrier>=30
