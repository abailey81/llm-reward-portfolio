...
**Auto-generated 2026-08-09 19:51 UTC -- T+286h42m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+286h42m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **26370** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 2.5 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
| LLM calls / spend | 2956 / **$45.5021** |
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on kimi-k3, 3.3 h ago; none live, newest failure 3.3 h ago** |
...
| | |
|---|---|
| cluster jobs | **705** = 144 running + **185 ELIGIBLE** + 376 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **100.0%** -- 1128 of 1128 cores (14 min old) |
...
```
generated 2026-08-09 19:52 UTC | elapsed 11.95 d | 17.2 d to the Aug-27 stop
test tier: 24,830 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 130.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     150 records     150.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     467 records     155.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1568 records     130.7 rec/h   usable
    last 24 h    2867 records     119.5 rec/h   usable
    12 h rate is 71% from ONE line (test); 6 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (131 rec/h); latest excludes cells already within
    8 of the ceiling (131 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      189         901     150  2026-08-10 02:46  2026-08-10 02:46  yes
      279       4,051     150  2026-08-11 02:52  2026-08-11 02:52  yes
      340       6,186     150  2026-08-11 19:12  2026-08-11 19:12  yes
      403       8,422     150  2026-08-12 12:19  2026-08-12 12:19  yes
      568      15,498     150  2026-08-14 18:28  2026-08-14 18:28  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 2% of the rung-568 backlog (379 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test -- baseline_return_minus_downside has 254 HOLE(S) below its frontier 406 -- that is what caps this line**
...
|---|---|---|---|---|
| nemotron_3_super | **151** | 154 | 5 |  |
| test | **158** | 160 | 9 |  |
| glm_5_2 | **174** | 177 | 5 |  |
| deepseek_v4_pro | **184** | 186 | 5 |  |
| qwen3_6_27b | **396** | 398 | 5 |  |
| kimi_k3 | **471** | 474 | 5 |  |
...
currently at **153 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-09T17:57:38Z  OK  records=26082 (+87)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=1488.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-09T18:17:22Z  OK  records=26171 (+89)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  cores=1112  sweep=1153.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-09T18:42:16Z  OK  records=26197 (+26)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.2m  drift=0  sci=OK  r115=22B  sweep=1464.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-09T19:02:04Z  OK  records=26249 (+52)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  cores=1128  sweep=1157.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-09T19:28:21Z  OK  records=26304 (+55)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  sweep=1546.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-09T19:47:56Z  OK  records=26355 (+51)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.8m  drift=0  sci=OK  r115=22B  cores=1152  sweep=1145.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
