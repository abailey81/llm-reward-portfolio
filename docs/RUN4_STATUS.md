...
**Auto-generated 2026-08-12 23:26 UTC -- T+362h17m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+362h17m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=2; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 29.8 h ago** |
...
| | |
|---|---|
| cluster jobs | **323** = 31 running + **239 ELIGIBLE** + 53 held by us + 0 held only by the site |
...
| **cores computing** | **248** |
| **cores doing RUNG-RAISING work** | **47.8%** -- 88 of 184 cores (70 min old) |
...
```
generated 2026-08-12 23:27 UTC | elapsed 15.10 d | 14.0 d to the Aug-27 stop
test tier: 31,536 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 45.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h       3 records       1.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     550 records      45.8 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (46 rec/h); latest excludes cells already within
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         424       0  2026-08-13 08:42  2026-08-13 08:42  yes
      403       2,445       0  2026-08-15 04:48  2026-08-15 04:48  yes
      568       8,792       0  2026-08-20 23:17  2026-08-20 23:17  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @248 cores      @830 cores   binding
       30           5.6 d           4.6 d   critical_chain
      100          12.8 d           4.6 d   critical_chain
      189          21.9 d           6.5 d   throughput
      279          31.0 d           9.3 d   throughput
      340          37.2 d          11.1 d   throughput
      403          43.7 d          13.0 d   throughput
      568          60.5 d          18.1 d   throughput
