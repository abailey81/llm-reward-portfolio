...
**Auto-generated 2026-07-31 01:08 UTC -- T+51h59m.** Refreshed every 5 minutes by the live session and pushed to GitHub, so
...
| | |
|---|---|
| elapsed | **T+51h59m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **1191** |
| LLM calls / spend | 1695 / **$26.8587** |
...
| | |
|---|---|
| cluster jobs | **185** (90 running, 95 queued) |
| **cores computing** | **700** |

Per-rung ETAs from the registered model at the cores we actually hold:

```
 rung              @700 cores              @830 cores   binding
  ...
  279           11.0 d  08-08            9.3 d  08-07   throughput
  340           13.2 d  08-11           11.1 d  08-09   throughput
  403           15.5 d  08-13           13.0 d  08-10   throughput
  568           21.4 d  08-19           18.1 d  08-15   throughput
```
...
|---|---|---|
| distributional | g5 of 5 | 265 |
| scalar | g5 of 5 | 230 |
| placebo | g3 of 5 | 98 |
| scalar_cvar5 | g3 of 5 | 104 |
| placebo_shuffled | g2 of 5 | 91 |
...
```
2026-07-31T00:56:14Z  RED  records=1174 (+0)  spend=$26.8558  guards=2  arms_full=10/10  budget=2  stalest=0.5m  drift=2  budget deep dive
2026-07-31T00:57:14Z  RED  records=1176 (+2)  spend=$26.8558  guards=2  arms_full=10/10  budget=2  stalest=0.7m  drift=2  budget projection audit
2026-07-31T00:59:23Z  RED  records=1176 (+0)  spend=$26.8558  guards=2  arms_full=10/10  budget=2  stalest=0.7m  drift=2  designing results-layer monitoring
2026-07-31T01:01:50Z  RED  records=1178 (+2)  spend=$26.8577  guards=2  arms_full=10/10  budget=2  stalest=0.7m  drift=2  auditing frozen-winner records
2026-07-31T01:05:21Z  ATTN  records=1178 (+0)  spend=$26.8588  guards=2  arms_full=10/10  budget=2  stalest=0.6m  drift=2  sci=OK  r115=9B  results layer live - first run
2026-07-31T01:06:06Z  OK  records=1179 (+1)  spend=$26.8588  guards=2  arms_full=10/10  budget=2  stalest=0.5m  drift=2  sci=OK  r115=9B  verifying publisher
```
...
```
  anthropic   spent $ 22.1521  + still to author $ 13.4662  = $ 35.6184   credited $ 28.1500   margin $  -7.4684 (-27%)  over the credit ESTIMATE (owner-watched)
  openrouter  spent $  4.7066  + still to author $  4.8027  = $  9.5093   credited $ 19.3100   margin $  +9.8007 (+51%)  comfortable
```
