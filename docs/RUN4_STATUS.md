...
**Auto-generated 2026-08-11 08:09 UTC -- T+323h00m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+323h00m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **606** = 145 running + **107 ELIGIBLE** + 354 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **100.0%** -- 1152 of 1152 cores (36 min old) |
...
```
generated 2026-08-11 08:10 UTC | elapsed 13.46 d | 15.7 d to the Aug-27 stop
test tier: 28,497 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 117.2 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     395 records     131.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1406 records     117.2 rec/h   usable
    last 24 h    2454 records     102.2 rec/h   usable
    12 h rate is 41% from ONE line (test); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         967     109  2026-08-11 16:25  2026-08-11 16:25  yes
      340       3,102     109  2026-08-12 10:38  2026-08-12 10:38  yes
      403       5,307     109  2026-08-13 05:27  2026-08-13 05:27  yes
      568      11,831     109  2026-08-15 13:08  2026-08-15 13:08  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 6% of the rung-568 backlog (695 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test -- baseline_volatility_scaled_return has 173 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **234 seeds each** (read live from the archive on this publish). !! THE
