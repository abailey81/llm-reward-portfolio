...
**Auto-generated 2026-08-11 12:33 UTC -- T+327h24m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+327h24m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on kimi-k3, 75 min ago; none live, newest failure 75 min ago** |
...
| | |
|---|---|
| cluster jobs | **?** = ? running + **? ELIGIBLE** + ? held by us + ? held only by the site |
...
| **cores computing** | **?** |
| **cores doing RUNG-RAISING work** | **77.9%** -- 816 of 1048 cores (61 min old) |
...
```
generated 2026-08-11 12:34 UTC | elapsed 13.64 d | 15.5 d to the Aug-27 stop
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last  3 h     302 records     100.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         507      99  2026-08-11 17:05  2026-08-11 17:05  yes
      340       2,642      99  2026-08-12 12:04  2026-08-12 12:04  yes
      403       4,847      99  2026-08-13 07:40  2026-08-13 07:40  yes
      568      11,371      99  2026-08-15 17:39  2026-08-15 17:39  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
    (core count unavailable this cycle -- the model table needs it; the EMPIRICAL
     block above does NOT and is unaffected)
...
```
UNDECIDED (cluster unreachable or tag unresolved -- NOT an alarm): test, test_leg_nemotron_3_super, test_leg_glm_5_2, test_leg_deepseek_v4_pro, test_leg_kimi_k3, test_leg_qwen3_6_27b
