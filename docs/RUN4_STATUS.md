...
**Auto-generated 2026-08-12 07:59 UTC -- T+346h50m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+346h50m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 14.3 h ago** |
...
| | |
|---|---|
| cluster jobs | **438** = 110 running + **0 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **880** |
| **cores doing RUNG-RAISING work** | **95.7%** -- 880 of 920 cores (26 min old) |
...
```
generated 2026-08-12 08:00 UTC | elapsed 14.45 d | 14.7 d to the Aug-27 stop
test tier: 30,633 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 64.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      64 records      64.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     271 records      90.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     769 records      64.1 rec/h   usable
    last 24 h    2156 records      89.8 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,143      64  2026-08-13 01:50  2026-08-13 01:50  yes
      403       3,348      64  2026-08-14 12:15  2026-08-14 12:15  yes
      568       9,695      64  2026-08-18 15:18  2026-08-18 15:18  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @880 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           6.2 d           6.5 d   throughput
      279           8.7 d           9.3 d   throughput
      340          10.5 d          11.1 d   throughput
      403          12.3 d          13.0 d   throughput
      568          17.0 d          18.1 d   throughput
