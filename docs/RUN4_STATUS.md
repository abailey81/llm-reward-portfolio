...
**Auto-generated 2026-08-12 08:07 UTC -- T+346h58m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+346h58m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 14.5 h ago** |
...
| | |
|---|---|
| cluster jobs | **437** = 109 running + **0 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **872** |
| **cores doing RUNG-RAISING work** | **95.7%** -- 880 of 920 cores (33 min old) |
...
```
generated 2026-08-12 08:08 UTC | elapsed 14.46 d | 14.7 d to the Aug-27 stop
test tier: 30,645 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 63.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      66 records      66.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     278 records      92.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     766 records      63.8 rec/h   usable
    last 24 h    2150 records      89.6 rec/h   usable
    12 h rate is 26% from ONE line (test_leg_glm_5_2); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,131      66  2026-08-13 01:51  2026-08-13 01:51  yes
      403       3,336      66  2026-08-14 12:24  2026-08-14 12:24  yes
      568       9,683      66  2026-08-18 15:49  2026-08-18 15:49  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 5% of the rung-568 backlog (476 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @872 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           6.2 d           6.5 d   throughput
      279           8.8 d           9.3 d   throughput
      340          10.6 d          11.1 d   throughput
      403          12.4 d          13.0 d   throughput
      568          17.2 d          18.1 d   throughput
...
currently at **305 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-12T07:18:16Z  ATTN  records=32126 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  sweep=55.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T07:19:54Z  ATTN  records=32126 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  cores=920  sweep=67.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T07:34:46Z  ATTN  records=32150 (+24)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=861.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T07:51:20Z  ATTN  records=32152 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.8m  drift=0  sci=OK  r115=22B  sweep=963.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T08:06:18Z  ATTN  records=32184 (+32)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.1m  drift=0  sci=OK  r115=22B  cores=872  sweep=867.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T08:07:53Z  ATTN  records=32186 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=65.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
