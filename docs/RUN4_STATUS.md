...
| transport health | **timeouts 6h=106; worst streak 11/240 (4.6% to fatal), pull on core, 2.4 h ago; none live, newest failure 2.4 h ago** |
...
| | |
|---|---|
| cluster jobs | **839** = 87 running + **27 ELIGIBLE** + 725 held by us + 0 held only by the site |
...
```
generated 2026-08-06 19:21 UTC | elapsed 8.93 d | 20.2 d to the Aug-27 stop
test tier: 18,397 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340      11,663      30  GATED             GATED             no-records:test/distributional,test/scalar>=30
      403      14,498      30  GATED             GATED             no-records:test/distributional,test/scalar>=30
      568      21,931      30  GATED             GATED             no-records:test/distributional,test/scalar>=30
