...
**Auto-generated 2026-08-12 14:18 UTC -- T+353h09m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+353h09m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 20.6 h ago** |
...
| | |
|---|---|
| cluster jobs | **355** = 55 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **440** |
| **cores doing RUNG-RAISING work** | **77.9%** -- 424 of 544 cores (45 min old) |
...
```
generated 2026-08-12 14:19 UTC | elapsed 14.72 d | 14.4 d to the Aug-27 stop
test tier: 31,300 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 94.5 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     114 records     114.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    ...
    last 12 h    1134 records      94.5 rec/h   usable
    last 24 h    2120 records      88.3 rec/h   usable
    12 h rate is 60% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         489     101  2026-08-12 19:29  2026-08-12 19:29  yes
      403       2,681     114  2026-08-13 18:41  2026-08-13 18:41  yes
      568       9,028     114  2026-08-16 13:51  2026-08-16 13:51  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 19% of the rung-568 backlog (1,752 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @440 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           7.2 d           4.6 d   critical_chain
      189          12.3 d           6.5 d   throughput
      279          17.5 d           9.3 d   throughput
      340          21.0 d          11.1 d   throughput
      403          24.6 d          13.0 d   throughput
      568          34.1 d          18.1 d   throughput
...
currently at **336 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-12T13:40:40Z  OK  records=32760 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.6m  drift=0  sci=OK  r115=22B  sweep=57.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T13:42:10Z  OK  records=32761 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=59.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T13:43:35Z  OK  records=32763 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.3m  drift=0  sci=OK  r115=22B  sweep=55.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T13:44:59Z  OK  records=32763 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.2m  drift=0  sci=OK  r115=22B  sweep=54.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T13:59:27Z  OK  records=32788 (+25)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.2m  drift=0  sci=OK  r115=22B  cores=488  sweep=837.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T14:11:08Z  OK  records=32798 (+10)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=4.0m  drift=0  sci=OK  r115=22B  sweep=671.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
