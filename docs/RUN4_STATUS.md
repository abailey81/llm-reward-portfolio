...
**Auto-generated 2026-08-04 22:47 UTC -- T+169h38m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+169h38m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 17 min ago; none live, newest failure 17 min ago** |
...
| | |
|---|---|
| cluster jobs | **993** (159 running, 832 queued) |
...
```
generated 2026-08-04 22:47 UTC | elapsed 7.07 d | 22.1 d to the Aug-27 stop
test tier: 13,135 records over 68 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     131 records     131.0 rec/h
    last  3 h     558 records     186.0 rec/h
    last 12 h    2210 records     184.2 rec/h
    last 24 h    4307 records     179.5 rec/h
    12 h rate is 67% from ONE line (test_leg_sonnet_5); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 67% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_sonnet_5, test_leg_gpt_5_6_luna) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30         240       9  GATED             GATED             barrier
      100       3,342      20  GATED             GATED             barrier>=30
      189       7,792      20  GATED             GATED             barrier>=30
      279      12,292      20  GATED             GATED             barrier>=30
      340      15,342      20  GATED             GATED             barrier>=30
      403      18,492      20  GATED             GATED             barrier>=30
      568      27,193     131  GATED             GATED             barrier>=30
