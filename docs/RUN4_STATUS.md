...
**Auto-generated 2026-08-11 17:52 UTC -- T+332h43m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+332h43m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 5.3 h ago; none live, newest failure 12 min ago** |
...
| **cores computing** | **528** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (20 min old) |
...
```
generated 2026-08-11 17:53 UTC | elapsed 13.86 d | 15.3 d to the Aug-27 stop
test tier: 29,710 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 125.4 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     419 records     139.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1505 records     125.4 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         128      17  GATED             GATED             barrier
      340       1,889      98  GATED             GATED             barrier>=279
      403       4,094      98  GATED             GATED             barrier>=279
      568      10,618      98  GATED             GATED             barrier>=279
