...
**Auto-generated 2026-08-11 09:18 UTC -- T+324h09m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+324h09m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **586** = 144 running + **88 ELIGIBLE** + 354 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **100.0%** -- 1152 of 1152 cores (105 min old **STALE**) |
...
```
generated 2026-08-11 09:19 UTC | elapsed 13.51 d | 15.6 d to the Aug-27 stop
test tier: 28,641 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 119.9 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     398 records     132.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1439 records     119.9 rec/h   usable
    last 24 h    2452 records     102.2 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (120 rec/h); latest excludes cells already within
    8 of the ceiling (120 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         823     110  2026-08-11 16:11  2026-08-11 16:11  yes
      340       2,958     110  2026-08-12 09:59  2026-08-12 09:59  yes
      403       5,163     110  2026-08-13 04:22  2026-08-13 04:22  yes
      568      11,687     110  2026-08-15 10:46  2026-08-15 10:46  yes
...
> **What is holding it right now: test -- baseline_return_minus_turnover has 163 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **244 seeds each** (read live from the archive on this publish). !! THE
...
2026-08-11T08:55:42Z  OK  records=30145 (+27)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.1m  drift=0  sci=OK  r115=22B  cores=1152  sweep=673.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T09:05:48Z  OK  records=30145 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.7m  drift=0  sci=OK  r115=22B  sweep=575.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T09:18:40Z  OK  records=30182 (+37)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=4.0m  drift=0  sci=OK  r115=22B  cores=1152  sweep=741.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
