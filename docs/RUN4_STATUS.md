...
**Auto-generated 2026-08-12 03:32 UTC -- T+342h24m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+342h24m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 9.9 h ago** |
...
| | |
|---|---|
| cluster jobs | **490** = 110 running + **52 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **880** |
| **cores doing RUNG-RAISING work** | **76.4%** -- 672 of 880 cores (0 min old) |
...
```
generated 2026-08-12 03:33 UTC | elapsed 14.27 d | 14.9 d to the Aug-27 stop
test tier: 30,270 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last  3 h     196 records      65.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     847 records      70.6 rec/h   usable
    last 24 h    2460 records     102.5 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,357      68  GATED             GATED             barrier>=279
      403       3,562      68  GATED             GATED             barrier>=279
      568      10,058      96  GATED             GATED             barrier>=279
