...
**Auto-generated 2026-08-11 19:24 UTC -- T+334h15m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+334h15m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 1.7 h ago; none live, newest failure 1.7 h ago** |
...
| | |
|---|---|
| cluster jobs | **439** = 65 running + **46 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **520** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (111 min old **STALE**) |
...
```
generated 2026-08-11 19:25 UTC | elapsed 13.93 d | 15.2 d to the Aug-27 stop
test tier: 29,812 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 116.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1400 records     116.7 rec/h   usable
    last 24 h    2778 records     115.8 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,787      56  GATED             GATED             barrier>=279
      403       3,992      56  GATED             GATED             barrier>=279
      568      10,516      56  GATED             GATED             barrier>=279
