...
**Auto-generated 2026-08-11 20:27 UTC -- T+335h18m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| **cores computing** | **472** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (55 min old) |
...
```
generated 2026-08-11 20:28 UTC | elapsed 13.97 d | 15.1 d to the Aug-27 stop
test tier: 29,906 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 113.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1357 records     113.1 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,693      85  GATED             GATED             barrier>=279
      403       3,898      85  GATED             GATED             barrier>=279
      568      10,422      85  GATED             GATED             barrier>=279
