...
**Auto-generated 2026-08-13 03:34 UTC -- T+366h25m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+366h25m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 33.9 h ago** |
...
| | |
|---|---|
| cluster jobs | **312** = 71 running + **175 ELIGIBLE** + 66 held by us + 0 held only by the site |
...
| **cores computing** | **568** |
| **cores doing RUNG-RAISING work** | **27.5%** -- 112 of 408 cores (120 min old **STALE**) |
...
```
generated 2026-08-13 03:35 UTC | elapsed 15.27 d | 13.9 d to the Aug-27 stop
test tier: 31,584 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 11.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      43 records      43.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    ...
    last 12 h     140 records      11.7 rec/h   usable
    last 24 h    1310 records      54.6 rec/h   usable
    12 h rate is 31% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (12 rec/h); latest excludes cells already within
    8 of the ceiling (12 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         376      43  2026-08-14 11:49  2026-08-14 11:49  yes
      403       2,397      43  2026-08-21 17:02  2026-08-21 17:02  yes
      568       8,744      43  2026-09-13 09:04  2026-09-13 09:04  NO
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @568 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           5.6 d           4.6 d   critical_chain
      189           9.5 d           6.5 d   throughput
      279          13.5 d           9.3 d   throughput
      340          16.3 d          11.1 d   throughput
      403          19.1 d          13.0 d   throughput
      568          26.4 d          18.1 d   throughput
...
2026-08-13T03:29:13Z  OK  records=33127 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.8m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=62.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-13T03:30:48Z  OK  records=33127 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.2m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=64.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
