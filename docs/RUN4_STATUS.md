...
**Auto-generated 2026-08-11 17:20 UTC -- T+332h11m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+332h11m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **31198** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 0.6 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
| LLM calls / spend | 2956 / **$45.5021** |
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 4.8 h ago; none live, newest failure 4.8 h ago** |
...
| | |
|---|---|
| cluster jobs | **459** = 64 running + **67 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **512** |
| **cores doing RUNG-RAISING work** | **36.1%** -- 240 of 664 cores (107 min old **STALE**) |
...
```
generated 2026-08-11 17:21 UTC | elapsed 13.84 d | 15.3 d to the Aug-27 stop
test tier: 29,660 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 126.5 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     478 records     159.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1518 records     126.5 rec/h   usable
    last 24 h    2740 records     114.2 rec/h   usable
    12 h rate is 97% from ONE line (test); 3 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (126 rec/h); latest excludes cells already within
    8 of the ceiling (126 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         128      32  2026-08-11 18:21  2026-08-11 18:21  yes
      340       1,939     115  2026-08-12 08:40  2026-08-12 08:40  yes
      403       4,144     115  2026-08-13 02:06  2026-08-13 02:06  yes
      568      10,668     115  2026-08-15 05:41  2026-08-15 05:41  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 27% of the rung-568 backlog (2,867 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @512 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           6.2 d           4.6 d   critical_chain
      189          10.6 d           6.5 d   throughput
      279          15.0 d           9.3 d   throughput
      340          18.0 d          11.1 d   throughput
      403          21.2 d          13.0 d   throughput
      568          29.3 d          18.1 d   throughput
...
> **What is holding it right now: test -- baseline_log_growth has 114 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **293 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T16:45:59Z  OK  records=31140 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  sweep=55.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T16:58:42Z  OK  records=31161 (+21)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  cores=544  sweep=733.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T17:08:54Z  OK  records=31164 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.3m  drift=0  sci=OK  r115=22B  sweep=581.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T17:17:23Z  OK  records=31192 (+28)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  sweep=478.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T17:19:02Z  OK  records=31197 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  cores=528  sweep=69.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T17:20:27Z  OK  records=31198 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  sweep=54.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
