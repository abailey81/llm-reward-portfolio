...
**Auto-generated 2026-08-05 06:22 UTC -- T+177h13m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+177h13m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **15482** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 7.1 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
| LLM calls / spend | 2956 / **$45.5021** |
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on qwen3_6-27b, 80 min ago; none live, newest failure 80 min ago** |
...
| | |
|---|---|
| cluster jobs | **953** (209 running, 744 queued) |
| **cores computing** | **1672** |
...
```
generated 2026-08-05 06:22 UTC | elapsed 7.38 d | 21.7 d to the Aug-27 stop
test tier: 13,939 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     265 records     265.0 rec/h
    last  3 h     336 records     112.0 rec/h
    last 12 h    1645 records     137.1 rec/h
    last 24 h    4010 records     167.1 rec/h
    12 h rate is 37% from ONE line (test_leg_qwen3_5_9b); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 45% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_qwen3_5_9b, test_leg_sonnet_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (137 rec/h); latest excludes cells already within
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       3,238      19  GATED             GATED             barrier>=30
      189       7,439     265  GATED             GATED             barrier>=30
      279      11,939     265  GATED             GATED             barrier>=30
      340      14,989     265  GATED             GATED             barrier>=30
      403      18,139     265  GATED             GATED             barrier>=30
      568      26,389     265  GATED             GATED             barrier>=30
    ...
    !! 68% of the rung-568 backlog (17,994 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1672 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           4.6 d           6.5 d   throughput
      279           4.6 d           9.3 d   throughput
      340           5.5 d          11.1 d   throughput
      403           6.5 d          13.0 d   throughput
      568           9.0 d          18.1 d   throughput
...
```
2026-08-05T05:52:21Z  OK  records=15346 (+26)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  sweep=247.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T06:00:43Z  OK  records=15382 (+36)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  cores=1664  sweep=472.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T06:05:33Z  OK  records=15420 (+38)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  sweep=259.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T06:12:41Z  OK  records=15445 (+25)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=397.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T06:16:40Z  OK  records=15457 (+12)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.4m  drift=0  sci=OK  r115=22B  sweep=209.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T06:21:48Z  OK  records=15475 (+18)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=0.7m  drift=0  sci=OK  r115=22B  cores=1672  sweep=277.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
