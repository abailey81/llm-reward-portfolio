...
**Auto-generated 2026-08-12 19:44 UTC -- T+358h35m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+358h35m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=2; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 26.1 h ago** |
...
| **cores computing** | **208** |
| **cores doing RUNG-RAISING work** | **42.3%** -- 88 of 208 cores (2 min old) |
...
```
generated 2026-08-12 19:45 UTC | elapsed 14.94 d | 14.2 d to the Aug-27 stop
test tier: 31,516 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 74.6 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h      35 records      11.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     895 records      74.6 rec/h   usable
    last 24 h    1675 records      69.8 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         444      12  2026-08-13 01:42  2026-08-13 01:42  yes
      403       2,465      12  2026-08-14 04:48  2026-08-14 04:48  yes
      568       8,812      12  2026-08-17 17:54  2026-08-17 17:54  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 6% of the rung-568 backlog (572 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test_leg_deepseek_v4_pro -- distributional has 26 HOLE(S) below its frontier 339 -- that is what caps this line**
...
2026-08-12T19:43:12Z  OK  records=33053 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.5m  drift=0  sci=OK  r115=22B  sweep=689.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
