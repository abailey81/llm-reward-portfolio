...
**Auto-generated 2026-08-11 19:53 UTC -- T+334h44m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+334h44m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 2.2 h ago; none live, newest failure 2.2 h ago** |
...
| | |
|---|---|
| cluster jobs | **433** = 61 running + **44 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **488** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (20 min old) |
...
```
generated 2026-08-11 19:53 UTC | elapsed 13.95 d | 15.2 d to the Aug-27 stop
test tier: 29,855 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 116.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     243 records      81.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1393 records     116.1 rec/h   usable
    last 24 h    2779 records     115.8 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,744      61  GATED             GATED             barrier>=279
      403       3,949      61  GATED             GATED             barrier>=279
      568      10,473      61  GATED             GATED             barrier>=279
...
currently at **297 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T19:34:40Z  OK  records=31368 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  sweep=53.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
