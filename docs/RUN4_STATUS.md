...
**Auto-generated 2026-08-12 02:11 UTC -- T+341h02m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+341h02m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **502** = 102 running + **72 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **816** |
| **cores doing RUNG-RAISING work** | **56.7%** -- 440 of 776 cores (39 min old) |
...
```
generated 2026-08-12 02:12 UTC | elapsed 14.21 d | 14.9 d to the Aug-27 stop
test tier: 30,161 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last  3 h     112 records      37.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,438      49  GATED             GATED             barrier>=279
      403       3,643      49  GATED             GATED             barrier>=279
      568      10,167      49  GATED             GATED             barrier>=279
