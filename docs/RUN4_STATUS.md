...
**Auto-generated 2026-08-05 00:12 UTC -- T+171h03m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+171h03m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 1.7 h ago; none live, newest failure 36 min ago** |
...
| | |
|---|---|
| cluster jobs | **968** (170 running, 796 queued) |
| **cores computing** | **1360** |
...
```
generated 2026-08-05 00:12 UTC | elapsed 7.13 d | 22.0 d to the Aug-27 stop
test tier: 13,386 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     156 records     156.0 rec/h
    last  3 h     456 records     152.0 rec/h
    last 12 h    2254 records     187.8 rec/h
    last 24 h    4232 records     176.3 rec/h
    12 h rate is 56% from ONE line (test_leg_sonnet_5); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 57% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_sonnet_5, test_leg_gpt_5_6_luna) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    8 of the ceiling (82 rec/h). Window 12 h.
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      568      26,942     156  GATED             GATED             barrier>=30
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1360 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           4.6 d           6.5 d   throughput
      279           5.7 d           9.3 d   throughput
      340           6.8 d          11.1 d   throughput
      403           8.0 d          13.0 d   throughput
      568          11.0 d          18.1 d   throughput
