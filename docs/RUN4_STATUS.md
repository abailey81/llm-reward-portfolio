...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 6.0 h ago; none live, newest failure 52 min ago** |
...
| | |
|---|---|
| cluster jobs | **445** = 65 running + **52 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **520** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (59 min old) |
...
```
generated 2026-08-11 18:32 UTC | elapsed 13.89 d | 15.2 d to the Aug-27 stop
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,823     103  GATED             GATED             barrier>=279
      403       4,028     103  GATED             GATED             barrier>=279
      568      10,552     103  GATED             GATED             barrier>=279
