...
**Auto-generated 2026-08-13 03:51 UTC -- T+366h42m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+366h42m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 34.2 h ago** |
...
| | |
|---|---|
| cluster jobs | **312** = 75 running + **171 ELIGIBLE** + 66 held by us + 0 held only by the site |
...
| **cores computing** | **600** |
| **cores doing RUNG-RAISING work** | **19.4%** -- 112 of 576 cores (15 min old) |
...
```
generated 2026-08-13 03:52 UTC | elapsed 15.28 d | 13.8 d to the Aug-27 stop
test tier: 31,600 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 11.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h      64 records      21.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     141 records      11.8 rec/h   usable
    last 24 h    1311 records      54.6 rec/h   usable
    12 h rate is 34% from ONE line (test_leg_glm_5_2); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         360      32  2026-08-14 10:30  2026-08-14 10:30  yes
      403       2,381      32  2026-08-21 14:30  2026-08-21 14:30  yes
      568       8,728      32  2026-09-13 02:40  2026-09-13 02:40  NO
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @600 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           5.3 d           4.6 d   critical_chain
      189           9.0 d           6.5 d   throughput
      279          12.8 d           9.3 d   throughput
      340          15.4 d          11.1 d   throughput
      403          18.1 d          13.0 d   throughput
      568          25.0 d          18.1 d   throughput
...
2026-08-13T03:45:47Z  OK  records=33127 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=4.7m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=869.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
