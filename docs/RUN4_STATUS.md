...
**Auto-generated 2026-08-11 16:47 UTC -- T+331h38m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+331h38m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 4.2 h ago; none live, newest failure 4.2 h ago** |
...
| | |
|---|---|
| cluster jobs | **466** = 70 running + **68 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **560** |
| **cores doing RUNG-RAISING work** | **36.1%** -- 240 of 664 cores (75 min old) |
...
```
generated 2026-08-11 16:48 UTC | elapsed 13.82 d | 15.3 d to the Aug-27 stop
test tier: 29,603 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 132.9 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     491 records     163.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1595 records     132.9 rec/h   usable
    last 24 h    2693 records     112.2 rec/h   usable
    12 h rate is 99% from ONE line (test); 3 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         151       9  2026-08-11 17:56  2026-08-11 17:56  yes
      340       1,996     140  2026-08-12 07:49  2026-08-12 07:49  yes
      403       4,201     140  2026-08-13 00:25  2026-08-13 00:25  yes
      568      10,725     140  2026-08-15 01:30  2026-08-15 01:30  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 27% of the rung-568 backlog (2,857 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test -- baseline_log_growth has 115 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **292 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T16:38:53Z  OK  records=31126 (+7)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.3m  drift=0  sci=OK  r115=22B  sweep=53.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T16:40:15Z  OK  records=31127 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=51.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
