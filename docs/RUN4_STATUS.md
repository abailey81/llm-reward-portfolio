...
**Auto-generated 2026-08-12 03:55 UTC -- T+342h47m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+342h47m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 10.3 h ago** |
...
| | |
|---|---|
| cluster jobs | **488** = 110 running + **50 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **880** |
| **cores doing RUNG-RAISING work** | **76.4%** -- 672 of 880 cores (23 min old) |
...
```
generated 2026-08-12 03:56 UTC | elapsed 14.28 d | 14.8 d to the Aug-27 stop
test tier: 30,291 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 66.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h     801 records      66.8 rec/h   usable
    last 24 h    2411 records     100.5 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,357      22  GATED             GATED             barrier>=279
      403       3,562      22  GATED             GATED             barrier>=279
      568      10,037      64  GATED             GATED             barrier>=279
...
2026-08-12T03:44:28Z  OK  records=31813 (+15)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.2m  drift=0  sci=OK  r115=22B  sweep=1220.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
