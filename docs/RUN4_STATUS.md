...
**Auto-generated 2026-08-12 19:41 UTC -- T+358h32m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+358h32m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=2; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 26.0 h ago** |
...
| **cores computing** | **208** |
| **cores doing RUNG-RAISING work** | **46.4%** -- 104 of 224 cores (128 min old **STALE**) |
...
```
generated 2026-08-12 19:42 UTC | elapsed 14.94 d | 14.2 d to the Aug-27 stop
test tier: 31,514 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 74.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h      34 records      11.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     896 records      74.7 rec/h   usable
    last 24 h    1677 records      69.9 rec/h   usable
    12 h rate is 96% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (75 rec/h); latest excludes cells already within
    8 of the ceiling (75 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         446      10  2026-08-13 01:40  2026-08-13 01:40  yes
      403       2,467      10  2026-08-14 04:44  2026-08-14 04:44  yes
      568       8,814      10  2026-08-17 17:44  2026-08-17 17:44  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 9% of the rung-568 backlog (827 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test_leg_deepseek_v4_pro -- scalar_cvar5 has 27 HOLE(S) below its frontier 339 -- that is what caps this line**
...
|---|---|---|---|---|
| nemotron_3_super | **305** | 308 | 5 |  |
| glm_5_2 | **309** | 311 | 5 |  |
| deepseek_v4_pro | **313** | 316 | 5 |  |
| test | **350** | 350 | 9 |  |
| kimi_k3 | **471** | 474 | 5 |  |
...
2026-08-12T19:15:38Z  OK  records=33048 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.2m  drift=0  sci=OK  r115=22B  sweep=57.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T19:17:06Z  OK  records=33048 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=57.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T19:18:34Z  OK  records=33048 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.3m  drift=0  sci=OK  r115=22B  sweep=57.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T19:31:12Z  OK  records=33053 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=4.3m  drift=0  sci=OK  r115=22B  cores=208  sweep=728.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
