...
**Auto-generated 2026-08-12 15:58 UTC -- T+354h49m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+354h49m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 22.3 h ago** |
...
| | |
|---|---|
| cluster jobs | **332** = 32 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **256** |
| **cores doing RUNG-RAISING work** | **58.3%** -- 168 of 288 cores (25 min old) |
...
```
generated 2026-08-12 15:59 UTC | elapsed 14.78 d | 14.3 d to the Aug-27 stop
test tier: 31,459 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 97.2 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      67 records      67.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     329 records     109.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1167 records      97.2 rec/h   usable
    last 24 h    1956 records      81.5 rec/h   usable
    12 h rate is 72% from ONE line (test); 3 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         472       0  2026-08-12 20:50  2026-08-12 20:50  yes
      403       2,522      67  2026-08-13 17:55  2026-08-13 17:55  yes
      568       8,869      67  2026-08-16 11:11  2026-08-16 11:11  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 34% of the rung-568 backlog (3,052 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @256 cores      @830 cores   binding
       30           5.5 d           4.6 d   critical_chain
      100          12.4 d           4.6 d   critical_chain
      189          21.2 d           6.5 d   throughput
      279          30.1 d           9.3 d   throughput
      340          36.1 d          11.1 d   throughput
      403          42.3 d          13.0 d   throughput
      568          58.6 d          18.1 d   throughput
...
currently at **347 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-12T15:18:30Z  OK  records=32962 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.3m  drift=0  sci=OK  r115=22B  sweep=55.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T15:19:58Z  OK  records=32965 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.9m  drift=0  sci=OK  r115=22B  sweep=58.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T15:21:23Z  OK  records=32965 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.2m  drift=0  sci=OK  r115=22B  sweep=54.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T15:34:00Z  OK  records=32982 (+17)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  cores=288  sweep=727.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T15:45:19Z  OK  records=32984 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.5m  drift=0  sci=OK  r115=22B  sweep=648.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T15:58:59Z  OK  records=33002 (+18)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=0.9m  drift=0  sci=OK  r115=22B  cores=256  sweep=790.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
