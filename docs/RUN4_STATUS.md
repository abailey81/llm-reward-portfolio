...
**Auto-generated 2026-08-11 16:23 UTC -- T+331h15m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+331h15m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **472** = 71 running + **73 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **568** |
| **cores doing RUNG-RAISING work** | **36.1%** -- 240 of 664 cores (51 min old) |
...
```
generated 2026-08-11 16:24 UTC | elapsed 13.80 d | 15.3 d to the Aug-27 stop
test tier: 29,548 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 133.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     486 records     162.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1606 records     133.8 rec/h   usable
    last 24 h    2651 records     110.5 rec/h   usable
    12 h rate is 97% from ONE line (test); 2 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         160       0  2026-08-11 17:36  2026-08-11 17:36  yes
      340       2,051     150  2026-08-12 07:44  2026-08-12 07:44  yes
      403       4,256     150  2026-08-13 00:12  2026-08-13 00:12  yes
      568      10,780     150  2026-08-15 00:57  2026-08-15 00:57  yes
...
> **What is holding it right now: test -- baseline_log_growth has 118 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **289 seeds each** (read live from the archive on this publish). !! THE
...
2026-08-11T16:15:23Z  OK  records=31083 (+26)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=0.9m  drift=0  sci=OK  r115=22B  cores=576  sweep=679.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
