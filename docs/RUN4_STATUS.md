...
| transport health | **timeouts 6h=156; worst streak 2/240 (0.8% to fatal), ops on core, 40 min ago; none live, newest failure 39 min ago** |
...
| **cores computing** | **576** |
| **cores doing RUNG-RAISING work** | **22.6%** -- 184 of 816 cores (150 min old **STALE**) |
...
```
generated 2026-08-06 15:14 UTC | elapsed 8.75 d | 20.4 d to the Aug-27 stop
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last  1 h     253 records     253.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30          66      34  GATED             GATED             unstarted:2-unit(s)-absent
      100       2,516      34  GATED             GATED             unstarted:2-unit(s)-absent>=30
      189       5,631      35  GATED             GATED             unstarted:2-unit(s)-absent>=30
      279       9,352     253  GATED             GATED             unstarted:2-unit(s)-absent>=30
      340      12,097     253  GATED             GATED             unstarted:2-unit(s)-absent>=30
      403      14,932     253  GATED             GATED             unstarted:2-unit(s)-absent>=30
      568      22,365     253  GATED             GATED             unstarted:2-unit(s)-absent>=30
