...
**Auto-generated 2026-08-13 03:44 UTC -- T+366h35m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+366h35m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 34.1 h ago** |
...
| | |
|---|---|
| cluster jobs | **312** = 74 running + **172 ELIGIBLE** + 66 held by us + 0 held only by the site |
...
| **cores computing** | **592** |
| **cores doing RUNG-RAISING work** | **19.4%** -- 112 of 576 cores (8 min old) |
...
```
generated 2026-08-13 03:45 UTC | elapsed 15.28 d | 13.8 d to the Aug-27 stop
test tier: 31,584 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 10.6 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h     127 records      10.6 rec/h   usable
    last 24 h    1300 records      54.2 rec/h   usable
    12 h rate is 25% from ONE line (test_leg_deepseek_v4_pro); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (11 rec/h); latest excludes cells already within
    8 of the ceiling (11 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         376      36  2026-08-14 15:17  2026-08-14 15:17  yes
      403       2,397      36  2026-08-22 14:14  2026-08-22 14:14  yes
      568       8,744      36  2026-09-16 13:57  2026-09-16 13:57  NO
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 17% of the rung-568 backlog (1,452 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @592 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           5.4 d           4.6 d   critical_chain
      189           9.2 d           6.5 d   throughput
      279          13.0 d           9.3 d   throughput
      340          15.6 d          11.1 d   throughput
      403          18.3 d          13.0 d   throughput
      568          25.3 d          18.1 d   throughput
