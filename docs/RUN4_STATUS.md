...
**Auto-generated 2026-08-12 06:24 UTC -- T+345h15m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+345h15m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **32057** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 0.1 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
...
| | |
|---|---|
| cluster jobs | **457** = 117 running + **12 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **936** |
| **cores doing RUNG-RAISING work** | **78.3%** -- 720 of 920 cores (52 min old) |
...
```
generated 2026-08-12 06:25 UTC | elapsed 14.39 d | 14.7 d to the Aug-27 stop
test tier: 30,516 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 63.2 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     258 records      86.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     758 records      63.2 rec/h   usable
    last 24 h    2255 records      94.0 rec/h   usable
    12 h rate is 28% from ONE line (test); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279           0      61  REACHED           REACHED           yes
      340       1,256      90  2026-08-13 02:18  2026-08-13 02:18  yes
      403       3,461      90  2026-08-14 13:13  2026-08-14 13:13  yes
      568       9,812     117  2026-08-18 17:45  2026-08-18 17:45  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @936 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           5.8 d           6.5 d   throughput
      279           8.2 d           9.3 d   throughput
      340           9.9 d          11.1 d   throughput
      403          11.6 d          13.0 d   throughput
      568          16.0 d          18.1 d   throughput
...
|---|---|---|---|---|
| nemotron_3_super | **283** | 287 | 5 |  |
| test | **306** | 307 | 9 |  |
| glm_5_2 | **307** | 309 | 5 |  |
| deepseek_v4_pro | **312** | 314 | 5 |  |
| kimi_k3 | **471** | 474 | 5 |  |
| qwen3_6_27b | **544** | 549 | 5 |  |
...
```
2026-08-12T06:03:58Z  OK  records=31971 (+18)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=1357.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T06:18:54Z  OK  records=32037 (+66)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.6m  drift=0  sci=OK  r115=22B  cores=944  sweep=865.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T06:20:40Z  OK  records=32041 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  sweep=76.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T06:22:11Z  OK  records=32047 (+6)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.5m  drift=0  sci=OK  r115=22B  sweep=60.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
