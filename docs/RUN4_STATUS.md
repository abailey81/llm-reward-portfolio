...
**Auto-generated 2026-08-06 15:25 UTC -- T+210h16m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+210h16m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=156; worst streak 2/240 (0.8% to fatal), ops on core, 51 min ago; none live, newest failure 51 min ago** |
...
| | |
|---|---|
| cluster jobs | **856** (62 running, 794 queued) |
| **cores computing** | **496** |
| **cores doing RUNG-RAISING work** | **22.6%** -- 184 of 816 cores (161 min old **STALE**) |
...
```
generated 2026-08-06 15:25 UTC | elapsed 8.76 d | 20.4 d to the Aug-27 stop
test tier: 18,015 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 78.0 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     273 records     273.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30          64      24  GATED             GATED             unstarted:2-unit(s)-absent
      100       2,514      24  GATED             GATED             unstarted:2-unit(s)-absent>=30
      189       5,629      25  GATED             GATED             unstarted:2-unit(s)-absent>=30
      279       9,300     273  GATED             GATED             unstarted:2-unit(s)-absent>=30
      340      12,045     273  GATED             GATED             unstarted:2-unit(s)-absent>=30
      403      14,880     273  GATED             GATED             unstarted:2-unit(s)-absent>=30
      568      22,313     273  GATED             GATED             unstarted:2-unit(s)-absent>=30
