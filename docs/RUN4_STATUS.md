...
**Auto-generated 2026-08-11 01:18 UTC -- T+316h09m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+316h09m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **29198** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 1.3 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
...
| | |
|---|---|
| cluster jobs | **712** = 136 running + **222 ELIGIBLE** + 354 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **100.0%** -- 1048 of 1048 cores (106 min old **STALE**) |
...
```
generated 2026-08-11 01:19 UTC | elapsed 13.17 d | 15.9 d to the Aug-27 stop
test tier: 27,657 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 84.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      82 records      82.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    ...
    last 12 h    1009 records      84.1 rec/h   usable
    last 24 h    2373 records      98.9 rec/h   usable
    12 h rate is 36% from ONE line (test_leg_nemotron_3_super); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (84 rec/h); latest excludes cells already within
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       1,807      82  2026-08-11 22:48  2026-08-11 22:48  yes
      340       3,942      82  2026-08-13 00:12  2026-08-13 00:12  yes
      403       6,147      82  2026-08-14 02:25  2026-08-14 02:25  yes
      568      12,671      82  2026-08-17 08:01  2026-08-17 08:01  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 38% of the rung-568 backlog (4,772 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
```
2026-08-11T00:42:28Z  RED  records=29157 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.0m  drift=0  sci=OK  r115=22B  sweep=51.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T00:43:52Z  RED  records=29157 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.3m  drift=0  sci=OK  r115=22B  sweep=53.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T00:45:15Z  RED  records=29161 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.1m  drift=0  sci=OK  r115=22B  sweep=52.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T00:46:36Z  RED  records=29163 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  sweep=50.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T01:01:22Z  RED  records=29183 (+20)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=4.4m  drift=0  sci=OK  r115=22B  cores=1104  sweep=856.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T01:17:57Z  ATTN  records=29183 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.5m  drift=0  sci=OK  r115=22B  cores=1104  sweep=964.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
