...
| **cores computing** | **680** |
| **cores doing RUNG-RAISING work** | **28.8%** -- 168 of 584 cores (39 min old) |
...
test tier: 18,376 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340      11,684      34  GATED             GATED             no-records:test/distributional,test/scalar>=30
      403      14,519      34  GATED             GATED             no-records:test/distributional,test/scalar>=30
      568      21,952      34  GATED             GATED             no-records:test/distributional,test/scalar>=30
