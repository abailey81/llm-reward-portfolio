...
**Auto-generated 2026-08-12 14:44 UTC -- T+353h35m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+353h35m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 21.1 h ago** |
...
| | |
|---|---|
| cluster jobs | **348** = 48 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **384** |
| **cores doing RUNG-RAISING work** | **77.9%** -- 424 of 544 cores (71 min old) |
...
```
generated 2026-08-12 14:45 UTC | elapsed 14.73 d | 14.4 d to the Aug-27 stop
test tier: 31,356 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    12 h rate is 64% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         472      81  2026-08-12 19:38  2026-08-12 19:38  yes
      403       2,625     133  2026-08-13 17:55  2026-08-13 17:55  yes
      568       8,972     133  2026-08-16 11:38  2026-08-16 11:38  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @384 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           8.3 d           4.6 d   critical_chain
      189          14.1 d           6.5 d   throughput
      279          20.0 d           9.3 d   throughput
      340          24.1 d          11.1 d   throughput
      403          28.2 d          13.0 d   throughput
      568          39.1 d          18.1 d   throughput
...
currently at **341 seeds each** (read live from the archive on this publish). !! THE
