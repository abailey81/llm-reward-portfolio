...
**Auto-generated 2026-08-05 02:25 UTC -- T+173h16m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+173h16m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 3.9 h ago; LIVE, still failing** |
...
| | |
|---|---|
| cluster jobs | **993** (180 running, 813 queued) |
| **cores computing** | **1440** |
...
```
generated 2026-08-05 02:25 UTC | elapsed 7.22 d | 21.9 d to the Aug-27 stop
test tier: 13,592 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h      65 records      65.0 rec/h
    last  3 h     317 records     105.7 rec/h
    last 12 h    2068 records     172.3 rec/h
    last 24 h    4199 records     175.0 rec/h
    12 h rate is 42% from ONE line (test_leg_sonnet_5); 10 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (172 rec/h); latest excludes cells already within
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      568      26,736      65  GATED             GATED             barrier>=30
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1440 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           4.6 d           6.5 d   throughput
      279           5.3 d           9.3 d   throughput
      340           6.4 d          11.1 d   throughput
      403           7.5 d          13.0 d   throughput
      568          10.4 d          18.1 d   throughput
