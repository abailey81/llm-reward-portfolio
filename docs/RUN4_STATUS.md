...
**Auto-generated 2026-08-10 07:20 UTC -- T+298h11m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+298h11m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **866** = 165 running + **325 ELIGIBLE** + 376 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **100.0%** -- 1320 of 1320 cores (96 min old **STALE**) |
...
```
generated 2026-08-10 07:20 UTC | elapsed 12.42 d | 16.7 d to the Aug-27 stop
test tier: 25,980 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 101.2 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     383 records     127.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1215 records     101.2 rec/h   usable
    last 24 h    2759 records     115.0 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (101 rec/h); latest excludes cells already within
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      189         145       0  2026-08-10 08:46  2026-08-10 08:46  yes
      279       2,909     110  2026-08-11 12:04  2026-08-11 12:04  yes
      340       5,044     110  2026-08-12 09:09  2026-08-12 09:09  yes
      403       7,272     110  2026-08-13 07:10  2026-08-13 07:10  yes
      568      14,348     110  2026-08-16 05:03  2026-08-16 05:03  yes
...
> **What is holding it right now: test -- baseline_log_growth has 205 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **202 seeds each** (read live from the archive on this publish). !! THE
...
2026-08-10T07:18:28Z  OK  records=27515 (+41)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=0.9m  drift=0  sci=OK  r115=22B  cores=1320  sweep=1513.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
