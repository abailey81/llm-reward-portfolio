...
**Auto-generated 2026-08-11 19:05 UTC -- T+333h56m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+333h56m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 85 min ago; none live, newest failure 85 min ago** |
...
| | |
|---|---|
| cluster jobs | **439** = 63 running + **48 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **504** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (92 min old **STALE**) |
...
```
generated 2026-08-11 19:05 UTC | elapsed 13.91 d | 15.2 d to the Aug-27 stop
test tier: 29,794 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 118.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1417 records     118.1 rec/h   usable
    last 24 h    2772 records     115.5 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (118 rec/h); latest excludes cells already within
    8 of the ceiling (118 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,805      63  GATED             GATED             barrier>=279
      403       4,010      63  GATED             GATED             barrier>=279
      568      10,534      63  GATED             GATED             barrier>=279
...
2026-08-11T18:56:25Z  ATTN  records=31337 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  sweep=52.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T18:57:48Z  ATTN  records=31337 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=52.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
