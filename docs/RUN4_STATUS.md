...
**Auto-generated 2026-08-05 12:16 UTC -- T+183h07m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+183h07m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=5; worst streak 1/240 (0.4% to fatal), ops on core, 5.0 h ago; none live, newest failure 86 min ago** |
...
```
generated 2026-08-05 12:16 UTC | elapsed 7.63 d | 21.5 d to the Aug-27 stop
test tier: 15,046 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     113 records     113.0 rec/h
    last  3 h     388 records     129.3 rec/h
    last 12 h    1652 records     137.7 rec/h
    last 24 h    3902 records     162.6 rec/h
    12 h rate is 63% from ONE line (test_leg_haiku_4_5); 5 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 12% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_qwen3_5_9b, test_leg_sonnet_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (138 rec/h); latest excludes cells already within
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279      10,891      54  GATED             GATED             barrier>=30
      340      13,882     113  GATED             GATED             barrier>=30
      403      17,032     113  GATED             GATED             barrier>=30
      568      25,282     113  GATED             GATED             barrier>=30
...
```
2026-08-05T11:53:23Z  OK  records=16554 (+15)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.4m  drift=0  sci=OK  r115=22B  cores=1472  sweep=319.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
