...
**Auto-generated 2026-08-05 02:59 UTC -- T+173h50m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+173h50m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
| lines up | **** 8 / 12 running; 3 COMPLETE (gemini-2.5-flash, gpt-5.6-luna, h3); **1 MISSING (qwen3.5-9b)** **  <- a roster line is MISSING or a stray is present**, all five arms submitted on **10 of the 10 leg lines** (h3ss is single-arm by design) |
| stalest driver log | **3 min (qwen3_5-9b)** old (P218: the STALEST of the still-running lines, completed ladders excluded; above ~30 means that line has stopped progressing) |
| records archived | **15140** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 7.2 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
| LLM calls / spend | 2956 / **$45.5021** |
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 4.5 h ago; none live, newest failure 43 min ago** |
...
| | |
|---|---|
| cluster jobs | **990** (185 running, 805 queued) |
...
```
generated 2026-08-05 02:59 UTC | elapsed 7.24 d | 21.9 d to the Aug-27 stop
test tier: 13,597 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h      23 records      23.0 rec/h
    last  3 h     237 records      79.0 rec/h
    last 12 h    1951 records     162.6 rec/h
    last 24 h    4103 records     171.0 rec/h
    12 h rate is 39% from ONE line (test_leg_sonnet_5); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 70% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_sonnet_5, test_leg_qwen3_5_9b) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (163 rec/h); latest excludes cells already within
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30         240       0  GATED             GATED             barrier
      100       3,324       1  GATED             GATED             barrier>=30
      189       7,774       1  GATED             GATED             barrier>=30
      279      12,274       1  GATED             GATED             barrier>=30
      340      15,324       1  GATED             GATED             barrier>=30
      403      18,474       1  GATED             GATED             barrier>=30
      568      26,731      23  GATED             GATED             barrier>=30
    ...
    !! those 2 unit(s) are NOT in the rate's denominator and CANNOT be -- they have produced nothing at all, so no window contains them.
       Both columns are OPTIMISTIC by that share until those units start.
    !! 67% of the rung-568 backlog (17,994 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1480 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           4.6 d           6.5 d   throughput
      279           5.2 d           9.3 d   throughput
      340           6.2 d          11.1 d   throughput
      403           7.3 d          13.0 d   throughput
      568          10.1 d          18.1 d   throughput
...
```
2026-08-05T02:26:51Z  OK  records=15135 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  sweep=295.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T02:34:15Z  OK  records=15135 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=414.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T02:39:49Z  OK  records=15135 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=303.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T02:47:38Z  ATTN  records=15138 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  cores=1456  sweep=439.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T02:53:38Z  ATTN  records=15139 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=329.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T02:58:58Z  ATTN  records=15140 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=4.6m  drift=0  sci=OK  r115=22B  sweep=289.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
