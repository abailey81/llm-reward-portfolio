...
**Auto-generated 2026-08-06 12:08 UTC -- T+206h59m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+206h59m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **898** (104 running, 794 queued) |
| **cores computing** | **832** |
| **cores doing RUNG-RAISING work** | **22.9%** -- 192 of 840 cores (2 min old) |
...
```
generated 2026-08-06 12:09 UTC | elapsed 8.63 d | 20.5 d to the Aug-27 stop
test tier: 17,613 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    12 h rate is 98% from ONE line (test_leg_kimi_k3); 2 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       9,702      22  GATED             GATED             barrier>=30
      340      12,447      22  GATED             GATED             barrier>=30
      403      15,282      22  GATED             GATED             barrier>=30
      568      22,715      22  GATED             GATED             barrier>=30
    ...
    !! 83% of the rung-568 backlog (18,958 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
2026-08-06T12:03:35Z  OK  records=19149 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  sweep=557.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-06T12:07:06Z  OK  records=19152 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=180.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
