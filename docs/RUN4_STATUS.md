...
**Auto-generated 2026-08-06 14:27 UTC -- T+209h18m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+209h18m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=126; worst streak 2/240 (0.8% to fatal), ops on core, 1 min ago; LIVE, still failing** |
| transport timeouts (cumulative, ever) | 312 -- a level with no rate; read the row above |
...
| | |
|---|---|
| cluster jobs | **884** (90 running, 794 queued) |
| **cores computing** | **720** |
| **cores doing RUNG-RAISING work** | **22.6%** -- 184 of 816 cores (107 min old **STALE**) |
...
```
generated 2026-08-06 14:31 UTC | elapsed 8.72 d | 20.4 d to the Aug-27 stop
test tier: 17,751 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 56.5 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h     678 records      56.5 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30          85      32  GATED             GATED             unstarted:2-unit(s)-absent
      100       2,535      32  GATED             GATED             unstarted:2-unit(s)-absent>=30
      189       5,651      35  GATED             GATED             unstarted:2-unit(s)-absent>=30
      279       9,564     106  GATED             GATED             unstarted:2-unit(s)-absent>=30
      340      12,309     106  GATED             GATED             unstarted:2-unit(s)-absent>=30
      403      15,144     106  GATED             GATED             unstarted:2-unit(s)-absent>=30
      568      22,577     106  GATED             GATED             unstarted:2-unit(s)-absent>=30
