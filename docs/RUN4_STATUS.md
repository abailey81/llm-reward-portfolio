...
**Auto-generated 2026-08-13 02:54 UTC -- T+365h45m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+365h45m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| stalest driver log | **0 min (nemotron-3-super)** old (P218: the STALEST of the still-running lines, completed ladders excluded; above ~30 means that line has stopped progressing) |
| records archived | **33111** |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 33.2 h ago** |
...
| | |
|---|---|
| cluster jobs | **314** = 65 running + **183 ELIGIBLE** + 66 held by us + 0 held only by the site |
...
| **cores computing** | **520** |
| **cores doing RUNG-RAISING work** | **27.5%** -- 112 of 408 cores (80 min old) |
...
```
generated 2026-08-13 02:54 UTC | elapsed 15.24 d | 13.9 d to the Aug-27 stop
test tier: 31,568 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 15.2 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      32 records      32.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h      32 records      10.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     183 records      15.2 rec/h   usable
    last 24 h    1349 records      56.2 rec/h   usable
    12 h rate is 56% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (15 rec/h); latest excludes cells already within
    8 of the ceiling (15 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         392      32  2026-08-14 04:37  2026-08-14 04:37  yes
      403       2,413      32  2026-08-19 17:08  2026-08-19 17:08  yes
      568       8,760      32  2026-09-06 01:20  2026-09-06 01:20  NO
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @520 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           6.1 d           4.6 d   critical_chain
      189          10.4 d           6.5 d   throughput
      279          14.8 d           9.3 d   throughput
      340          17.8 d          11.1 d   throughput
      403          20.8 d          13.0 d   throughput
      568          28.9 d          18.1 d   throughput
...
> **What is holding it right now: test_leg_deepseek_v4_pro -- distributional has 25 HOLE(S) below its frontier 339 -- that is what caps this line**
...
```
2026-08-13T02:33:27Z  OK  records=33081 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=60.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
