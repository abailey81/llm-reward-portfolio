...
**Auto-generated 2026-08-12 02:52 UTC -- T+341h43m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+341h43m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **498** = 106 running + **64 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **848** |
| **cores doing RUNG-RAISING work** | **56.7%** -- 440 of 776 cores (79 min old) |
...
```
generated 2026-08-12 02:52 UTC | elapsed 14.24 d | 14.9 d to the Aug-27 stop
test tier: 30,211 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last  3 h     159 records      53.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,391      60  GATED             GATED             barrier>=279
      403       3,596      60  GATED             GATED             barrier>=279
      568      10,117      63  GATED             GATED             barrier>=279
