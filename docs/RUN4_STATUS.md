...
| **cores computing** | **528** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (15 min old) |
...
```
generated 2026-08-11 17:48 UTC | elapsed 13.86 d | 15.3 d to the Aug-27 stop
test tier: 29,702 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 125.2 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1503 records     125.2 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,897      98  GATED             GATED             barrier>=279
      403       4,102      98  GATED             GATED             barrier>=279
      568      10,626      98  GATED             GATED             barrier>=279
