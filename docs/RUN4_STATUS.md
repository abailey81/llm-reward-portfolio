...
**Auto-generated 2026-08-05 02:51 UTC -- T+173h42m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+173h42m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 4.4 h ago; none live, newest failure 35 min ago** |
...
| | |
|---|---|
| cluster jobs | **990** (183 running, 807 queued) |
| **cores computing** | **1464** |
...
```
generated 2026-08-05 02:51 UTC | elapsed 7.24 d | 21.9 d to the Aug-27 stop
test tier: 13,597 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h      26 records      26.0 rec/h
    last  3 h     257 records      85.7 rec/h
    last 12 h    1977 records     164.8 rec/h
    last 24 h    4136 records     172.3 rec/h
    12 h rate is 40% from ONE line (test_leg_sonnet_5); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 71% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_sonnet_5, test_leg_qwen3_5_9b) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (165 rec/h); latest excludes cells already within
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       3,324       2  GATED             GATED             barrier>=30
      189       7,774       2  GATED             GATED             barrier>=30
      279      12,274       2  GATED             GATED             barrier>=30
      340      15,324       2  GATED             GATED             barrier>=30
      403      18,474       2  GATED             GATED             barrier>=30
      568      26,731      26  GATED             GATED             barrier>=30
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1464 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           4.6 d           6.5 d   throughput
      279           5.3 d           9.3 d   throughput
      340           6.3 d          11.1 d   throughput
      403           7.4 d          13.0 d   throughput
      568          10.2 d          18.1 d   throughput
...
```
2026-08-05T02:17:00Z  ATTN  records=15129 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.7m  drift=0  sci=OK  r115=22B  sweep=281.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T02:21:25Z  OK  records=15132 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  cores=1416  sweep=234.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
