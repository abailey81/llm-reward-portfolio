...
**Auto-generated 2026-08-11 17:56 UTC -- T+332h47m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+332h47m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 5.4 h ago; none live, newest failure 17 min ago** |
...
| | |
|---|---|
| cluster jobs | **454** = 66 running + **60 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **528** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (23 min old) |
...
```
generated 2026-08-11 17:57 UTC | elapsed 13.87 d | 15.3 d to the Aug-27 stop
test tier: 29,714 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 124.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     412 records     137.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1498 records     124.8 rec/h   usable
    last 24 h    2770 records     115.4 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         128      14  GATED             GATED             barrier
      340       1,885      96  GATED             GATED             barrier>=279
      403       4,090      96  GATED             GATED             barrier>=279
      568      10,614      96  GATED             GATED             barrier>=279
...
> **What is holding it right now: test -- bayes_opt has 113 HOLE(S) below its frontier 406 -- that is what caps this line**
...
2026-08-11T17:53:10Z  OK  records=31227 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  sweep=788.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
