...
**Auto-generated 2026-08-12 13:31 UTC -- T+352h22m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+352h22m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 19.8 h ago** |
...
| | |
|---|---|
| cluster jobs | **368** = 68 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **544** |
| **cores doing RUNG-RAISING work** | **100.0%** -- 600 of 600 cores (118 min old **STALE**) |
...
```
generated 2026-08-12 13:32 UTC | elapsed 14.68 d | 14.4 d to the Aug-27 stop
test tier: 31,211 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 90.6 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1087 records      90.6 rec/h   usable
    last 24 h    2143 records      89.3 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         565     133  2026-08-12 19:46  2026-08-12 19:46  yes
      403       2,770     133  2026-08-13 20:07  2026-08-13 20:07  yes
      568       9,117     133  2026-08-16 18:11  2026-08-16 18:11  yes
...
currently at **332 seeds each** (read live from the archive on this publish). !! THE
