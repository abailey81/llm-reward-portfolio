...
**Auto-generated 2026-08-11 21:56 UTC -- T+336h47m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+336h47m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| **cores computing** | **640** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (144 min old **STALE**) |
...
```
generated 2026-08-11 21:57 UTC | elapsed 14.03 d | 15.1 d to the Aug-27 stop
test tier: 30,005 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,594      62  GATED             GATED             barrier>=279
      403       3,799      62  GATED             GATED             barrier>=279
      568      10,323      62  GATED             GATED             barrier>=279
...
> **What is holding it right now: test_leg_deepseek_v4_pro -- scalar_cvar5 has 32 HOLE(S) below its frontier 339 -- that is what caps this line**
