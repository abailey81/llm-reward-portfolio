...
| **cores computing** | **592** |
| **cores doing RUNG-RAISING work** | **28.8%** -- 168 of 584 cores (16 min old) |
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340      11,696      62  GATED             GATED             no-records:test/distributional,test/scalar>=30
      403      14,531      62  GATED             GATED             no-records:test/distributional,test/scalar>=30
      568      21,964      62  GATED             GATED             no-records:test/distributional,test/scalar>=30
