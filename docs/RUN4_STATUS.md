...
**Auto-generated 2026-08-05 03:54 UTC -- T+174h45m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+174h45m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| stalest driver log | **4 min (sonnet-5)** old (P218: the STALEST of the still-running lines, completed ladders excluded; above ~30 means that line has stopped progressing) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 5.4 h ago; none live, newest failure 1.6 h ago** |
...
```
generated 2026-08-05 03:54 UTC | elapsed 7.28 d | 21.8 d to the Aug-27 stop
test tier: 13,604 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h       7 records       7.0 rec/h
    last  3 h     136 records      45.3 rec/h
    last 12 h    1804 records     150.3 rec/h
    last 24 h    3993 records     166.4 rec/h
    12 h rate is 34% from ONE line (test_leg_qwen3_5_9b); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 68% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_qwen3_5_9b, test_leg_sonnet_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      568      26,724       7  GATED             GATED             barrier>=30
...
2026-08-05T03:48:09Z  OK  records=15147 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.5m  drift=0  sci=OK  r115=22B  sweep=321.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T03:54:17Z  OK  records=15147 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  cores=1504  sweep=338.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
