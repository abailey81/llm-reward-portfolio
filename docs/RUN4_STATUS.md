...
**Auto-generated 2026-08-12 09:19 UTC -- T+348h10m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+348h10m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 15.7 h ago** |
...
| | |
|---|---|
| cluster jobs | **428** = 100 running + **0 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **800** |
| **cores doing RUNG-RAISING work** | **95.7%** -- 880 of 920 cores (105 min old **STALE**) |
...
```
generated 2026-08-12 09:20 UTC | elapsed 14.51 d | 14.6 d to the Aug-27 stop
test tier: 30,726 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 63.9 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      61 records      61.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     221 records      73.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     767 records      63.9 rec/h   usable
    last 24 h    2083 records      86.8 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,050      61  2026-08-13 01:46  2026-08-13 01:46  yes
      403       3,255      61  2026-08-14 12:16  2026-08-14 12:16  yes
      568       9,602      61  2026-08-18 15:34  2026-08-18 15:34  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @800 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           6.8 d           6.5 d   throughput
      279           9.6 d           9.3 d   throughput
      340          11.5 d          11.1 d   throughput
      403          13.5 d          13.0 d   throughput
      568          18.8 d          18.1 d   throughput
...
currently at **310 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-12T08:57:25Z  OK  records=32240 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.5m  drift=0  sci=OK  r115=22B  sweep=57.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T08:58:48Z  OK  records=32242 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.5m  drift=0  sci=OK  r115=22B  sweep=52.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T09:00:19Z  OK  records=32244 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=60.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T09:01:43Z  OK  records=32244 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=0.9m  drift=0  sci=OK  r115=22B  sweep=54.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T09:03:08Z  OK  records=32244 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.3m  drift=0  sci=OK  r115=22B  sweep=54.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T09:17:23Z  OK  records=32261 (+17)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=4.5m  drift=0  sci=OK  r115=22B  cores=800  sweep=825.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
