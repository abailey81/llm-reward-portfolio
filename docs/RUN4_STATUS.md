...
**Auto-generated 2026-08-11 19:32 UTC -- T+334h23m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+334h23m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 1.9 h ago; none live, newest failure 1.9 h ago** |
...
| | |
|---|---|
| cluster jobs | **437** = 64 running + **45 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **512** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (119 min old **STALE**) |
...
```
generated 2026-08-11 19:33 UTC | elapsed 13.93 d | 15.2 d to the Aug-27 stop
test tier: 29,825 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 116.5 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1398 records     116.5 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    8 of the ceiling (116 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,774      45  GATED             GATED             barrier>=279
      403       3,979      45  GATED             GATED             barrier>=279
      568      10,503      45  GATED             GATED             barrier>=279
...
2026-08-11T19:19:25Z  ATTN  records=31337 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.7m  drift=0  sci=OK  r115=22B  sweep=739.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T19:31:50Z  OK  records=31364 (+27)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  cores=512  sweep=714.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T19:33:16Z  OK  records=31365 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=56.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
