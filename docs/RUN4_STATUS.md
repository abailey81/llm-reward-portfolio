...
**Auto-generated 2026-08-05 02:04 UTC -- T+172h55m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+172h55m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 3.6 h ago; none live, newest failure 2.5 h ago** |
...
| | |
|---|---|
| cluster jobs | **992** (174 running, 817 queued) |
| **cores computing** | **1392** |
...
```
generated 2026-08-05 02:04 UTC | elapsed 7.21 d | 21.9 d to the Aug-27 stop
test tier: 13,580 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h      89 records      89.0 rec/h
    last  3 h     376 records     125.3 rec/h
    last 12 h    2140 records     178.3 rec/h
    last 24 h    4226 records     176.1 rec/h
    12 h rate is 45% from ONE line (test_leg_sonnet_5); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 68% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_sonnet_5, test_leg_qwen3_5_9b) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (178 rec/h); latest excludes cells already within
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       3,325      15  GATED             GATED             barrier>=30
      189       7,775      15  GATED             GATED             barrier>=30
      279      12,275      15  GATED             GATED             barrier>=30
      340      15,325      15  GATED             GATED             barrier>=30
      403      18,475      15  GATED             GATED             barrier>=30
      568      26,748      89  GATED             GATED             barrier>=30
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1392 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           4.6 d           6.5 d   throughput
      279           5.5 d           9.3 d   throughput
      340           6.6 d          11.1 d   throughput
      403           7.8 d          13.0 d   throughput
      568          10.8 d          18.1 d   throughput
...
```
2026-08-05T01:34:43Z  OK  records=15082 (+12)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.2m  drift=0  sci=OK  r115=22B  cores=1416  sweep=303.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T01:38:57Z  OK  records=15087 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=223.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T01:44:44Z  OK  records=15094 (+7)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.9m  drift=0  sci=OK  r115=22B  sweep=316.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T01:51:44Z  OK  records=15108 (+14)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  sweep=389.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T01:57:11Z  ATTN  records=15116 (+8)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  cores=1400  sweep=297.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T02:02:14Z  ATTN  records=15117 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  sweep=271.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
