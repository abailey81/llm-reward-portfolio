...
| | |
|---|---|
| cluster jobs | **835** = 102 running + **57 ELIGIBLE** + 546 held by us + 130 held only by the site |
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340      11,641      31  GATED             GATED             no-records:test/distributional,test/scalar>=30
      403      14,476      31  GATED             GATED             no-records:test/distributional,test/scalar>=30
      568      21,909      31  GATED             GATED             no-records:test/distributional,test/scalar>=30
