...
**Auto-generated 2026-08-11 17:08 UTC -- T+331h59m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+331h59m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 4.6 h ago; none live, newest failure 4.6 h ago** |
...
| | |
|---|---|
| cluster jobs | **461** = 65 running + **68 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **520** |
| **cores doing RUNG-RAISING work** | **36.1%** -- 240 of 664 cores (95 min old **STALE**) |
...
```
generated 2026-08-11 17:09 UTC | elapsed 13.83 d | 15.3 d to the Aug-27 stop
test tier: 29,640 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 128.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     106 records     106.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     477 records     159.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1546 records     128.8 rec/h   usable
    last 24 h    2725 records     113.5 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (129 rec/h); latest excludes cells already within
    8 of the ceiling (129 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         131      29  2026-08-11 18:10  2026-08-11 18:10  yes
      340       1,959     106  2026-08-12 08:21  2026-08-12 08:21  yes
      403       4,164     106  2026-08-13 01:28  2026-08-13 01:28  yes
      568      10,688     106  2026-08-15 04:06  2026-08-15 04:06  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 21% of the rung-568 backlog (2,266 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test -- baseline_differential_sharpe has 114 HOLE(S) below its frontier 406 -- that is what caps this line**
...
```
2026-08-11T16:41:48Z  OK  records=31131 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=63.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T16:43:12Z  OK  records=31135 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=54.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T16:44:34Z  OK  records=31136 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.3m  drift=0  sci=OK  r115=22B  sweep=51.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
