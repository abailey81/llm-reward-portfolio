...
**Auto-generated 2026-08-11 15:38 UTC -- T+330h29m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+330h29m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **30978** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 0.7 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
| LLM calls / spend | 2956 / **$45.5021** |
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 3.1 h ago; none live, newest failure 3.1 h ago** |
...
| | |
|---|---|
| cluster jobs | **489** = 85 running + **76 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **680** |
| **cores doing RUNG-RAISING work** | **36.1%** -- 240 of 664 cores (5 min old) |
...
```
generated 2026-08-11 15:39 UTC | elapsed 13.77 d | 15.3 d to the Aug-27 stop
test tier: 29,436 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 133.9 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     192 records     192.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     459 records     153.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1607 records     133.9 rec/h   usable
    last 24 h    2593 records     108.0 rec/h   usable
    12 h rate is 94% from ONE line (test); 2 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (134 rec/h); latest excludes cells already within
    8 of the ceiling (134 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         160      61  2026-08-11 16:50  2026-08-11 16:50  yes
      340       2,163     192  2026-08-12 07:48  2026-08-12 07:48  yes
      403       4,368     192  2026-08-13 00:16  2026-08-13 00:16  yes
      568      10,892     192  2026-08-15 00:59  2026-08-15 00:59  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 34% of the rung-568 backlog (3,743 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test -- tpe has 124 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **284 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T15:15:23Z  OK  records=30905 (+8)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  sweep=50.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T15:16:44Z  OK  records=30909 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=51.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T15:18:04Z  OK  records=30913 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=49.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T15:19:25Z  OK  records=30922 (+9)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=51.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T15:20:45Z  OK  records=30929 (+7)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  sweep=49.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T15:22:05Z  OK  records=30933 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=50.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
