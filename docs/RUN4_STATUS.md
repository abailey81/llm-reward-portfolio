...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 59.5 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h     714 records      59.5 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       9,709      25  GATED             GATED             barrier>=30
      340      12,454      25  GATED             GATED             barrier>=30
      403      15,289      25  GATED             GATED             barrier>=30
      568      22,722      25  GATED             GATED             barrier>=30
