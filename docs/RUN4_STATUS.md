...
**Auto-generated 2026-08-05 11:15 UTC -- T+182h06m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+182h06m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on core, 4.0 h ago; none live, newest failure 25 min ago** |
...
| | |
|---|---|
| cluster jobs | **989** (163 running, 826 queued) |
| **cores computing** | **1304** |
...
```
generated 2026-08-05 11:15 UTC | elapsed 7.59 d | 21.5 d to the Aug-27 stop
test tier: 14,930 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     139 records     139.0 rec/h
    last  3 h     513 records     171.0 rec/h
    last 12 h    1686 records     140.5 rec/h
    last 24 h    3948 records     164.5 rec/h
    12 h rate is 55% from ONE line (test_leg_haiku_4_5); 5 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 20% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_qwen3_5_9b, test_leg_sonnet_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (140 rec/h); latest excludes cells already within
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30         180      18  GATED             GATED             barrier
      100       2,980      18  GATED             GATED             barrier>=30
      189       6,841      37  GATED             GATED             barrier>=30
      279      10,948     139  GATED             GATED             barrier>=30
      340      13,998     139  GATED             GATED             barrier>=30
      403      17,148     139  GATED             GATED             barrier>=30
      568      25,398     139  GATED             GATED             barrier>=30
    ...
    !! 81% of the rung-568 backlog (20,624 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1304 cores      @830 cores   binding
...
```
2026-08-05T10:59:46Z  OK  records=16436 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  cores=1352  sweep=454.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T11:01:54Z  OK  records=16441 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  sweep=98.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T11:04:53Z  OK  records=16450 (+9)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  sweep=148.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T11:10:30Z  OK  records=16462 (+12)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=307.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T11:13:20Z  OK  records=16465 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.8m  drift=0  sci=OK  r115=22B  sweep=140.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T11:15:36Z  OK  records=16470 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=0.6m  drift=0  sci=OK  r115=22B  sweep=105.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
