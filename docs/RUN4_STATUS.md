...
**Auto-generated 2026-08-12 09:00 UTC -- T+347h51m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+347h51m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 15.3 h ago** |
...
| | |
|---|---|
| cluster jobs | **429** = 101 running + **0 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **808** |
| **cores doing RUNG-RAISING work** | **95.7%** -- 880 of 920 cores (86 min old) |
...
```
generated 2026-08-12 09:01 UTC | elapsed 14.49 d | 14.6 d to the Aug-27 stop
test tier: 30,701 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 62.9 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     239 records      79.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     755 records      62.9 rec/h   usable
    last 24 h    2094 records      87.2 rec/h   usable
    12 h rate is 27% from ONE line (test_leg_glm_5_2); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (63 rec/h); latest excludes cells already within
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,075      64  2026-08-13 02:06  2026-08-13 02:06  yes
      403       3,280      64  2026-08-14 13:09  2026-08-14 13:09  yes
      568       9,627      64  2026-08-18 18:01  2026-08-18 18:01  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @808 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           6.7 d           6.5 d   throughput
      279           9.5 d           9.3 d   throughput
      340          11.4 d          11.1 d   throughput
      403          13.4 d          13.0 d   throughput
      568          18.6 d          18.1 d   throughput
...
currently at **308 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-12T08:39:17Z  OK  records=32211 (+11)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.3m  drift=0  sci=OK  r115=22B  sweep=1402.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T08:54:27Z  OK  records=32238 (+27)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.3m  drift=0  sci=OK  r115=22B  cores=816  sweep=880.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T08:55:58Z  OK  records=32238 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  sweep=60.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
