...
**Auto-generated 2026-08-12 04:24 UTC -- T+343h15m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+343h15m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **483** = 116 running + **39 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **928** |
| **cores doing RUNG-RAISING work** | **76.4%** -- 672 of 880 cores (51 min old) |
...
```
generated 2026-08-12 04:25 UTC | elapsed 14.30 d | 14.8 d to the Aug-27 stop
test tier: 30,304 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 63.0 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      46 records      46.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     187 records      62.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     756 records      63.0 rec/h   usable
    last 24 h    2362 records      98.4 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      568      10,024      46  GATED             GATED             barrier>=279
