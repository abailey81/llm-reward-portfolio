...
**Auto-generated 2026-08-11 18:26 UTC -- T+333h17m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+333h17m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 5.9 h ago; none live, newest failure 47 min ago** |
...
| | |
|---|---|
| cluster jobs | **448** = 65 running + **55 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **520** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (54 min old) |
...
```
generated 2026-08-11 18:27 UTC | elapsed 13.89 d | 15.2 d to the Aug-27 stop
test tier: 29,758 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 124.5 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1494 records     124.5 rec/h   usable
    last 24 h    2787 records     116.1 rec/h   usable
    12 h rate is 91% from ONE line (test); 2 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (124 rec/h); latest excludes cells already within
    8 of the ceiling (124 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,841      90  GATED             GATED             barrier>=279
      403       4,046      90  GATED             GATED             barrier>=279
      568      10,570      90  GATED             GATED             barrier>=279
...
2026-08-11T18:10:09Z  OK  records=31282 (+8)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  sweep=52.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T18:21:37Z  OK  records=31294 (+12)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  sweep=657.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T18:23:26Z  OK  records=31294 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.7m  drift=0  sci=OK  r115=22B  cores=520  sweep=78.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
