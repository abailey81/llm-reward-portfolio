...
**Auto-generated 2026-08-11 03:24 UTC -- T+318h15m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+318h15m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 16.6 h ago** |
...
| | |
|---|---|
| cluster jobs | **698** = 145 running + **199 ELIGIBLE** + 354 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **100.0%** -- 1120 of 1120 cores (106 min old **STALE**) |
...
```
generated 2026-08-11 03:25 UTC | elapsed 13.26 d | 15.9 d to the Aug-27 stop
test tier: 27,789 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last 24 h    2315 records      96.5 rec/h   usable
    12 h rate is 31% from ONE line (test_leg_deepseek_v4_pro); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       1,675      67  2026-08-12 00:14  2026-08-12 00:14  yes
      340       3,810      67  2026-08-13 02:47  2026-08-13 02:47  yes
      403       6,015      67  2026-08-14 06:13  2026-08-14 06:13  yes
      568      12,539      67  2026-08-17 15:20  2026-08-17 15:20  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 32% of the rung-568 backlog (4,055 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
