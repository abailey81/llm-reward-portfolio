...
**Auto-generated 2026-08-12 15:10 UTC -- T+354h01m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+354h01m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 21.5 h ago** |
...
| | |
|---|---|
| cluster jobs | **342** = 42 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **336** |
| **cores doing RUNG-RAISING work** | **77.9%** -- 424 of 544 cores (98 min old **STALE**) |
...
```
generated 2026-08-12 15:11 UTC | elapsed 14.75 d | 14.4 d to the Aug-27 stop
test tier: 31,414 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 97.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     131 records     131.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     371 records     123.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1165 records      97.1 rec/h   usable
    last 24 h    2064 records      86.0 rec/h   usable
    12 h rate is 68% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         472      25  2026-08-12 20:03  2026-08-12 20:03  yes
      403       2,567     131  2026-08-13 17:38  2026-08-13 17:38  yes
      568       8,914     131  2026-08-16 11:00  2026-08-16 11:00  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @336 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           9.4 d           4.6 d   critical_chain
      189          16.1 d           6.5 d   throughput
      279          22.9 d           9.3 d   throughput
      340          27.5 d          11.1 d   throughput
      403          32.2 d          13.0 d   throughput
      568          44.7 d          18.1 d   throughput
