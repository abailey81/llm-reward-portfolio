...
**Auto-generated 2026-08-06 06:02 UTC -- T+200h53m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+200h53m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on nemotron-3-super, 6.0 h ago; none live, newest failure 6.0 h ago** |
...
| | |
|---|---|
| cluster jobs | **943** (71 running, 872 queued) |
| **cores computing** | **568** |
...
```
generated 2026-08-06 06:02 UTC | elapsed 8.37 d | 20.7 d to the Aug-27 stop
test tier: 17,284 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     137 records     137.0 rec/h
    last  3 h     206 records      68.7 rec/h
    last 12 h    1285 records     107.1 rec/h
    last 24 h    3403 records     141.8 rec/h
    12 h rate is 39% from ONE line (test_leg_kimi_k3); 4 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 33% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_haiku_4_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      189       5,983     137  GATED             GATED             barrier>=30
      279      10,031     137  GATED             GATED             barrier>=30
      340      12,776     137  GATED             GATED             barrier>=30
      403      15,611     137  GATED             GATED             barrier>=30
      568      23,044     137  GATED             GATED             barrier>=30
