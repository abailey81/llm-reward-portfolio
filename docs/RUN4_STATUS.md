...
**Auto-generated 2026-08-11 19:39 UTC -- T+334h30m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+334h30m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 2.0 h ago; none live, newest failure 2.0 h ago** |
...
| | |
|---|---|
| cluster jobs | **436** = 63 running + **45 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **504** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (6 min old) |
...
```
generated 2026-08-11 19:40 UTC | elapsed 13.94 d | 15.2 d to the Aug-27 stop
test tier: 29,835 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,764      48  GATED             GATED             barrier>=279
      403       3,969      48  GATED             GATED             barrier>=279
      568      10,493      48  GATED             GATED             barrier>=279
...
currently at **296 seeds each** (read live from the archive on this publish). !! THE
