...
| transport health | **timeouts 6h=156; worst streak 2/240 (0.8% to fatal), ops on core, 86 min ago; none live, newest failure 86 min ago** |
...
| | |
|---|---|
| cluster jobs | **870** (49 running, 821 queued) |
| **cores computing** | **392** |
...
test tier: 18,119 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 85.6 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     245 records     245.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       9,196     245  GATED             GATED             unstarted:2-unit(s)-absent>=30
      340      11,941     245  GATED             GATED             unstarted:2-unit(s)-absent>=30
      403      14,776     245  GATED             GATED             unstarted:2-unit(s)-absent>=30
      568      22,209     245  GATED             GATED             unstarted:2-unit(s)-absent>=30
