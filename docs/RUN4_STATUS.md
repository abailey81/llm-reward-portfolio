...
**Auto-generated 2026-08-11 21:20 UTC -- T+336h11m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+336h11m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **31501** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 0.4 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
| LLM calls / spend | 2956 / **$45.5021** |
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 3.7 h ago; none live, newest failure 3.7 h ago** |
...
| | |
|---|---|
| cluster jobs | **420** = 68 running + **24 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **544** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (107 min old **STALE**) |
...
```
generated 2026-08-11 21:21 UTC | elapsed 14.01 d | 15.1 d to the Aug-27 stop
test tier: 29,959 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 109.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      59 records      59.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     208 records      69.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1316 records     109.7 rec/h   usable
    last 24 h    2752 records     114.7 rec/h   usable
    12 h rate is 88% from ONE line (test); 2 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (110 rec/h); latest excludes cells already within
    8 of the ceiling (110 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         128       0  GATED             GATED             barrier
      340       1,640      59  GATED             GATED             barrier>=279
      403       3,845      59  GATED             GATED             barrier>=279
      568      10,369      59  GATED             GATED             barrier>=279
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 36% of the rung-568 backlog (3,767 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test_leg_deepseek_v4_pro -- placebo_shuffled has 31 HOLE(S) below its frontier 335 -- that is what caps this line**
...
currently at **303 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T20:56:25Z  OK  records=31485 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  cores=520  sweep=63.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T20:57:56Z  OK  records=31486 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  cores=520  sweep=60.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T20:59:16Z  OK  records=31486 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.2m  drift=0  sci=OK  r115=22B  sweep=49.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T21:00:38Z  OK  records=31487 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=52.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T21:01:59Z  OK  records=31488 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.4m  drift=0  sci=OK  r115=22B  sweep=50.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T21:03:20Z  OK  records=31489 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=51.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
