...
**Auto-generated 2026-08-12 15:26 UTC -- T+354h17m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+354h17m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 21.8 h ago** |
...
| | |
|---|---|
| cluster jobs | **337** = 37 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **296** |
| **cores doing RUNG-RAISING work** | **77.9%** -- 424 of 544 cores (114 min old **STALE**) |
...
```
generated 2026-08-12 15:27 UTC | elapsed 14.76 d | 14.4 d to the Aug-27 stop
test tier: 31,429 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 97.3 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     118 records     118.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     362 records     120.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1168 records      97.3 rec/h   usable
    last 24 h    2024 records      84.3 rec/h   usable
    12 h rate is 69% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         472      14  2026-08-12 20:18  2026-08-12 20:18  yes
      403       2,552     118  2026-08-13 17:40  2026-08-13 17:40  yes
      568       8,899     118  2026-08-16 10:53  2026-08-16 10:53  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 28% of the rung-568 backlog (2,532 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @296 cores      @830 cores   binding
       30           4.7 d           4.6 d   critical_chain
      100          10.7 d           4.6 d   critical_chain
      189          18.3 d           6.5 d   throughput
      279          26.0 d           9.3 d   throughput
      340          31.2 d          11.1 d   throughput
      403          36.6 d          13.0 d   throughput
      568          50.7 d          18.1 d   throughput
...
currently at **344 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-12T15:13:59Z  OK  records=32959 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  sweep=61.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T15:15:24Z  OK  records=32960 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.2m  drift=0  sci=OK  r115=22B  sweep=54.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
