...
**Auto-generated 2026-08-11 17:32 UTC -- T+332h23m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+332h23m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 5.0 h ago; none live, newest failure 5.0 h ago** |
...
| | |
|---|---|
| cluster jobs | **458** = 63 running + **67 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **504** |
| **cores doing RUNG-RAISING work** | **36.1%** -- 240 of 664 cores (119 min old **STALE**) |
...
```
generated 2026-08-11 17:32 UTC | elapsed 13.85 d | 15.3 d to the Aug-27 stop
test tier: 29,673 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 125.0 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     108 records     108.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    ...
    last 12 h    1500 records     125.0 rec/h   usable
    last 24 h    2746 records     114.4 rec/h   usable
    12 h rate is 96% from ONE line (test); 3 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (125 rec/h); latest excludes cells already within
    8 of the ceiling (125 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         128      32  2026-08-11 18:34  2026-08-11 18:34  yes
      340       1,926     108  2026-08-12 08:57  2026-08-12 08:57  yes
      403       4,131     108  2026-08-13 02:35  2026-08-13 02:35  yes
      568      10,655     108  2026-08-15 06:47  2026-08-15 06:47  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 33% of the rung-568 backlog (3,467 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
2026-08-11T17:21:50Z  OK  records=31203 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.3m  drift=0  sci=OK  r115=22B  sweep=52.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T17:23:11Z  OK  records=31204 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.3m  drift=0  sci=OK  r115=22B  sweep=51.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T17:24:36Z  OK  records=31206 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.9m  drift=0  sci=OK  r115=22B  sweep=54.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T17:25:59Z  OK  records=31210 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=52.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
