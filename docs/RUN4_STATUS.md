...
**Auto-generated 2026-08-11 18:01 UTC -- T+332h52m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+332h52m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 5.5 h ago; none live, newest failure 21 min ago** |
...
| | |
|---|---|
| cluster jobs | **452** = 65 running + **59 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **520** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (28 min old) |
...
```
generated 2026-08-11 18:02 UTC | elapsed 13.87 d | 15.2 d to the Aug-27 stop
test tier: 29,729 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last  3 h     408 records     136.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         128       9  GATED             GATED             barrier
      340       1,870     104  GATED             GATED             barrier>=279
      403       4,075     104  GATED             GATED             barrier>=279
      568      10,599     104  GATED             GATED             barrier>=279
