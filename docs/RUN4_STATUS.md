...
**Auto-generated 2026-08-13 03:21 UTC -- T+366h12m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+366h12m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| stalest driver log | **3 min (qwen3_6-27b)** old (P218: the STALEST of the still-running lines, completed ladders excluded; above ~30 means that line has stopped progressing) |
| records archived | **33127** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in IN PROGRESS or PASSED). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
| LLM calls / spend | 2956 / **$45.5021** |
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 33.7 h ago** |
| transport timeouts (cumulative, ever) | 356 -- a level with no rate; read the row above |
...
| | |
|---|---|
| cluster jobs | **314** = 68 running + **180 ELIGIBLE** + 66 held by us + 0 held only by the site |
...
| **cores computing** | **544** |
| **cores doing RUNG-RAISING work** | **27.5%** -- 112 of 408 cores (108 min old **STALE**) |
...
```
generated 2026-08-13 03:22 UTC | elapsed 15.26 d | 13.9 d to the Aug-27 stop
test tier: 31,584 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 13.3 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      48 records      48.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h      48 records      16.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     160 records      13.3 rec/h   usable
    last 24 h    1328 records      55.3 rec/h   usable
    12 h rate is 40% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (13 rec/h); latest excludes cells already within
    8 of the ceiling (13 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279           0       0  REACHED           REACHED           yes
      340         376      48  2026-08-14 07:34  2026-08-14 07:34  yes
      403       2,397      48  2026-08-20 15:09  2026-08-20 15:09  yes
      568       8,744      48  2026-09-09 11:10  2026-09-09 11:10  NO
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 7% of the rung-568 backlog (572 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @544 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           5.8 d           4.6 d   critical_chain
      189          10.0 d           6.5 d   throughput
      279          14.1 d           9.3 d   throughput
      340          17.0 d          11.1 d   throughput
      403          19.9 d          13.0 d   throughput
      568          27.6 d          18.1 d   throughput
...
> **What is holding it right now: test_leg_deepseek_v4_pro -- distributional has 21 HOLE(S) below its frontier 339 -- that is what caps this line**
...
|---|---|---|---|---|
| nemotron_3_super | **310** | 312 | 5 |  |
| glm_5_2 | **314** | 315 | 5 |  |
| deepseek_v4_pro | **319** | 320 | 5 |  |
| test | **350** | 350 | 9 |  |
| kimi_k3 | **471** | 474 | 5 |  |
| qwen3_6_27b | **548** | 549 | 5 |  |
...
currently at **348 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-13T02:36:26Z  OK  records=33083 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=60.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-13T02:37:57Z  OK  records=33084 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.3m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=60.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-13T02:39:27Z  OK  records=33086 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.8m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=60.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-13T02:54:28Z  OK  records=33111 (+25)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=BLIND(deep_audit)  r115=22B  cores=520  sweep=870.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-13T03:07:11Z  OK  records=33111 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.8m  drift=0  sci=BLIND(deep_audit)  r115=22B  cores=520  sweep=732.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-13T03:21:11Z  OK  records=33127 (+16)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.4m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=810.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
