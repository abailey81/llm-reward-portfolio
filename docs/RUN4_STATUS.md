...
**Auto-generated 2026-08-12 14:50 UTC -- T+353h41m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+353h41m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 21.2 h ago** |
...
| | |
|---|---|
| cluster jobs | **346** = 46 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **368** |
| **cores doing RUNG-RAISING work** | **77.9%** -- 424 of 544 cores (77 min old) |
...
```
generated 2026-08-12 14:51 UTC | elapsed 14.74 d | 14.4 d to the Aug-27 stop
test tier: 31,370 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 96.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     134 records     134.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     352 records     117.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1160 records      96.7 rec/h   usable
    last 24 h    2085 records      86.9 rec/h   usable
    12 h rate is 65% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (97 rec/h); latest excludes cells already within
    8 of the ceiling (97 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         472      68  2026-08-12 19:44  2026-08-12 19:44  yes
      403       2,611     134  2026-08-13 17:51  2026-08-13 17:51  yes
      568       8,958     134  2026-08-16 11:31  2026-08-16 11:31  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 20% of the rung-568 backlog (1,752 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @368 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           8.6 d           4.6 d   critical_chain
      189          14.7 d           6.5 d   throughput
      279          20.9 d           9.3 d   throughput
      340          25.1 d          11.1 d   throughput
      403          29.4 d          13.0 d   throughput
      568          40.8 d          18.1 d   throughput
...
currently at **342 seeds each** (read live from the archive on this publish). !! THE
...
## Monitoring -- the cycle (**last monitoring cycle was 17 min ago -- the loop has lapsed**)
...
```
2026-08-12T14:26:15Z  OK  records=32852 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.7m  drift=0  sci=OK  r115=22B  sweep=58.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T14:27:43Z  OK  records=32853 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.2m  drift=0  sci=OK  r115=22B  sweep=56.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T14:29:06Z  OK  records=32854 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.5m  drift=0  sci=OK  r115=22B  sweep=53.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T14:30:32Z  OK  records=32863 (+9)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=55.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T14:31:59Z  OK  records=32870 (+7)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.6m  drift=0  sci=OK  r115=22B  sweep=56.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T14:33:26Z  OK  records=32870 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=57.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
