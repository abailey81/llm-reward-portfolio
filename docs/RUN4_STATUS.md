...
**Auto-generated 2026-08-11 14:08 UTC -- T+328h59m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+328h59m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 1.6 h ago; none live, newest failure 1.6 h ago** |
...
| | |
|---|---|
| cluster jobs | **522** = 116 running + **78 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **928** |
| **cores doing RUNG-RAISING work** | **59.0%** -- 576 of 976 cores (36 min old) |
...
```
generated 2026-08-11 14:09 UTC | elapsed 13.71 d | 15.4 d to the Aug-27 stop
test tier: 29,159 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 120.6 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     336 records     112.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1447 records     120.6 rec/h   usable
    last 24 h    2446 records     101.9 rec/h   usable
    12 h rate is 86% from ONE line (test); 2 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         305     123  2026-08-11 16:41  2026-08-11 16:41  yes
      340       2,440     123  2026-08-12 10:23  2026-08-12 10:23  yes
      403       4,645     123  2026-08-13 04:40  2026-08-13 04:40  yes
      568      11,169     123  2026-08-15 10:46  2026-08-15 10:46  yes
...
> **What is holding it right now: test -- baseline_return_minus_turnover has 138 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **269 seeds each** (read live from the archive on this publish). !! THE
...
2026-08-11T14:01:54Z  OK  records=30687 (+34)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.9m  drift=0  sci=OK  r115=22B  sweep=718.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
