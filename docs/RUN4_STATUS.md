...
**Auto-generated 2026-08-12 15:18 UTC -- T+354h09m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+354h09m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 21.6 h ago** |
...
| | |
|---|---|
| cluster jobs | **341** = 41 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **328** |
| **cores doing RUNG-RAISING work** | **77.9%** -- 424 of 544 cores (105 min old **STALE**) |
...
```
generated 2026-08-12 15:19 UTC | elapsed 14.76 d | 14.4 d to the Aug-27 stop
test tier: 31,421 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 97.4 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     119 records     119.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     364 records     121.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1169 records      97.4 rec/h   usable
    last 24 h    2036 records      84.8 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         472      16  2026-08-12 20:09  2026-08-12 20:09  yes
      403       2,560     119  2026-08-13 17:35  2026-08-13 17:35  yes
      568       8,907     119  2026-08-16 10:44  2026-08-16 10:44  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 23% of the rung-568 backlog (2,013 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @328 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           9.7 d           4.6 d   critical_chain
      189          16.5 d           6.5 d   throughput
      279          23.5 d           9.3 d   throughput
      340          28.2 d          11.1 d   throughput
      403          33.0 d          13.0 d   throughput
      568          45.7 d          18.1 d   throughput
...
```
2026-08-12T14:57:29Z  OK  records=32895 (+25)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.5m  drift=0  sci=OK  r115=22B  sweep=1413.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T15:12:27Z  OK  records=32957 (+62)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  cores=336  sweep=867.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
