...
| | |
|---|---|
| cluster jobs | **939** (81 running, 858 queued) |
| **cores computing** | **648** |
...
test tier: 17,315 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     149 records     149.0 rec/h
    last  3 h     236 records      78.7 rec/h
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      189       5,952     149  GATED             GATED             barrier>=30
      279      10,000     149  GATED             GATED             barrier>=30
      340      12,745     149  GATED             GATED             barrier>=30
      403      15,580     149  GATED             GATED             barrier>=30
      568      23,013     149  GATED             GATED             barrier>=30
