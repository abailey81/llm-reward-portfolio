...
**Auto-generated 2026-08-11 15:25 UTC -- T+330h17m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+330h17m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **492** = 86 running + **78 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **688** |
| **cores doing RUNG-RAISING work** | **59.0%** -- 576 of 976 cores (113 min old **STALE**) |
...
```
generated 2026-08-11 15:26 UTC | elapsed 13.76 d | 15.4 d to the Aug-27 stop
test tier: 29,400 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 133.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     201 records     201.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    ...
    last 24 h    2574 records     107.2 rec/h   usable
    12 h rate is 92% from ONE line (test); 2 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         160     105  2026-08-11 16:38  2026-08-11 16:38  yes
      340       2,199     201  2026-08-12 07:53  2026-08-12 07:53  yes
      403       4,404     201  2026-08-13 00:22  2026-08-13 00:22  yes
      568      10,928     201  2026-08-15 01:08  2026-08-15 01:08  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @688 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           7.9 d           6.5 d   throughput
      279          11.2 d           9.3 d   throughput
      340          13.4 d          11.1 d   throughput
      403          15.7 d          13.0 d   throughput
      568          21.8 d          18.1 d   throughput
...
> **What is holding it right now: test -- baseline_log_growth has 126 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **281 seeds each** (read live from the archive on this publish). !! THE
