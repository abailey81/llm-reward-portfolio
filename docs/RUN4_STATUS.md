...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 57 min ago; none live, newest failure 57 min ago** |
...
| **cores computing** | **512** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (64 min old) |
...
```
generated 2026-08-11 18:37 UTC | elapsed 13.89 d | 15.2 d to the Aug-27 stop
test tier: 29,785 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,814     102  GATED             GATED             barrier>=279
      403       4,019     102  GATED             GATED             barrier>=279
      568      10,543     102  GATED             GATED             barrier>=279
