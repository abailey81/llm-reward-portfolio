...
**Auto-generated 2026-08-11 15:07 UTC -- T+329h58m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+329h58m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 2.6 h ago; none live, newest failure 2.6 h ago** |
...
| | |
|---|---|
| cluster jobs | **502** = 96 running + **78 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **768** |
| **cores doing RUNG-RAISING work** | **59.0%** -- 576 of 976 cores (95 min old **STALE**) |
...
```
generated 2026-08-11 15:08 UTC | elapsed 13.75 d | 15.4 d to the Aug-27 stop
test tier: 29,340 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 132.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     181 records     181.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     420 records     140.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1585 records     132.1 rec/h   usable
    last 24 h    2551 records     106.3 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         160     145  2026-08-11 16:21  2026-08-11 16:21  yes
      340       2,259     181  2026-08-12 08:14  2026-08-12 08:14  yes
      403       4,464     181  2026-08-13 00:56  2026-08-13 00:56  yes
      568      10,988     181  2026-08-15 02:20  2026-08-15 02:20  yes
...
> **What is holding it right now: test -- baseline_log_growth has 128 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **279 seeds each** (read live from the archive on this publish). !! THE
