...
**Auto-generated 2026-08-13 03:35 UTC -- T+366h26m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+366h26m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **312** = 72 running + **174 ELIGIBLE** + 66 held by us + 0 held only by the site |
...
| **cores computing** | **576** |
| **cores doing RUNG-RAISING work** | **27.5%** -- 112 of 408 cores (122 min old **STALE**) |
...
```
generated 2026-08-13 03:36 UTC | elapsed 15.27 d | 13.8 d to the Aug-27 stop
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last  1 h      42 records      42.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    ...
    last 24 h    1307 records      54.5 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         376      42  2026-08-14 11:50  2026-08-14 11:50  yes
      403       2,397      42  2026-08-21 17:04  2026-08-21 17:04  yes
      568       8,744      42  2026-09-13 09:05  2026-09-13 09:05  NO
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @576 cores      @830 cores   binding
      ...
      100           5.5 d           4.6 d   critical_chain
      189           9.4 d           6.5 d   throughput
      279          13.4 d           9.3 d   throughput
      340          16.0 d          11.1 d   throughput
      403          18.8 d          13.0 d   throughput
      568          26.0 d          18.1 d   throughput
