...
**Auto-generated 2026-08-06 15:13 UTC -- T+210h04m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+210h04m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=156; worst streak 2/240 (0.8% to fatal), ops on core, 39 min ago; none live, newest failure 39 min ago** |
...
| | |
|---|---|
| cluster jobs | **866** (72 running, 794 queued) |
| **cores computing** | **576** |
| **cores doing RUNG-RAISING work** | **22.6%** -- 184 of 816 cores (149 min old **STALE**) |
...
```
generated 2026-08-06 15:13 UTC | elapsed 8.75 d | 20.4 d to the Aug-27 stop
test tier: 17,963 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 73.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     255 records     255.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30          66      35  GATED             GATED             unstarted:2-unit(s)-absent
      100       2,516      35  GATED             GATED             unstarted:2-unit(s)-absent>=30
      189       5,631      36  GATED             GATED             unstarted:2-unit(s)-absent>=30
      279       9,352     255  GATED             GATED             unstarted:2-unit(s)-absent>=30
      340      12,097     255  GATED             GATED             unstarted:2-unit(s)-absent>=30
      403      14,932     255  GATED             GATED             unstarted:2-unit(s)-absent>=30
      568      22,365     255  GATED             GATED             unstarted:2-unit(s)-absent>=30
