...
**Auto-generated 2026-08-12 02:34 UTC -- T+341h25m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+341h25m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 8.9 h ago** |
...
| **cores computing** | **840** |
| **cores doing RUNG-RAISING work** | **56.7%** -- 440 of 776 cores (61 min old) |
...
```
generated 2026-08-12 02:35 UTC | elapsed 14.23 d | 14.9 d to the Aug-27 stop
test tier: 30,175 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 79.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     125 records      41.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     949 records      79.1 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,424      45  GATED             GATED             barrier>=279
      403       3,629      45  GATED             GATED             barrier>=279
      568      10,153      45  GATED             GATED             barrier>=279
...
```
2026-08-12T02:28:04Z  OK  records=31712 (+15)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.3m  drift=0  sci=OK  r115=22B  cores=840  sweep=765.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
