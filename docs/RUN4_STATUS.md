...
**Auto-generated 2026-08-10 23:35 UTC -- T+314h26m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+314h26m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 12.8 h ago** |
...
| | |
|---|---|
| cluster jobs | **731** = 130 running + **247 ELIGIBLE** + 354 held by us + 0 held only by the site |
...
| **cores computing** | **1040** |
| **cores doing RUNG-RAISING work** | **100.0%** -- 1048 of 1048 cores (3 min old) |
...
```
generated 2026-08-10 23:36 UTC | elapsed 13.10 d | 16.0 d to the Aug-27 stop
test tier: 27,510 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 81.9 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     397 records     132.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     983 records      81.9 rec/h   usable
    last 24 h    2329 records      97.0 rec/h   usable
    12 h rate is 42% from ONE line (test_leg_nemotron_3_super); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       1,954     132  2026-08-11 23:27  2026-08-11 23:27  yes
      340       4,089     132  2026-08-13 01:31  2026-08-13 01:31  yes
      403       6,294     132  2026-08-14 04:26  2026-08-14 04:26  yes
      568      12,818     132  2026-08-17 12:05  2026-08-17 12:05  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1040 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           5.2 d           6.5 d   throughput
      279           7.4 d           9.3 d   throughput
      340           8.9 d          11.1 d   throughput
      403          10.4 d          13.0 d   throughput
      568          14.4 d          18.1 d   throughput
