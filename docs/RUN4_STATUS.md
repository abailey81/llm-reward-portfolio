...
**Auto-generated 2026-08-12 07:11 UTC -- T+346h02m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+346h02m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 13.5 h ago** |
...
| | |
|---|---|
| cluster jobs | **444** = 116 running + **0 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **928** |
| **cores doing RUNG-RAISING work** | **78.3%** -- 720 of 920 cores (99 min old **STALE**) |
...
```
generated 2026-08-12 07:12 UTC | elapsed 14.42 d | 14.7 d to the Aug-27 stop
test tier: 30,580 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 65.5 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      97 records      97.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    ...
    last 12 h     786 records      65.5 rec/h   usable
    last 24 h    2188 records      91.2 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279           0       2  REACHED           REACHED           yes
      340       1,196      86  2026-08-13 01:28  2026-08-13 01:28  yes
      403       3,401      86  2026-08-14 11:07  2026-08-14 11:07  yes
      568       9,748      97  2026-08-18 12:02  2026-08-18 12:02  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @928 cores      @830 cores   binding
      ...
      279           8.3 d           9.3 d   throughput
      340          10.0 d          11.1 d   throughput
      403          11.7 d          13.0 d   throughput
      568          16.2 d          18.1 d   throughput
