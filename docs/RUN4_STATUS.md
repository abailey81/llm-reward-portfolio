...
**Auto-generated 2026-08-13 03:27 UTC -- T+366h18m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+366h18m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 33.8 h ago** |
...
| | |
|---|---|
| cluster jobs | **313** = 68 running + **179 ELIGIBLE** + 66 held by us + 0 held only by the site |
...
| **cores computing** | **544** |
| **cores doing RUNG-RAISING work** | **27.5%** -- 112 of 408 cores (114 min old **STALE**) |
...
```
generated 2026-08-13 03:28 UTC | elapsed 15.26 d | 13.9 d to the Aug-27 stop
test tier: 31,584 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 12.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h     153 records      12.8 rec/h   usable
    last 24 h    1321 records      55.0 rec/h   usable
    12 h rate is 37% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         376      48  2026-08-14 08:58  2026-08-14 08:58  yes
      403       2,397      48  2026-08-20 23:28  2026-08-20 23:28  yes
      568       8,744      48  2026-09-10 17:17  2026-09-10 17:17  NO
...
2026-08-13T03:24:20Z  OK  records=33127 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.6m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=60.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-13T03:25:51Z  OK  records=33127 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=60.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-13T03:27:40Z  OK  records=33127 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.6m  drift=0  sci=BLIND(deep_audit)  r115=22B  cores=552  sweep=79.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
