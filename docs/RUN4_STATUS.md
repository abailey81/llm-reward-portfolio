...
| transport health | **timeouts 6h=35; worst streak 11/240 (4.6% to fatal), pull on core, 3.5 h ago; none live, newest failure 3.5 h ago** |
...
| | |
|---|---|
| cluster jobs | **835** = 102 running + **162 ELIGIBLE** + 546 held by us + 25 held only by the site |
...
| **cores computing** | **816** |
| **cores doing RUNG-RAISING work** | **29.4%** -- 240 of 816 cores (1 min old) |
...
```
generated 2026-08-06 20:26 UTC | elapsed 8.97 d | 20.1 d to the Aug-27 stop
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340      11,641      10  GATED             GATED             no-records:test/distributional,test/scalar>=30
      403      14,476      10  GATED             GATED             no-records:test/distributional,test/scalar>=30
      568      21,909      10  GATED             GATED             no-records:test/distributional,test/scalar>=30
