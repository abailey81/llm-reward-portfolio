...
**Auto-generated 2026-08-12 15:43 UTC -- T+354h34m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+354h34m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 22.0 h ago** |
...
| | |
|---|---|
| cluster jobs | **334** = 34 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **272** |
| **cores doing RUNG-RAISING work** | **58.3%** -- 168 of 288 cores (10 min old) |
...
```
generated 2026-08-12 15:43 UTC | elapsed 14.77 d | 14.3 d to the Aug-27 stop
test tier: 31,454 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 97.6 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      98 records      98.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     356 records     118.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1171 records      97.6 rec/h   usable
    last 24 h    2004 records      83.5 rec/h   usable
    12 h rate is 71% from ONE line (test); 3 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (98 rec/h); latest excludes cells already within
    8 of the ceiling (98 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         472       0  2026-08-12 20:34  2026-08-12 20:34  yes
      403       2,527      98  2026-08-13 17:37  2026-08-13 17:37  yes
      568       8,874      98  2026-08-16 10:40  2026-08-16 10:40  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @272 cores      @830 cores   binding
       30           5.1 d           4.6 d   critical_chain
      100          11.7 d           4.6 d   critical_chain
      189          19.9 d           6.5 d   throughput
      279          28.3 d           9.3 d   throughput
      340          34.0 d          11.1 d   throughput
      403          39.8 d          13.0 d   throughput
      568          55.2 d          18.1 d   throughput
...
currently at **346 seeds each** (read live from the archive on this publish). !! THE
