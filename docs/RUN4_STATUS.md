...
| **cores computing** | **888** |
| **cores doing RUNG-RAISING work** | **56.7%** -- 440 of 776 cores (113 min old **STALE**) |
...
```
generated 2026-08-12 03:26 UTC | elapsed 14.26 d | 14.9 d to the Aug-27 stop
test tier: 30,258 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      568      10,070      89  GATED             GATED             barrier>=279
