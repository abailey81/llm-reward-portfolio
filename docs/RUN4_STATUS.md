...
**Auto-generated 2026-08-06 02:21 UTC -- T+197h12m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+197h12m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on nemotron-3-super, 2.3 h ago; none live, newest failure 2.3 h ago** |
...
| | |
|---|---|
| cluster jobs | **964** (67 running, 897 queued) |
| **cores computing** | **536** |
...
```
generated 2026-08-06 02:22 UTC | elapsed 8.22 d | 20.9 d to the Aug-27 stop
test tier: 17,067 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h      59 records      59.0 rec/h
    last  3 h     340 records     113.3 rec/h
    last 12 h    1780 records     148.3 rec/h
    last 24 h    3475 records     144.8 rec/h
    12 h rate is 64% from ONE line (test_leg_haiku_4_5); 4 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 64% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_haiku_4_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       2,636      59  GATED             GATED             barrier>=30
      189       6,200      59  GATED             GATED             barrier>=30
      279      10,248      59  GATED             GATED             barrier>=30
      340      12,993      59  GATED             GATED             barrier>=30
      403      15,828      59  GATED             GATED             barrier>=30
      568      23,261      59  GATED             GATED             barrier>=30
...
2026-08-06T02:12:56Z  OK  records=18607 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=27.5s  auto-cycle
2026-08-06T02:19:09Z  OK  records=18607 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  sweep=343.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
