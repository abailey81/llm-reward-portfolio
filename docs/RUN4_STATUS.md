...
**Auto-generated 2026-08-12 09:35 UTC -- T+348h26m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+348h26m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **424** = 96 running + **0 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **768** |
| **cores doing RUNG-RAISING work** | **100.0%** -- 776 of 776 cores (2 min old) |
...
```
generated 2026-08-12 09:36 UTC | elapsed 14.52 d | 14.6 d to the Aug-27 stop
test tier: 30,775 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 66.6 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     236 records      78.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     799 records      66.6 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,001      86  2026-08-13 00:38  2026-08-13 00:38  yes
      403       3,206      86  2026-08-14 09:45  2026-08-14 09:45  yes
      568       9,553      86  2026-08-18 09:04  2026-08-18 09:04  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @768 cores      @830 cores   binding
      ...
      279          10.0 d           9.3 d   throughput
      340          12.0 d          11.1 d   throughput
      403          14.1 d          13.0 d   throughput
      568          19.5 d          18.1 d   throughput
