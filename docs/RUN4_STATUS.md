...
**Auto-generated 2026-08-11 17:48 UTC -- T+332h39m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+332h39m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 5.2 h ago; LIVE, still failing** |
...
| | |
|---|---|
| cluster jobs | **456** = 66 running + **62 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **528** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (16 min old) |
...
```
generated 2026-08-11 17:49 UTC | elapsed 13.86 d | 15.3 d to the Aug-27 stop
test tier: 29,703 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 125.3 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1504 records     125.3 rec/h   usable
    last 24 h    2764 records     115.2 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         128      22  GATED             GATED             barrier
      340       1,896      98  GATED             GATED             barrier>=279
      403       4,101      98  GATED             GATED             barrier>=279
      568      10,625      98  GATED             GATED             barrier>=279
