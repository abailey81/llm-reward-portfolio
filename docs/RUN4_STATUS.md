...
**Auto-generated 2026-08-11 08:35 UTC -- T+323h26m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+323h26m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **596** = 141 running + **101 ELIGIBLE** + 354 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **100.0%** -- 1152 of 1152 cores (63 min old) |
...
```
generated 2026-08-11 08:36 UTC | elapsed 13.48 d | 15.6 d to the Aug-27 stop
test tier: 28,566 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 121.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     387 records     129.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1453 records     121.1 rec/h   usable
    last 24 h    2486 records     103.6 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (121 rec/h); latest excludes cells already within
    8 of the ceiling (121 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         898     134  2026-08-11 16:01  2026-08-11 16:01  yes
      340       3,033     134  2026-08-12 09:39  2026-08-12 09:39  yes
      403       5,238     134  2026-08-13 03:51  2026-08-13 03:51  yes
      568      11,762     134  2026-08-15 09:44  2026-08-15 09:44  yes
...
> **What is holding it right now: test -- baseline_volatility_scaled_return has 169 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **238 seeds each** (read live from the archive on this publish). !! THE
...
2026-08-11T08:20:28Z  OK  records=30027 (+22)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  sweep=1459.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T08:35:27Z  OK  records=30100 (+73)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  cores=1136  sweep=868.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
