...
**Auto-generated 2026-08-07 00:31 UTC -- T+219h22m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+219h22m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| **cores computing** | **880** |
| **cores doing RUNG-RAISING work** | **33.6%** -- 288 of 856 cores (51 min old) |
...
```
generated 2026-08-07 00:31 UTC | elapsed 9.14 d | 20.0 d to the Aug-27 stop
test tier: 18,438 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 68.0 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h     816 records      68.0 rec/h   usable
    last 24 h    1455 records      60.6 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       2,502       8  GATED             GATED             no-records:test/distributional,test/scalar>=30
      189       5,617       8  GATED             GATED             no-records:test/distributional,test/scalar>=30
      279       9,147       8  GATED             GATED             no-records:test/distributional,test/scalar>=30
      340      11,622      11  GATED             GATED             no-records:test/distributional,test/scalar>=30
      403      14,457      11  GATED             GATED             no-records:test/distributional,test/scalar>=30
      568      21,890      11  GATED             GATED             no-records:test/distributional,test/scalar>=30
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 72% of the rung-568 backlog (15,670 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
2026-08-07T00:21:20Z  OK  records=19974 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=32.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
