...
**Auto-generated 2026-08-12 01:06 UTC -- T+339h57m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+339h57m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 7.4 h ago** |
...
| | |
|---|---|
| cluster jobs | **510** = 93 running + **89 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **744** |
| **cores doing RUNG-RAISING work** | **37.8%** -- 248 of 656 cores (94 min old **STALE**) |
...
```
generated 2026-08-12 01:07 UTC | elapsed 14.17 d | 15.0 d to the Aug-27 stop
test tier: 30,105 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 89.6 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h      87 records      29.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1075 records      89.6 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279          72      37  GATED             GATED             barrier
      340       1,494      45  GATED             GATED             barrier>=279
      403       3,699      45  GATED             GATED             barrier>=279
      568      10,223      45  GATED             GATED             barrier>=279
...
```
2026-08-12T00:51:00Z  OK  records=31635 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.1m  drift=0  sci=OK  r115=22B  sweep=60.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
