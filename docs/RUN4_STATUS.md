...
**Auto-generated 2026-08-12 14:34 UTC -- T+353h25m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+353h25m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 20.9 h ago** |
...
| | |
|---|---|
| cluster jobs | **353** = 53 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **424** |
| **cores doing RUNG-RAISING work** | **77.9%** -- 424 of 544 cores (61 min old) |
...
```
generated 2026-08-12 14:34 UTC | elapsed 14.73 d | 14.4 d to the Aug-27 stop
test tier: 31,332 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 96.4 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     327 records     109.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1157 records      96.4 rec/h   usable
    last 24 h    2106 records      87.8 rec/h   usable
    12 h rate is 62% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         477      86  2026-08-12 19:31  2026-08-12 19:31  yes
      403       2,649     119  2026-08-13 18:03  2026-08-13 18:03  yes
      568       8,996     119  2026-08-16 11:52  2026-08-16 11:52  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @424 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           7.5 d           4.6 d   critical_chain
      189          12.8 d           6.5 d   throughput
      279          18.1 d           9.3 d   throughput
      340          21.8 d          11.1 d   throughput
      403          25.5 d          13.0 d   throughput
      568          35.4 d          18.1 d   throughput
...
currently at **338 seeds each** (read live from the archive on this publish). !! THE
