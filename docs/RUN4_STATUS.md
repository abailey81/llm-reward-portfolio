...
**Auto-generated 2026-08-11 13:52 UTC -- T+328h43m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+328h43m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 79 min ago; none live, newest failure 78 min ago** |
...
| | |
|---|---|
| cluster jobs | **528** = 122 running + **78 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **976** |
| **cores doing RUNG-RAISING work** | **59.0%** -- 576 of 976 cores (20 min old) |
...
```
generated 2026-08-11 13:53 UTC | elapsed 13.70 d | 15.4 d to the Aug-27 stop
test tier: 29,119 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 119.3 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1432 records     119.3 rec/h   usable
    last 24 h    2422 records     100.9 rec/h   usable
    12 h rate is 84% from ONE line (test); 2 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         345     118  2026-08-11 16:47  2026-08-11 16:47  yes
      340       2,480     118  2026-08-12 10:40  2026-08-12 10:40  yes
      403       4,685     118  2026-08-13 05:09  2026-08-13 05:09  yes
      568      11,209     118  2026-08-15 11:49  2026-08-15 11:49  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 33% of the rung-568 backlog (3,743 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @976 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           5.6 d           6.5 d   throughput
      279           7.9 d           9.3 d   throughput
      340           9.5 d          11.1 d   throughput
      403          11.1 d          13.0 d   throughput
      568          15.4 d          18.1 d   throughput
...
> **What is holding it right now: test -- baseline_return_minus_downside has 140 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **267 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T13:42:19Z  OK  records=30640 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=52.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T13:43:40Z  OK  records=30643 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.1m  drift=0  sci=OK  r115=22B  sweep=51.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T13:45:00Z  OK  records=30646 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=50.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T13:46:34Z  OK  records=30649 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  sweep=63.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T13:47:55Z  OK  records=30650 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  sweep=51.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T13:49:26Z  OK  records=30653 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  cores=984  sweep=60.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
