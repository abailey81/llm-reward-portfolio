...
**Auto-generated 2026-08-06 05:31 UTC -- T+200h22m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+200h22m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **959** (70 running, 889 queued) |
| **cores computing** | **560** |
...
```
generated 2026-08-06 05:32 UTC | elapsed 8.35 d | 20.8 d to the Aug-27 stop
test tier: 17,216 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     103 records     103.0 rec/h
    last  3 h     142 records      47.3 rec/h
    last 12 h    1311 records     109.2 rec/h
    last 24 h    3491 records     145.5 rec/h
    12 h rate is 40% from ONE line (test_leg_haiku_4_5); 4 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 40% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_haiku_4_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       2,570      20  GATED             GATED             barrier>=30
      189       6,051     103  GATED             GATED             barrier>=30
      279      10,099     103  GATED             GATED             barrier>=30
      340      12,844     103  GATED             GATED             barrier>=30
      403      15,679     103  GATED             GATED             barrier>=30
      568      23,112     103  GATED             GATED             barrier>=30
...
```
2026-08-06T05:19:42Z  OK  records=18711 (+28)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  cores=568  sweep=727.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
