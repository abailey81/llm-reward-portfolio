...
**Auto-generated 2026-08-11 20:21 UTC -- T+335h12m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+335h12m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 2.7 h ago; none live, newest failure 2.7 h ago** |
...
| | |
|---|---|
| cluster jobs | **429** = 60 running + **41 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **480** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (48 min old) |
...
```
generated 2026-08-11 20:22 UTC | elapsed 13.97 d | 15.2 d to the Aug-27 stop
test tier: 29,900 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 113.4 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1361 records     113.4 rec/h   usable
    last 24 h    2794 records     116.4 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,699      96  GATED             GATED             barrier>=279
      403       3,904      96  GATED             GATED             barrier>=279
      568      10,428      96  GATED             GATED             barrier>=279
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @480 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           6.6 d           4.6 d   critical_chain
      189          11.3 d           6.5 d   throughput
      279          16.0 d           9.3 d   throughput
      340          19.2 d          11.1 d   throughput
      403          22.6 d          13.0 d   throughput
      568          31.3 d          18.1 d   throughput
...
currently at **300 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T20:15:02Z  OK  records=31429 (+40)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.1m  drift=0  sci=OK  r115=22B  cores=464  sweep=705.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T20:16:41Z  OK  records=31434 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=69.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
