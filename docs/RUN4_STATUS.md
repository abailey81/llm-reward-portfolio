...
**Auto-generated 2026-08-12 17:36 UTC -- T+356h28m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+356h28m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=1; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 23.9 h ago** |
| transport timeouts (cumulative, ever) | 355 -- a level with no rate; read the row above |
...
| | |
|---|---|
| cluster jobs | **328** = 28 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **224** |
| **cores doing RUNG-RAISING work** | **46.4%** -- 104 of 224 cores (4 min old) |
...
```
generated 2026-08-12 17:37 UTC | elapsed 14.85 d | 14.3 d to the Aug-27 stop
test tier: 31,490 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 90.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      10 records      10.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     144 records      48.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1081 records      90.1 rec/h   usable
    last 24 h    1807 records      75.3 rec/h   usable
    12 h rate is 80% from ONE line (test); 3 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (90 rec/h); latest excludes cells already within
    8 of the ceiling (90 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         470       2  2026-08-12 22:50  2026-08-12 22:50  yes
      403       2,491      10  2026-08-13 21:17  2026-08-13 21:17  yes
      568       8,838      10  2026-08-16 19:44  2026-08-16 19:44  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 35% of the rung-568 backlog (3,052 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @224 cores      @830 cores   binding
       30           6.2 d           4.6 d   critical_chain
      100          14.1 d           4.6 d   critical_chain
      189          24.2 d           6.5 d   throughput
      279          34.4 d           9.3 d   throughput
      340          41.2 d          11.1 d   throughput
      403          48.4 d          13.0 d   throughput
      568          67.0 d          18.1 d   throughput
...
```
2026-08-12T17:05:27Z  OK  records=33028 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.9m  drift=0  sci=OK  r115=22B  sweep=60.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T17:21:09Z  OK  records=33028 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  sweep=912.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T17:32:35Z  ATTN  records=33030 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.8m  drift=0  sci=OK  r115=22B  cores=224  sweep=655.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T17:34:05Z  ATTN  records=33031 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.5m  drift=0  sci=OK  r115=22B  sweep=59.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T17:35:36Z  ATTN  records=33031 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=59.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T17:37:07Z  ATTN  records=33031 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=0.8m  drift=0  sci=OK  r115=22B  sweep=61.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
