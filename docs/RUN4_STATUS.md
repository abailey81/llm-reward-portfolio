...
**Auto-generated 2026-08-12 10:38 UTC -- T+349h29m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+349h29m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **408** = 89 running + **0 ELIGIBLE** + 319 held by us + 0 held only by the site |
...
| **cores computing** | **712** |
| **cores doing RUNG-RAISING work** | **100.0%** -- 776 of 776 cores (66 min old) |
...
```
generated 2026-08-12 10:39 UTC | elapsed 14.56 d | 14.6 d to the Aug-27 stop
test tier: 30,875 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 69.0 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h     828 records      69.0 rec/h   usable
    last 24 h    2107 records      87.8 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         901      86  2026-08-12 23:43  2026-08-12 23:43  yes
      403       3,106      86  2026-08-14 07:40  2026-08-14 07:40  yes
      568       9,453      86  2026-08-18 03:39  2026-08-18 03:39  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @712 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           7.6 d           6.5 d   throughput
      279          10.8 d           9.3 d   throughput
      340          13.0 d          11.1 d   throughput
      403          15.2 d          13.0 d   throughput
      568          21.1 d          18.1 d   throughput
...
currently at **316 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-12T10:31:05Z  OK  records=32412 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.3m  drift=0  sci=OK  r115=22B  sweep=60.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
