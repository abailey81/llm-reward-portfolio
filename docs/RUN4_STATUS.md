...
**Auto-generated 2026-08-07 01:11 UTC -- T+220h02m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+220h02m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **826** = 111 running + **169 ELIGIBLE** + 546 held by us + 0 held only by the site |
...
| **cores computing** | **888** |
| **cores doing RUNG-RAISING work** | **33.6%** -- 288 of 856 cores (91 min old **STALE**) |
...
```
generated 2026-08-07 01:11 UTC | elapsed 9.17 d | 20.0 d to the Aug-27 stop
test tier: 18,515 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last 24 h    1513 records      63.0 rec/h   usable
    12 h rate is 80% from ONE line (test_leg_kimi_k3); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30          59       1  GATED             GATED             no-records:test/scalar
      100       2,462      48  GATED             GATED             no-records:test/scalar>=30
      189       5,577      48  GATED             GATED             no-records:test/scalar>=30
      279       9,107      48  GATED             GATED             no-records:test/scalar>=30
      340      11,551      80  GATED             GATED             no-records:test/scalar>=30
      403      14,380      86  GATED             GATED             no-records:test/scalar>=30
      568      21,813      86  GATED             GATED             no-records:test/scalar>=30
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 67% of the rung-568 backlog (14,564 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
|---|---|---|---|---|
| test | **0** | 30 | 9 | 1 arm(s) still at zero |
...
```
2026-08-07T00:53:31Z  OK  records=20020 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  sweep=32.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-07T00:54:33Z  OK  records=20020 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.1m  drift=0  sci=OK  r115=22B  sweep=31.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-07T00:55:35Z  OK  records=20030 (+10)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=32.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-07T01:01:10Z  OK  records=20040 (+10)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  cores=848  sweep=304.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-07T01:06:31Z  OK  records=20048 (+8)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=291.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-07T01:07:34Z  OK  records=20049 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=32.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
