...
| **cores computing** | **904** |
| **cores doing RUNG-RAISING work** | **21.2%** -- 192 of 904 cores (0 min old) |
...
```
generated 2026-08-06 10:21 UTC | elapsed 8.55 d | 20.6 d to the Aug-27 stop
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      189       5,735      73  GATED             GATED             barrier>=30
      279       9,783      73  GATED             GATED             barrier>=30
      340      12,528      73  GATED             GATED             barrier>=30
      403      15,363      73  GATED             GATED             barrier>=30
      568      22,796      73  GATED             GATED             barrier>=30
    ...
    !! 78% of the rung-568 backlog (17,878 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
