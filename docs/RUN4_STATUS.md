...
**Auto-generated 2026-08-12 20:12 UTC -- T+359h03m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+359h03m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=2; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 26.5 h ago** |
...
| | |
|---|---|
| cluster jobs | **325** = 24 running + **1 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **192** |
| **cores doing RUNG-RAISING work** | **42.3%** -- 88 of 208 cores (31 min old) |
...
```
generated 2026-08-12 20:13 UTC | elapsed 14.96 d | 14.2 d to the Aug-27 stop
test tier: 31,527 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 72.4 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h      41 records      13.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     869 records      72.4 rec/h   usable
    last 24 h    1640 records      68.3 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         433      22  2026-08-13 02:12  2026-08-13 02:12  yes
      403       2,454      22  2026-08-14 06:06  2026-08-14 06:06  yes
      568       8,801      22  2026-08-17 21:45  2026-08-17 21:45  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @192 cores      @830 cores   binding
       30           7.3 d           4.6 d   critical_chain
      100          16.5 d           4.6 d   critical_chain
      189          28.2 d           6.5 d   throughput
      279          40.1 d           9.3 d   throughput
      340          48.1 d          11.1 d   throughput
      403          56.4 d          13.0 d   throughput
      568          78.1 d          18.1 d   throughput
