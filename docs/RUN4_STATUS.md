...
| transport health | **timeouts 6h=156; worst streak 2/240 (0.8% to fatal), ops on core, 43 min ago; none live, newest failure 43 min ago** |
...
| | |
|---|---|
| cluster jobs | **863** (69 running, 794 queued) |
| **cores computing** | **552** |
| **cores doing RUNG-RAISING work** | **22.6%** -- 184 of 816 cores (153 min old **STALE**) |
...
```
generated 2026-08-06 15:17 UTC | elapsed 8.76 d | 20.4 d to the Aug-27 stop
test tier: 17,989 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 75.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     262 records     262.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    ...
    last 24 h    2481 records     103.4 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30          64      28  GATED             GATED             unstarted:2-unit(s)-absent
      100       2,514      28  GATED             GATED             unstarted:2-unit(s)-absent>=30
      189       5,629      29  GATED             GATED             unstarted:2-unit(s)-absent>=30
      279       9,326     262  GATED             GATED             unstarted:2-unit(s)-absent>=30
      340      12,071     262  GATED             GATED             unstarted:2-unit(s)-absent>=30
      403      14,906     262  GATED             GATED             unstarted:2-unit(s)-absent>=30
      568      22,339     262  GATED             GATED             unstarted:2-unit(s)-absent>=30
