...
**Auto-generated 2026-08-12 14:22 UTC -- T+353h13m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+353h13m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 20.7 h ago** |
...
| | |
|---|---|
| cluster jobs | **354** = 54 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **432** |
| **cores doing RUNG-RAISING work** | **77.9%** -- 424 of 544 cores (50 min old) |
...
```
generated 2026-08-12 14:23 UTC | elapsed 14.72 d | 14.4 d to the Aug-27 stop
test tier: 31,308 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 95.0 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     112 records     112.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     331 records     110.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1140 records      95.0 rec/h   usable
    last 24 h    2122 records      88.4 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         487      93  2026-08-12 19:31  2026-08-12 19:31  yes
      403       2,673     112  2026-08-13 18:31  2026-08-13 18:31  yes
      568       9,020     112  2026-08-16 13:20  2026-08-16 13:20  yes
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @432 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           7.3 d           4.6 d   critical_chain
      189          12.5 d           6.5 d   throughput
      279          17.8 d           9.3 d   throughput
      340          21.4 d          11.1 d   throughput
      403          25.1 d          13.0 d   throughput
      568          34.7 d          18.1 d   throughput
...
currently at **337 seeds each** (read live from the archive on this publish). !! THE
