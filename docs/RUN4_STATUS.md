...
**Auto-generated 2026-08-09 15:54 UTC -- T+282h45m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+282h45m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **758** = 140 running + **242 ELIGIBLE** + 376 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **99.3%** -- 1080 of 1088 cores (22 min old) |
...
```
generated 2026-08-09 15:55 UTC | elapsed 11.78 d | 17.3 d to the Aug-27 stop
test tier: 24,215 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 100.2 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     263 records     263.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     429 records     143.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1202 records     100.2 rec/h   usable
    last 24 h    2786 records     116.1 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (100 rec/h); latest excludes cells already within
    8 of the ceiling (100 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      189       1,516     263  2026-08-10 07:03  2026-08-10 07:03  yes
      279       4,666     263  2026-08-11 14:30  2026-08-11 14:30  yes
      340       6,801     263  2026-08-12 11:49  2026-08-12 11:49  yes
      403       9,037     263  2026-08-13 10:08  2026-08-13 10:08  yes
      568      16,113     263  2026-08-16 08:47  2026-08-16 08:47  yes
...
> **What is holding it right now: test -- baseline_return_minus_downside has 275 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **132 seeds each** (read live from the archive on this publish). !! THE
