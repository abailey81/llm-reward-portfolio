...
**Auto-generated 2026-08-11 00:22 UTC -- T+315h13m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+315h13m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **722** = 135 running + **233 ELIGIBLE** + 354 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **100.0%** -- 1048 of 1048 cores (49 min old) |
...
```
generated 2026-08-11 00:22 UTC | elapsed 13.13 d | 16.0 d to the Aug-27 stop
test tier: 27,581 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 84.0 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1008 records      84.0 rec/h   usable
    last 24 h    2354 records      98.1 rec/h   usable
    12 h rate is 39% from ONE line (test_leg_nemotron_3_super); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       1,883      87  2026-08-11 22:47  2026-08-11 22:47  yes
      340       4,018      87  2026-08-13 00:12  2026-08-13 00:12  yes
      403       6,223      87  2026-08-14 02:27  2026-08-14 02:27  yes
      568      12,747      87  2026-08-17 08:07  2026-08-17 08:07  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 37% of the rung-568 backlog (4,772 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
2026-08-11T00:09:37Z  OK  records=29091 (+12)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  sweep=475.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T00:11:05Z  OK  records=29095 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=58.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
