...
**Auto-generated 2026-08-11 21:33 UTC -- T+336h24m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+336h24m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 3.9 h ago; none live, newest failure 3.9 h ago** |
...
| | |
|---|---|
| cluster jobs | **418** = 72 running + **18 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **576** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (121 min old **STALE**) |
...
```
generated 2026-08-11 21:34 UTC | elapsed 14.02 d | 15.1 d to the Aug-27 stop
test tier: 29,973 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last 24 h    2714 records     113.1 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,626      53  GATED             GATED             barrier>=279
      403       3,831      53  GATED             GATED             barrier>=279
      568      10,355      53  GATED             GATED             barrier>=279
...
> **What is holding it right now: test_leg_deepseek_v4_pro -- distributional has 31 HOLE(S) below its frontier 335 -- that is what caps this line**
...
2026-08-11T21:28:04Z  OK  records=31497 (+8)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.0m  drift=0  sci=OK  r115=22B  sweep=1453.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
