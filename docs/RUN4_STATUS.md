...
**Auto-generated 2026-08-12 02:18 UTC -- T+341h09m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+341h09m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| **cores computing** | **824** |
| **cores doing RUNG-RAISING work** | **56.7%** -- 440 of 776 cores (45 min old) |
...
```
generated 2026-08-12 02:19 UTC | elapsed 14.22 d | 14.9 d to the Aug-27 stop
test tier: 30,165 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last  3 h     116 records      38.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,434      51  GATED             GATED             barrier>=279
      403       3,639      51  GATED             GATED             barrier>=279
      568      10,163      51  GATED             GATED             barrier>=279
