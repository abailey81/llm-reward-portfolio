...
**Auto-generated 2026-08-06 18:18 UTC -- T+213h09m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+213h09m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=156; worst streak 11/240 (4.6% to fatal), pull on core, 82 min ago; none live, newest failure 80 min ago** |
...
| | |
|---|---|
| cluster jobs | **842** (75 running, 767 queued) |
| **cores computing** | **600** |
| **cores doing RUNG-RAISING work** | **28.8%** -- 168 of 584 cores (17 min old) |
...
```
generated 2026-08-06 18:18 UTC | elapsed 8.88 d | 20.2 d to the Aug-27 stop
test tier: 18,367 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340      11,693      64  GATED             GATED             no-records:test/distributional,test/scalar>=30
      403      14,528      64  GATED             GATED             no-records:test/distributional,test/scalar>=30
      568      21,961      64  GATED             GATED             no-records:test/distributional,test/scalar>=30
...
```
2026-08-06T17:59:20Z  OK  records=19902 (+17)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=726.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
