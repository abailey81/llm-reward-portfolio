...
| transport health | **timeouts 6h=156; worst streak 2/240 (0.8% to fatal), ops on core, 48 min ago; none live, newest failure 48 min ago** |
...
| | |
|---|---|
| cluster jobs | **857** (63 running, 794 queued) |
| **cores computing** | **504** |
| **cores doing RUNG-RAISING work** | **22.6%** -- 184 of 816 cores (158 min old **STALE**) |
...
```
generated 2026-08-06 15:22 UTC | elapsed 8.76 d | 20.4 d to the Aug-27 stop
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       9,308     270  GATED             GATED             unstarted:2-unit(s)-absent>=30
      340      12,053     270  GATED             GATED             unstarted:2-unit(s)-absent>=30
      403      14,888     270  GATED             GATED             unstarted:2-unit(s)-absent>=30
      568      22,321     270  GATED             GATED             unstarted:2-unit(s)-absent>=30
