...
**Auto-generated 2026-08-11 10:06 UTC -- T+324h57m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+324h57m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **578** = 138 running + **112 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **87.4%** -- 1000 of 1144 cores (33 min old) |
...
```
generated 2026-08-11 10:07 UTC | elapsed 13.54 d | 15.6 d to the Aug-27 stop
test tier: 28,713 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 115.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     332 records     110.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1381 records     115.1 rec/h   usable
    last 24 h    2423 records     101.0 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         751      98  2026-08-11 16:38  2026-08-11 16:38  yes
      340       2,886      98  2026-08-12 11:11  2026-08-12 11:11  yes
      403       5,091      98  2026-08-13 06:21  2026-08-13 06:21  yes
      568      11,615      98  2026-08-15 15:02  2026-08-15 15:02  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 14% of the rung-568 backlog (1,659 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test -- baseline_return_minus_variance has 159 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **248 seeds each** (read live from the archive on this publish). !! THE
...
2026-08-11T10:00:57Z  OK  records=30248 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=0.5m  drift=0  sci=OK  r115=22B  sweep=56.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T10:02:22Z  OK  records=30248 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.9m  drift=0  sci=OK  r115=22B  sweep=54.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T10:03:48Z  OK  records=30254 (+6)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.3m  drift=0  sci=OK  r115=22B  sweep=55.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T10:05:09Z  OK  records=30254 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.4m  drift=0  sci=OK  r115=22B  sweep=51.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
