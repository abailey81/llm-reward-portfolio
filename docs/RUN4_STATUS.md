...
**Auto-generated 2026-08-10 23:28 UTC -- T+314h19m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+314h19m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **29045** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 1.4 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
| LLM calls / spend | 2956 / **$45.5021** |
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 12.7 h ago** |
...
| | |
|---|---|
| cluster jobs | **732** = 131 running + **247 ELIGIBLE** + 354 held by us + 0 held only by the site |
...
| **cores computing** | **1048** |
| **cores doing RUNG-RAISING work** | **100.0%** -- 1096 of 1096 cores (115 min old **STALE**) |
...
```
generated 2026-08-10 23:29 UTC | elapsed 13.10 d | 16.0 d to the Aug-27 stop
test tier: 27,503 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 83.6 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     137 records     137.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     393 records     131.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1003 records      83.6 rec/h   usable
    last 24 h    2322 records      96.8 rec/h   usable
    12 h rate is 41% from ONE line (test_leg_nemotron_3_super); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       1,961     137  2026-08-11 22:56  2026-08-11 22:56  yes
      340       4,096     137  2026-08-13 00:29  2026-08-13 00:29  yes
      403       6,301     137  2026-08-14 02:52  2026-08-14 02:52  yes
      568      12,825     137  2026-08-17 08:55  2026-08-17 08:55  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 46% of the rung-568 backlog (5,851 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1048 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           5.2 d           6.5 d   throughput
      279           7.3 d           9.3 d   throughput
      340           8.8 d          11.1 d   throughput
      403          10.3 d          13.0 d   throughput
      568          14.3 d          18.1 d   throughput
...
```
2026-08-10T23:10:37Z  OK  records=29005 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  sweep=47.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-10T23:11:53Z  OK  records=29009 (+4)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  sweep=46.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
