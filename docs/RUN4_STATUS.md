...
**Auto-generated 2026-08-12 18:50 UTC -- T+357h41m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+357h41m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=2; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 25.2 h ago** |
...
| | |
|---|---|
| cluster jobs | **326** = 26 running + **0 ELIGIBLE** + 300 held by us + 0 held only by the site |
...
| **cores computing** | **208** |
| **cores doing RUNG-RAISING work** | **46.4%** -- 104 of 224 cores (77 min old) |
...
```
generated 2026-08-12 18:51 UTC | elapsed 14.90 d | 14.2 d to the Aug-27 stop
test tier: 31,504 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 78.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h       8 records       8.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h      46 records      15.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     944 records      78.7 rec/h   usable
    last 24 h    1710 records      71.2 rec/h   usable
    12 h rate is 92% from ONE line (test); 3 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (79 rec/h); latest excludes cells already within
    8 of the ceiling (79 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         456       8  2026-08-13 00:38  2026-08-13 00:38  yes
      403       2,477       8  2026-08-14 02:20  2026-08-14 02:20  yes
      568       8,824       8  2026-08-17 11:01  2026-08-17 11:01  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 21% of the rung-568 backlog (1,848 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @208 cores      @830 cores   binding
       30           6.7 d           4.6 d   critical_chain
      100          15.2 d           4.6 d   critical_chain
      189          26.1 d           6.5 d   throughput
      279          37.0 d           9.3 d   throughput
      340          44.4 d          11.1 d   throughput
      403          52.1 d          13.0 d   throughput
      568          72.1 d          18.1 d   throughput
...
> **What is holding it right now: test_leg_deepseek_v4_pro -- distributional has 28 HOLE(S) below its frontier 339 -- that is what caps this line**
...
|---|---|---|---|---|
| nemotron_3_super | **305** | 307 | 5 |  |
| glm_5_2 | **309** | 311 | 5 |  |
| deepseek_v4_pro | **312** | 314 | 5 |  |
| test | **350** | 350 | 9 |  |
| kimi_k3 | **471** | 474 | 5 |  |
...
## Monitoring -- the cycle (**last monitoring cycle was 18 min ago -- the loop has lapsed**)
...
```
2026-08-12T18:24:57Z  OK  records=33047 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  sweep=64.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T18:26:22Z  OK  records=33047 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.7m  drift=0  sci=OK  r115=22B  sweep=54.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T18:27:51Z  OK  records=33047 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.2m  drift=0  sci=OK  r115=22B  sweep=59.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T18:29:26Z  OK  records=33047 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  cores=208  sweep=64.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T18:30:53Z  OK  records=33047 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  sweep=56.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T18:32:21Z  OK  records=33047 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.5m  drift=0  sci=OK  r115=22B  sweep=58.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
