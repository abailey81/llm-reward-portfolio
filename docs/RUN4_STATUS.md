...
**Auto-generated 2026-08-12 22:40 UTC -- T+361h31m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+361h31m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=2; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 29.0 h ago** |
...
| | |
|---|---|
| cluster jobs | **323** = 25 running + **164 ELIGIBLE** + 53 held by us + 81 held only by the site |
...
| **cores computing** | **200** |
| **cores doing RUNG-RAISING work** | **47.8%** -- 88 of 184 cores (24 min old) |
...
```
generated 2026-08-12 22:41 UTC | elapsed 15.06 d | 14.1 d to the Aug-27 stop
test tier: 31,536 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 54.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h      21 records       7.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     658 records      54.8 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         424       0  2026-08-13 06:25  2026-08-13 06:25  yes
      403       2,445       0  2026-08-14 19:17  2026-08-14 19:17  yes
      568       8,792       0  2026-08-19 15:02  2026-08-19 15:02  yes
