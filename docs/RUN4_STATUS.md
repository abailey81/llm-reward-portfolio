...
**Auto-generated 2026-08-11 22:02 UTC -- T+336h53m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+336h53m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 4.4 h ago; none live, newest failure 4.4 h ago** |
...
| | |
|---|---|
| cluster jobs | **414** = 80 running + **6 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **640** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (149 min old **STALE**) |
...
```
generated 2026-08-11 22:03 UTC | elapsed 14.04 d | 15.1 d to the Aug-27 stop
test tier: 30,011 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,588      63  GATED             GATED             barrier>=279
      403       3,793      63  GATED             GATED             barrier>=279
      568      10,317      63  GATED             GATED             barrier>=279
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 37% of the rung-568 backlog (3,767 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test_leg_deepseek_v4_pro -- scalar has 33 HOLE(S) below its frontier 337 -- that is what caps this line**
