...
**Auto-generated 2026-08-11 18:54 UTC -- T+333h45m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+333h45m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **31337** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 0.5 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
| LLM calls / spend | 2956 / **$45.5021** |
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 75 min ago; none live, newest failure 75 min ago** |
...
| | |
|---|---|
| cluster jobs | **439** = 62 running + **49 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **496** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (82 min old) |
...
```
generated 2026-08-11 18:55 UTC | elapsed 13.91 d | 15.2 d to the Aug-27 stop
test tier: 29,794 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 121.2 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h      80 records      80.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     309 records     103.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1455 records     121.2 rec/h   usable
    last 24 h    2790 records     116.2 rec/h   usable
    12 h rate is 90% from ONE line (test); 2 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,805      80  GATED             GATED             barrier>=279
      403       4,010      80  GATED             GATED             barrier>=279
      568      10,534      80  GATED             GATED             barrier>=279
...
> **What is holding it right now: test_leg_deepseek_v4_pro -- distributional has 28 HOLE(S) below its frontier 328 -- that is what caps this line**
...
currently at **295 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T18:36:59Z  OK  records=31296 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.4m  drift=0  sci=OK  r115=22B  sweep=783.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T18:49:14Z  ATTN  records=31334 (+38)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.3m  drift=0  sci=OK  r115=22B  cores=488  sweep=705.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T18:50:53Z  ATTN  records=31334 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.2m  drift=0  sci=OK  r115=22B  sweep=68.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T18:52:14Z  ATTN  records=31337 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.1m  drift=0  sci=OK  r115=22B  sweep=50.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T18:53:37Z  ATTN  records=31337 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=52.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T18:55:02Z  ATTN  records=31337 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  sweep=54.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
