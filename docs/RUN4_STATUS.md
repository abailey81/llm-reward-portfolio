...
**Auto-generated 2026-08-07 01:01 UTC -- T+219h52m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+219h52m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **827** = 106 running + **175 ELIGIBLE** + 546 held by us + 0 held only by the site |
...
| **cores computing** | **848** |
| **cores doing RUNG-RAISING work** | **33.6%** -- 288 of 856 cores (81 min old) |
...
```
generated 2026-08-07 01:01 UTC | elapsed 9.16 d | 20.0 d to the Aug-27 stop
test tier: 18,498 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last  3 h      71 records      23.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       2,468      42  GATED             GATED             no-records:test/distributional,test/scalar>=30
      189       5,583      42  GATED             GATED             no-records:test/distributional,test/scalar>=30
      279       9,113      42  GATED             GATED             no-records:test/distributional,test/scalar>=30
      340      11,562      71  GATED             GATED             no-records:test/distributional,test/scalar>=30
      403      14,397      71  GATED             GATED             no-records:test/distributional,test/scalar>=30
      568      21,830      71  GATED             GATED             no-records:test/distributional,test/scalar>=30
...
```
2026-08-07T00:51:28Z  OK  records=20016 (+24)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.2m  drift=0  sci=OK  r115=22B  sweep=706.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-07T00:52:29Z  OK  records=20017 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=30.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
