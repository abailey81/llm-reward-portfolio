...
**Auto-generated 2026-08-12 15:09 UTC -- T+354h00m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+354h00m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| **cores computing** | **336** |
| **cores doing RUNG-RAISING work** | **77.9%** -- 424 of 544 cores (97 min old **STALE**) |
...
```
generated 2026-08-12 15:10 UTC | elapsed 14.75 d | 14.4 d to the Aug-27 stop
test tier: 31,412 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 97.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     132 records     132.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     370 records     123.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1165 records      97.1 rec/h   usable
    last 24 h    2065 records      86.0 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         472      28  2026-08-12 20:02  2026-08-12 20:02  yes
      403       2,569     132  2026-08-13 17:38  2026-08-13 17:38  yes
      568       8,916     132  2026-08-16 11:00  2026-08-16 11:00  yes
...
currently at **343 seeds each** (read live from the archive on this publish). !! THE
