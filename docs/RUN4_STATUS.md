...
**Auto-generated 2026-08-11 16:04 UTC -- T+330h55m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+330h55m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 3.5 h ago; none live, newest failure 3.5 h ago** |
...
| | |
|---|---|
| cluster jobs | **477** = 75 running + **74 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **600** |
| **cores doing RUNG-RAISING work** | **36.1%** -- 240 of 664 cores (31 min old) |
...
```
generated 2026-08-11 16:04 UTC | elapsed 13.79 d | 15.3 d to the Aug-27 stop
test tier: 29,520 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 136.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     191 records     191.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     497 records     165.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1633 records     136.1 rec/h   usable
    last 24 h    2644 records     110.2 rec/h   usable
    12 h rate is 96% from ONE line (test); 2 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (136 rec/h); latest excludes cells already within
    8 of the ceiling (136 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         160       0  2026-08-11 17:15  2026-08-11 17:15  yes
      340       2,079     191  2026-08-12 07:21  2026-08-12 07:21  yes
      403       4,284     191  2026-08-12 23:33  2026-08-12 23:33  yes
      568      10,808     191  2026-08-14 23:30  2026-08-14 23:30  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 35% of the rung-568 backlog (3,743 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test -- baseline_return_minus_turnover has 121 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **286 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T15:56:39Z  OK  records=31024 (+7)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.3m  drift=0  sci=OK  r115=22B  sweep=68.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T15:58:01Z  OK  records=31034 (+10)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=0.2m  drift=0  sci=OK  r115=22B  sweep=51.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T15:59:21Z  OK  records=31034 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.5m  drift=0  sci=OK  r115=22B  sweep=49.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T16:00:54Z  OK  records=31041 (+7)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=62.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T16:02:13Z  OK  records=31050 (+9)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.3m  drift=0  sci=OK  r115=22B  sweep=49.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T16:03:34Z  OK  records=31057 (+7)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=50.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
