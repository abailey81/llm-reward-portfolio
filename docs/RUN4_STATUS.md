...
**Auto-generated 2026-08-04 22:43 UTC -- T+169h34m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+169h34m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 13 min ago; none live, newest failure 13 min ago** |
...
| | |
|---|---|
| cluster jobs | **994** (160 running, 832 queued) |
| **cores computing** | **1280** |
...
```
generated 2026-08-04 22:43 UTC | elapsed 7.07 d | 22.1 d to the Aug-27 stop
test tier: 13,116 records over 68 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     120 records     120.0 rec/h
    last  3 h     551 records     183.7 rec/h
    last 12 h    2196 records     183.0 rec/h
    last 24 h    4297 records     179.0 rec/h
    12 h rate is 67% from ONE line (test_leg_sonnet_5); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 68% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_sonnet_5, test_leg_gpt_5_6_luna) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (183 rec/h); latest excludes cells already within
    8 of the ceiling (59 rec/h). Window 12 h.
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
       30         240      12  GATED             GATED             barrier
      100       3,342      25  GATED             GATED             barrier>=30
      189       7,792      25  GATED             GATED             barrier>=30
      279      12,292      25  GATED             GATED             barrier>=30
      340      15,342      25  GATED             GATED             barrier>=30
      403      18,492      25  GATED             GATED             barrier>=30
      568      27,212     120  GATED             GATED             barrier>=30
...
2026-08-04T22:21:58Z  OK  records=14593 (+6)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  sweep=330.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
