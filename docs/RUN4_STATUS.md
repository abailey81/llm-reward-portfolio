...
| transport health | **timeouts 6h=156; worst streak 2/240 (0.8% to fatal), ops on core, 15 min ago; none live, newest failure 15 min ago** |
...
| **cores computing** | **664** |
| **cores doing RUNG-RAISING work** | **22.6%** -- 184 of 816 cores (125 min old **STALE**) |
...
test tier: 17,820 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279       9,495     156  GATED             GATED             unstarted:2-unit(s)-absent>=30
      340      12,240     156  GATED             GATED             unstarted:2-unit(s)-absent>=30
      403      15,075     156  GATED             GATED             unstarted:2-unit(s)-absent>=30
      568      22,508     156  GATED             GATED             unstarted:2-unit(s)-absent>=30
