...
**Auto-generated 2026-08-12 01:41 UTC -- T+340h32m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+340h32m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **506** = 96 running + **82 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **768** |
| **cores doing RUNG-RAISING work** | **56.7%** -- 440 of 776 cores (8 min old) |
...
```
generated 2026-08-12 01:42 UTC | elapsed 14.19 d | 14.9 d to the Aug-27 stop
test tier: 30,135 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
MEASURED test-tier throughput (record mtimes; an observation, not a model):
    ...
    last  3 h      86 records      28.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279          72      13  GATED             GATED             barrier
      340       1,464      51  GATED             GATED             barrier>=279
      403       3,669      51  GATED             GATED             barrier>=279
      568      10,193      51  GATED             GATED             barrier>=279
...
2026-08-12T01:40:56Z  OK  records=31673 (+22)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.1m  drift=0  sci=OK  r115=22B  cores=768  sweep=917.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
