...
**Auto-generated 2026-08-11 08:16 UTC -- T+323h07m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+323h07m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **30052** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 1.0 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
...
| | |
|---|---|
| cluster jobs | **605** = 144 running + **107 ELIGIBLE** + 354 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **100.0%** -- 1152 of 1152 cores (44 min old) |
...
```
generated 2026-08-11 08:17 UTC | elapsed 13.46 d | 15.7 d to the Aug-27 stop
test tier: 28,523 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 118.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     396 records     132.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1425 records     118.8 rec/h   usable
    last 24 h    2457 records     102.4 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (119 rec/h); latest excludes cells already within
    8 of the ceiling (119 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         941     123  2026-08-11 16:13  2026-08-11 16:13  yes
      340       3,076     123  2026-08-12 10:11  2026-08-12 10:11  yes
      403       5,281     123  2026-08-13 04:45  2026-08-13 04:45  yes
      568      11,805     123  2026-08-15 11:42  2026-08-15 11:42  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 6% of the rung-568 backlog (749 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
> **What is holding it right now: test -- baseline_return_minus_turnover has 172 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **235 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-11T07:48:25Z  OK  records=29989 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.1m  drift=0  sci=OK  r115=22B  sweep=54.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T07:49:49Z  OK  records=29989 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  sweep=53.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T07:51:18Z  OK  records=29994 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  sweep=58.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T07:52:40Z  OK  records=29997 (+3)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.6m  drift=0  sci=OK  r115=22B  sweep=51.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T07:54:15Z  OK  records=30005 (+8)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.1m  drift=0  sci=OK  r115=22B  cores=1152  sweep=64.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T07:55:39Z  OK  records=30005 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.5m  drift=0  sci=OK  r115=22B  sweep=53.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
