...
**Auto-generated 2026-08-04 23:56 UTC -- T+170h47m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+170h47m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **14891** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 7.3 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
| LLM calls / spend | 2956 / **$45.5021** |
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 86 min ago; none live, newest failure 20 min ago** |
...
| | |
|---|---|
| cluster jobs | **973** (168 running, 803 queued) |
| **cores computing** | **1344** |
...
```
generated 2026-08-04 23:57 UTC | elapsed 7.12 d | 22.0 d to the Aug-27 stop
test tier: 13,348 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     184 records     184.0 rec/h
    last  3 h     480 records     160.0 rec/h
    last 12 h    2255 records     187.9 rec/h
    last 24 h    4232 records     176.3 rec/h
    12 h rate is 58% from ONE line (test_leg_sonnet_5); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 58% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_sonnet_5, test_leg_gpt_5_6_luna) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       3,340       1  GATED             GATED             barrier>=30
      189       7,790       1  GATED             GATED             barrier>=30
      279      12,290       1  GATED             GATED             barrier>=30
      340      15,340       1  GATED             GATED             barrier>=30
      403      18,490       1  GATED             GATED             barrier>=30
      568      26,980     184  GATED             GATED             barrier>=30
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1344 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.6 d           4.6 d   critical_chain
      189           4.6 d           6.5 d   throughput
      279           5.7 d           9.3 d   throughput
      340           6.9 d          11.1 d   throughput
      403           8.1 d          13.0 d   throughput
      568          11.2 d          18.1 d   throughput
...
```
2026-08-04T23:06:06Z  OK  records=14734 (+36)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=536.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-04T23:11:14Z  OK  records=14754 (+20)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.3m  drift=0  sci=OK  r115=22B  sweep=278.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-04T23:19:55Z  ATTN  records=14778 (+24)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.2m  drift=0  sci=OK  r115=22B  cores=1312  sweep=490.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-04T23:25:02Z  ATTN  records=14803 (+25)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.2m  drift=0  sci=OK  r115=22B  sweep=276.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-04T23:39:37Z  ATTN  records=14834 (+31)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.3m  drift=0  sci=OK  r115=22B  sweep=845.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-04T23:52:59Z  ATTN  records=14872 (+38)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=99  stalest=2.1m  drift=0  sci=OK  r115=22B  cores=1360  sweep=770.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
