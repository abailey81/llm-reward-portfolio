...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 5.8 h ago; none live, newest failure 45 min ago** |
...
| **cores computing** | **520** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (52 min old) |
...
```
generated 2026-08-11 18:25 UTC | elapsed 13.89 d | 15.2 d to the Aug-27 stop
test tier: 29,756 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,843      89  GATED             GATED             barrier>=279
      403       4,048      89  GATED             GATED             barrier>=279
      568      10,572      89  GATED             GATED             barrier>=279
