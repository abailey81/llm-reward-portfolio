...
**Auto-generated 2026-08-04 22:36 UTC -- T+169h28m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+169h28m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| stalest driver log | **1 min (qwen3_5-9b)** old (P218: the STALEST of the still-running lines, completed ladders excluded; above ~30 means that line has stopped progressing) |
| records archived | **14636** |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 7 min ago; LIVE, still failing** |
...
```
generated 2026-08-04 22:37 UTC | elapsed 7.06 d | 22.1 d to the Aug-27 stop
test tier: 13,097 records over 68 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     106 records     106.0 rec/h
    last  3 h     554 records     184.7 rec/h
    last 12 h    2192 records     182.7 rec/h
    last 24 h    4308 records     179.5 rec/h
    12 h rate is 68% from ONE line (test_leg_sonnet_5); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 69% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_sonnet_5, test_leg_gpt_5_6_luna) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    8 of the ceiling (57 rec/h). Window 12 h.
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       3,343      26  GATED             GATED             barrier>=30
      189       7,793      26  GATED             GATED             barrier>=30
      279      12,293      26  GATED             GATED             barrier>=30
      340      15,343      26  GATED             GATED             barrier>=30
      403      18,493      26  GATED             GATED             barrier>=30
      568      27,231     106  GATED             GATED             barrier>=30
