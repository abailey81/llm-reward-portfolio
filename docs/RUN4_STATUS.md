...
**Auto-generated 2026-08-13 00:09 UTC -- T+363h00m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+363h00m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| stalest driver log | **5 min (qwen3_6-27b)** old (P218: the STALEST of the still-running lines, completed ladders excluded; above ~30 means that line has stopped progressing) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 30.5 h ago** |
...
| | |
|---|---|
| cluster jobs | **323** = 39 running + **218 ELIGIBLE** + 66 held by us + 0 held only by the site |
...
| **cores computing** | **312** |
| **cores doing RUNG-RAISING work** | **43.8%** -- 112 of 256 cores (38 min old) |
...
```
generated 2026-08-13 00:11 UTC | elapsed 15.13 d | 14.0 d to the Aug-27 stop
test tier: 31,536 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 41.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h     493 records      41.1 rec/h   usable
    last 24 h    1472 records      61.3 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (41 rec/h); latest excludes cells already within
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         424       0  2026-08-13 10:31  2026-08-13 10:31  yes
      403       2,445       0  2026-08-15 11:42  2026-08-15 11:42  yes
      568       8,792       0  2026-08-21 22:12  2026-08-21 22:12  yes
