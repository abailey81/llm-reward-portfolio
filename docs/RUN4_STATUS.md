...
**Auto-generated 2026-08-13 02:11 UTC -- T+365h02m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+365h02m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 32.5 h ago** |
...
| | |
|---|---|
| cluster jobs | **316** = 59 running + **191 ELIGIBLE** + 66 held by us + 0 held only by the site |
...
| **cores computing** | **472** |
| **cores doing RUNG-RAISING work** | **27.5%** -- 112 of 408 cores (37 min old) |
...
```
generated 2026-08-13 02:12 UTC | elapsed 15.21 d | 13.9 d to the Aug-27 stop
test tier: 31,536 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 21.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h       0 records       0.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     253 records      21.1 rec/h   usable
    last 24 h    1372 records      57.2 rec/h   usable
    12 h rate is 81% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (21 rec/h); latest excludes cells already within
    8 of the ceiling (21 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         424       0  2026-08-13 22:19  2026-08-13 22:19  yes
      403       2,445       0  2026-08-17 22:10  2026-08-17 22:10  yes
      568       8,792       0  2026-08-30 11:13  2026-08-30 11:13  NO
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @472 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           6.7 d           4.6 d   critical_chain
      189          11.5 d           6.5 d   throughput
      279          16.3 d           9.3 d   throughput
      340          19.6 d          11.1 d   throughput
      403          22.9 d          13.0 d   throughput
      568          31.8 d          18.1 d   throughput
...
## Monitoring -- the cycle (**last monitoring cycle was 20 min ago -- the loop has lapsed**)
...
```
2026-08-13T01:44:42Z  OK  records=33079 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.9m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=63.2s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-13T01:46:13Z  OK  records=33079 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.4m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=61.6s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-13T01:47:40Z  OK  records=33079 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.1m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=56.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-13T01:49:08Z  OK  records=33079 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=BLIND(deep_audit)  r115=22B  sweep=57.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
