...
**Auto-generated 2026-08-09 15:32 UTC -- T+282h23m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+282h23m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **626** = 136 running + **114 ELIGIBLE** + 376 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **99.3%** -- 1120 of 1128 cores (10 min old) |
...
```
generated 2026-08-09 15:33 UTC | elapsed 11.77 d | 17.4 d to the Aug-27 stop
test tier: 24,137 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 96.8 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     211 records     211.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h     388 records     129.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1161 records      96.8 rec/h   usable
    last 24 h    2722 records     113.4 rec/h   usable
    12 h rate is 61% from ONE line (test); 6 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      189       1,594     211  2026-08-10 08:01  2026-08-10 08:01  yes
      279       4,744     211  2026-08-11 16:35  2026-08-11 16:35  yes
      340       6,879     211  2026-08-12 14:39  2026-08-12 14:39  yes
      403       9,115     211  2026-08-13 13:45  2026-08-13 13:45  yes
      568      16,191     211  2026-08-16 14:54  2026-08-16 14:54  yes
...
> **What is holding it right now: test -- baseline_return_minus_downside has 277 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **130 seeds each** (read live from the archive on this publish). !! THE
...
```
2026-08-09T13:26:30Z  OK  records=25347 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  sweep=380.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-09T13:46:09Z  OK  records=25409 (+62)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  cores=1104  sweep=1147.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
