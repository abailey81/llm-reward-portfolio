...
**Auto-generated 2026-08-09 20:30 UTC -- T+287h21m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+287h21m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on kimi-k3, 3.9 h ago; none live, newest failure 3.9 h ago** |
...
| | |
|---|---|
| cluster jobs | **692** = 141 running + **175 ELIGIBLE** + 376 held by us + 0 held only by the site |
...
| **cores doing RUNG-RAISING work** | **100.0%** -- 1128 of 1128 cores (52 min old) |
...
```
generated 2026-08-09 20:31 UTC | elapsed 11.97 d | 17.1 d to the Aug-27 stop
test tier: 24,906 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 133.2 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h     122 records     122.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    ...
    last 12 h    1599 records     133.2 rec/h   usable
    last 24 h    2896 records     120.7 rec/h   usable
    12 h rate is 74% from ONE line (test); 5 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (133 rec/h); latest excludes cells already within
    8 of the ceiling (133 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      189         825     122  2026-08-10 02:42  2026-08-10 02:42  yes
      279       3,975     122  2026-08-11 02:20  2026-08-11 02:20  yes
      340       6,110     122  2026-08-11 18:22  2026-08-11 18:22  yes
      403       8,346     122  2026-08-12 11:09  2026-08-12 11:09  yes
      568      15,422     122  2026-08-14 16:15  2026-08-14 16:15  yes
...
> **What is holding it right now: test -- baseline_log_growth has 249 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **158 seeds each** (read live from the archive on this publish). !! THE
...
2026-08-09T20:12:41Z  OK  records=26400 (+45)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.7m  drift=0  sci=OK  r115=22B  sweep=1454.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
