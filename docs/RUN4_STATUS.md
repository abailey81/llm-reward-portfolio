...
**Auto-generated 2026-08-05 05:53 UTC -- T+176h44m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+176h44m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on qwen3_6-27b, 50 min ago; none live, newest failure 50 min ago** |
...
| | |
|---|---|
| cluster jobs | **968** (206 running, 762 queued) |
...
```
generated 2026-08-05 05:53 UTC | elapsed 7.36 d | 21.8 d to the Aug-27 stop
test tier: 13,830 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     212 records     212.0 rec/h
    last  3 h     233 records      77.7 rec/h
    last 12 h    1633 records     136.1 rec/h
    ...
    !! 51% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_qwen3_5_9b, test_leg_sonnet_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       3,238      72  GATED             GATED             barrier>=30
      189       7,548     212  GATED             GATED             barrier>=30
      279      12,048     212  GATED             GATED             barrier>=30
      340      15,098     212  GATED             GATED             barrier>=30
      403      18,248     212  GATED             GATED             barrier>=30
      568      26,498     212  GATED             GATED             barrier>=30
...
```
2026-08-05T05:21:24Z  OK  records=15205 (+15)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.2m  drift=0  sci=OK  r115=22B  sweep=288.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
