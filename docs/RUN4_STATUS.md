...
**Auto-generated 2026-08-11 17:51 UTC -- T+332h42m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+332h42m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 5.3 h ago; none live, newest failure 11 min ago** |
...
| | |
|---|---|
| cluster jobs | **454** = 65 running + **61 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **520** |
| **cores doing RUNG-RAISING work** | **4.8%** -- 24 of 504 cores (18 min old) |
...
```
generated 2026-08-11 17:52 UTC | elapsed 13.86 d | 15.3 d to the Aug-27 stop
test tier: 29,710 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 125.5 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     422 records     140.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1506 records     125.5 rec/h   usable
    last 24 h    2767 records     115.3 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         128      19  GATED             GATED             barrier
      340       1,889     100  GATED             GATED             barrier>=279
      403       4,094     100  GATED             GATED             barrier>=279
      568      10,618     100  GATED             GATED             barrier>=279
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 35% of the rung-568 backlog (3,767 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
currently at **294 seeds each** (read live from the archive on this publish). !! THE
...
2026-08-11T17:37:45Z  OK  records=31224 (+14)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.5m  drift=0  sci=OK  r115=22B  sweep=675.3s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T17:39:31Z  OK  records=31226 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  cores=512  sweep=76.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
