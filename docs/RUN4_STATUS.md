...
**Auto-generated 2026-08-05 02:00 UTC -- T+172h51m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+172h51m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 3.5 h ago; none live, newest failure 2.4 h ago** |
...
| | |
|---|---|
| cluster jobs | **995** (176 running, 818 queued) |
| **cores computing** | **1408** |
...
```
generated 2026-08-05 02:00 UTC | elapsed 7.20 d | 21.9 d to the Aug-27 stop
test tier: 13,575 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h      90 records      90.0 rec/h
    last  3 h     387 records     129.0 rec/h
    last 12 h    2143 records     178.6 rec/h
    last 24 h    4225 records     176.0 rec/h
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (179 rec/h); latest excludes cells already within
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      568      26,753      90  GATED             GATED             barrier>=30
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1408 cores      @830 cores   binding
      ...
      403           7.7 d          13.0 d   throughput
      568          10.7 d          18.1 d   throughput
...
```
2026-08-05T01:29:10Z  OK  records=15070 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.3m  drift=0  sci=OK  r115=22B  sweep=301.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
