...
**Auto-generated 2026-08-11 23:45 UTC -- T+338h36m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+338h36m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| records archived | **31594** |
| **Myriad maintenance** | **2026-08-12 from 08:00 UTC, at risk all day** (in 0.3 days). Delayed from Aug 11. Jobs may die and REQUEUE idempotently; the supervisors ride it. Playbook: docs/ops/MAINTENANCE_2026-08-12.md |
...
| | |
|---|---|
| cluster jobs | **516** = 84 running + **104 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **672** |
| **cores doing RUNG-RAISING work** | **37.8%** -- 248 of 656 cores (13 min old) |
...
```
generated 2026-08-11 23:46 UTC | elapsed 14.11 d | 15.0 d to the Aug-27 stop
test tier: 30,051 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 97.7 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     118 records      39.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1172 records      97.7 rec/h   usable
    last 24 h    2527 records     105.3 rec/h   usable
    12 h rate is 82% from ONE line (test); 3 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         118       2  2026-08-12 00:59  2026-08-12 00:59  yes
      340       1,548       2  2026-08-12 15:37  2026-08-12 15:37  yes
      403       3,753       2  2026-08-13 14:12  2026-08-13 14:12  yes
      568      10,277       2  2026-08-16 09:00  2026-08-16 09:00  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 22% of the rung-568 backlog (2,266 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.

REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @672 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           4.7 d           4.6 d   critical_chain
      189           8.1 d           6.5 d   throughput
      279          11.5 d           9.3 d   throughput
      340          13.7 d          11.1 d   throughput
      403          16.1 d          13.0 d   throughput
      568          22.3 d          18.1 d   throughput
...
```
2026-08-11T23:21:51Z  OK  records=31592 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.0m  drift=0  sci=OK  r115=22B  cores=656  sweep=59.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T23:23:14Z  OK  records=31592 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=1.9m  drift=0  sci=OK  r115=22B  sweep=52.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T23:30:24Z  OK  records=31593 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=399.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T23:31:49Z  OK  records=31593 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.8m  drift=0  sci=OK  r115=22B  sweep=55.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T23:33:12Z  OK  records=31593 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=52.4s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T23:34:41Z  OK  records=31593 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  sweep=58.8s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
