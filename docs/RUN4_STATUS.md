...
**Auto-generated 2026-08-11 13:39 UTC -- T+328h30m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+328h30m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on core, 65 min ago; none live, newest failure 65 min ago** |
...
| | |
|---|---|
| cluster jobs | **530** = 124 running + **78 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **992** |
| **cores doing RUNG-RAISING work** | **59.0%** -- 576 of 976 cores (6 min old) |
...
```
generated 2026-08-11 13:39 UTC | elapsed 13.69 d | 15.4 d to the Aug-27 stop
test tier: 29,092 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 117.9 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1415 records     117.9 rec/h   usable
    last 24 h    2421 records     100.9 rec/h   usable
    12 h rate is 83% from ONE line (test); 2 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      279         372     113  2026-08-11 16:49  2026-08-11 16:49  yes
      340       2,507     113  2026-08-12 10:55  2026-08-12 10:55  yes
      403       4,712     113  2026-08-13 05:37  2026-08-13 05:37  yes
      568      11,236     113  2026-08-15 12:57  2026-08-15 12:57  yes
...
> **What is holding it right now: test -- baseline_return_minus_downside has 142 HOLE(S) below its frontier 406 -- that is what caps this line**
...
currently at **265 seeds each** (read live from the archive on this publish). !! THE
...
2026-08-11T13:30:32Z  OK  records=30594 (+23)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.5m  drift=0  sci=OK  r115=22B  sweep=1315.0s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-11T13:39:34Z  OK  records=30628 (+34)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.7m  drift=0  sci=OK  r115=22B  cores=992  sweep=511.9s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
