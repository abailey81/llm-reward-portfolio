...
**Auto-generated 2026-08-12 17:28 UTC -- T+356h19m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+356h19m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| stalest driver log | **0 min (qwen3_6-27b)** old (P218: the STALEST of the still-running lines, completed ladders excluded; above ~30 means that line has stopped progressing) |
...
| **cores computing** | **224** |
| **cores doing RUNG-RAISING work** | **58.3%** -- 168 of 288 cores (115 min old **STALE**) |
...
```
generated 2026-08-12 17:28 UTC | elapsed 14.85 d | 14.3 d to the Aug-27 stop
test tier: 31,487 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 90.3 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last  3 h     167 records      55.7 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h    1084 records      90.3 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         472       0  2026-08-12 22:42  2026-08-12 22:42  yes
      403       2,494      10  2026-08-13 21:05  2026-08-13 21:05  yes
      568       8,841      10  2026-08-16 19:21  2026-08-16 19:21  yes
