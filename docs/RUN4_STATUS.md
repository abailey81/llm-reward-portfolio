...
**Auto-generated 2026-08-11 19:46 UTC -- T+334h37m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+334h37m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 2.1 h ago; none live, newest failure 2.1 h ago** |
...
| | |
|---|---|
| cluster jobs | **434** = 62 running + **44 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **496** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (13 min old) |
...
```
generated 2026-08-11 19:47 UTC | elapsed 13.94 d | 15.2 d to the Aug-27 stop
test tier: 29,842 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 116.3 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1396 records     116.3 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,757      51  GATED             GATED             barrier>=279
      403       3,962      51  GATED             GATED             barrier>=279
      568      10,486      51  GATED             GATED             barrier>=279
