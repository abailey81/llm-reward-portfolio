...
| **cores computing** | **816** |
| **cores doing RUNG-RAISING work** | **29.4%** -- 240 of 816 cores (14 min old) |
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340      11,641       3  GATED             GATED             no-records:test/distributional,test/scalar>=30
      403      14,476       3  GATED             GATED             no-records:test/distributional,test/scalar>=30
      568      21,909       3  GATED             GATED             no-records:test/distributional,test/scalar>=30
