...
**Auto-generated 2026-08-06 13:15 UTC -- T+208h06m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+208h06m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=21; worst streak 2/240 (0.8% to fatal), ops on kimi-k3, 35 min ago; none live, newest failure 35 min ago** |
...
| **cores computing** | **808** |
| **cores doing RUNG-RAISING work** | **22.6%** -- 184 of 816 cores (35 min old) |
...
```
generated 2026-08-06 13:20 UTC | elapsed 8.67 d | 20.4 d to the Aug-27 stop
test tier: 17,638 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      189       5,689       0  GATED             GATED             barrier>=30
      279       9,677      19  GATED             GATED             barrier>=30
      340      12,422      19  GATED             GATED             barrier>=30
      403      15,257      19  GATED             GATED             barrier>=30
      568      22,690      19  GATED             GATED             barrier>=30
    ...
    !! 92% of the rung-568 backlog (20,855 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
```
2026-08-06T13:02:12Z  ATTN  records=19177 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.6m  drift=0  sci=OK  r115=22B  sweep=227.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
