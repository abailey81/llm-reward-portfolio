...
**Auto-generated 2026-08-12 19:10 UTC -- T+358h01m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+358h01m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| stalest driver log | **4 min (qwen3_6-27b)** old (P218: the STALEST of the still-running lines, completed ladders excluded; above ~30 means that line has stopped progressing) |
| records archived | **33048** |
...
| transport health | **timeouts 6h=2; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 25.5 h ago** |
...
| **cores computing** | **208** |
| **cores doing RUNG-RAISING work** | **46.4%** -- 104 of 224 cores (97 min old **STALE**) |
...
```
generated 2026-08-12 19:11 UTC | elapsed 14.92 d | 14.2 d to the Aug-27 stop
test tier: 31,505 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 77.1 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    last  1 h       1 records       1.0 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last  3 h      40 records      13.3 rec/h   NOISE, not a rate: shorter than one job's 15.0 h quantum, so it samples the gaps between 8-record bursts
    last 12 h     925 records      77.1 rec/h   usable
    last 24 h    1711 records      71.3 rec/h   usable
    12 h rate is 94% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (77 rec/h); latest excludes cells already within
    8 of the ceiling (77 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         455       1  2026-08-13 01:05  2026-08-13 01:05  yes
      403       2,476       1  2026-08-14 03:18  2026-08-14 03:18  yes
      568       8,823       1  2026-08-17 13:38  2026-08-17 13:38  yes
    GATED = the relevant rate is zero, so no throughput number can date that row -- it is
    waiting on a stage barrier (C1 chain / C3 gate), not on cores.
    !! 18% of the rung-568 backlog (1,594 records) sits on cells that produced NOTHING in the 12 h window -- work behind a stage barrier (C1 chain / C3 gate) is not accelerated by redirected cores. Neither column models when it starts.
...
2026-08-12T18:55:52Z  OK  records=33047 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=3.1m  drift=0  sci=OK  r115=22B  sweep=1380.7s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-12T19:09:38Z  OK  records=33048 (+1)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  cores=208  sweep=795.1s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
