...
**Auto-generated 2026-08-06 05:58 UTC -- T+200h49m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+200h49m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| | |
|---|---|
| cluster jobs | **944** (73 running, 871 queued) |
| **cores computing** | **577** |
...
```
generated 2026-08-06 05:59 UTC | elapsed 8.37 d | 20.8 d to the Aug-27 stop
test tier: 17,276 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h     136 records     136.0 rec/h
    last  3 h     198 records      66.0 rec/h
    last 12 h    1293 records     107.8 rec/h
    last 24 h    3413 records     142.2 rec/h
    12 h rate is 38% from ONE line (test_leg_kimi_k3); 4 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 34% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_haiku_4_5) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       2,570       2  GATED             GATED             barrier>=30
      189       5,991     136  GATED             GATED             barrier>=30
      279      10,039     136  GATED             GATED             barrier>=30
      340      12,784     136  GATED             GATED             barrier>=30
      403      15,619     136  GATED             GATED             barrier>=30
      568      23,052     136  GATED             GATED             barrier>=30
...
2026-08-06T05:56:58Z  OK  records=18814 (+0)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.0m  drift=0  sci=OK  r115=22B  sweep=27.6s  auto-cycle
2026-08-06T05:57:58Z  OK  records=18816 (+2)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.9m  drift=0  sci=OK  r115=22B  sweep=29.4s  auto-cycle
```
