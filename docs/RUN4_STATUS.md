...
**Auto-generated 2026-08-11 20:10 UTC -- T+335h01m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+335h01m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), pull on deepseek-v4-pro, 2.5 h ago; none live, newest failure 2.5 h ago** |
...
| | |
|---|---|
| cluster jobs | **432** = 60 running + **44 ELIGIBLE** + 328 held by us + 0 held only by the site |
...
| **cores computing** | **480** |
| **cores doing RUNG-RAISING work** | **46.9%** -- 240 of 512 cores (38 min old) |
...
```
generated 2026-08-11 20:11 UTC | elapsed 13.96 d | 15.2 d to the Aug-27 stop
test tier: 29,882 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 115.4 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h    1385 records     115.4 rec/h   usable
    last 24 h    2791 records     116.3 rec/h   usable
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340       1,717      88  GATED             GATED             barrier>=279
      403       3,922      88  GATED             GATED             barrier>=279
      568      10,446      88  GATED             GATED             barrier>=279
...
currently at **298 seeds each** (read live from the archive on this publish). !! THE
