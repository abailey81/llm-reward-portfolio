...
**Auto-generated 2026-08-13 02:00 UTC -- T+364h51m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+364h51m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 0/240 (0.0% to fatal), - on -, age UNKNOWN; none live, newest failure 32.3 h ago** |
...
| | |
|---|---|
| cluster jobs | **318** = 57 running + **195 ELIGIBLE** + 66 held by us + 0 held only by the site |
...
| **cores computing** | **456** |
| **cores doing RUNG-RAISING work** | **27.5%** -- 112 of 408 cores (27 min old) |
...
```
generated 2026-08-13 02:01 UTC | elapsed 15.20 d | 13.9 d to the Aug-27 stop
test tier: 31,536 records over 71 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    => OPERATIVE RATE 23.0 rec/h  (the 12 h window; the shortest one an ETA may be priced from)
    ...
    last 12 h     276 records      23.0 rec/h   usable
    last 24 h    1382 records      57.6 rec/h   usable
    12 h rate is 83% from ONE line (test); 4 line(s) contributed at all
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (23 rec/h); latest excludes cells already within
    8 of the ceiling (23 rec/h). Window 12 h.
    ...
    !! `remaining` IS A RECORD COUNT AND `REACHED` MEANS *THE COUNT IS MET* --
     ...
     rung  rec-count left     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      340         424       0  2026-08-13 20:27  2026-08-13 20:27  yes
      403       2,445       0  2026-08-17 12:19  2026-08-17 12:19  yes
      568       8,792       0  2026-08-29 00:17  2026-08-29 00:17  NO
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung      @456 cores      @830 cores   binding
       30           4.6 d           4.6 d   critical_chain
      100           7.0 d           4.6 d   critical_chain
      189          11.9 d           6.5 d   throughput
      279          16.9 d           9.3 d   throughput
      340          20.3 d          11.1 d   throughput
      403          23.8 d          13.0 d   throughput
      568          32.9 d          18.1 d   throughput
