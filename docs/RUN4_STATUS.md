...
**Auto-generated 2026-08-05 02:13 UTC -- T+173h04m.** Refreshed about every 1-1.5 minutes (measured; the publish itself takes
...
| | |
|---|---|
| elapsed | **T+173h04m** (launched 2026-07-28 21:08 UTC; exogenous stop 2026-08-27) |
...
| transport health | **timeouts 6h=0; worst streak 1/240 (0.4% to fatal), ops on glm-5_2, 3.7 h ago; none live, newest failure 2.6 h ago** |
...
| | |
|---|---|
| cluster jobs | **990** (179 running, 810 queued) |
| **cores computing** | **1432** |
...
```
generated 2026-08-05 02:14 UTC | elapsed 7.21 d | 21.9 d to the Aug-27 stop
test tier: 13,586 records over 69 of the 71 registered units (lanes.py _TEST_UNITS_PER_RUNG)

MEASURED test-tier throughput (record mtimes; an observation, not a model):
    last  1 h      72 records      72.0 rec/h
    last  3 h     352 records     117.3 rec/h
    last 12 h    2099 records     174.9 rec/h
    last 24 h    4215 records     175.6 rec/h
    12 h rate is 43% from ONE line (test_leg_sonnet_5); 10 line(s) contributed at all
    (windows under 12 h are a STALL INDICATOR ONLY and do not price the ETA -- the arrival quantum is a 15 h pack-8 job)
    !! 72% of the 12 h window came from cell(s) now within 8 records of rung 568 (test_leg_sonnet_5, test_leg_qwen3_5_9b) -- that rate STOPS. The ETA below assumes the cluster redirects those slots; it is an assumption, not a measurement.
...
EMPIRICAL ETA -- BOTH columns divide total remaining by a FLEET-WIDE rate, so both
    ...
    the whole fleet (175 rec/h); latest excludes cells already within
    ...
    'is this plausible on current throughput', NOT as an assurance verdict.
     rung   remaining     -1h  earliest (UTC)    latest (UTC)      Aug-27?
      ...
      100       3,325      13  GATED             GATED             barrier>=30
      189       7,775      13  GATED             GATED             barrier>=30
      279      12,275      13  GATED             GATED             barrier>=30
      340      15,325      13  GATED             GATED             barrier>=30
      403      18,475      13  GATED             GATED             barrier>=30
      568      26,742      72  GATED             GATED             barrier>=30
...
REGISTERED MODEL (src/cluster/lanes.py) -- a DURATION from a standing start, not a date:
     rung     @1432 cores      @830 cores   binding
...
2026-08-05T02:06:43Z  ATTN  records=15123 (+6)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.4m  drift=0  sci=OK  r115=22B  sweep=238.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
2026-08-05T02:11:49Z  ATTN  records=15128 (+5)  spend=$45.5019  guards=0n/2k  arms_full=10/10legs-ever  budget=2  stalest=2.6m  drift=0  sci=OK  r115=22B  sweep=275.5s(SWEEP-BOUND: >30s sleep)  auto-cycle
```
